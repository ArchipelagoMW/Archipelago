...
class KHCOMContext(CommonContext):
    command_processor: int = KHCOMClientCommandProcessor
    game = "Kingdom Hearts Chain of Memories"
    items_handling = 0b111  # full remote

    def __init__(self, server_address, password):
        super(KHCOMContext, self).__init__(server_address, password)
...
async def game_watcher(ctx: KHCOMContext):
...
if __name__ == '__main__':
    async def main(args):
        ...
        progression_watcher = asyncio.create_task(
            game_watcher(ctx), name="KHCOMProgressionWatcher")
    ...
    parser = get_base_parser(description="KHCOM Client, for text interfacing.")
