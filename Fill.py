...
def distribute_planned(multiworld: MultiWorld) -> None:
    ...
    early_locations: dict[int, list[str]] = defaultdict(list)
    non_early_locations: dict[int, list[str]] = defaultdict(list)
    ...
    plando_blocks: list[dict[str, object]] = []  # TODO: TypedDict for plando block
    ...
    for placement in plando_blocks:
        ...
        try:
            ...
            count = 0
            err: list[str] = []
            successful_pairs: list[tuple[int, Item, Location]] = []
            claimed_indices: set[int | None] = set()
            for item_name in items:
                ...
                if from_pool:
                    try:
                        ...
                        warn(f"Could not remove {item_name} from pool for "
                             f"{multiworld.player_name[player]} as it's already missing from it.",
                             placement['force'])
