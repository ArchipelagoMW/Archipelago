...
class CollectionState():
    ...
    def can_extend_magic(self, player: int, smallmagic: int = 16,
        ...
        if self.can_buy_unlimited('Green Potion', player) or self.can_buy_unlimited('Blue Potion', player):
            if self.multiworld.item_functionality[player] == ItemFunc.option_hard and not fullrefill:
                basemagic = basemagic + int(basemagic * 0.5 * self.bottle_count(player))
            elif self.multiworld.item_functionality[player] == ItemFunc.option_expert and not fullrefill:
    ...
    def is_not_bunny(self, region: Region, player: int) -> bool:
        ...
        return region.is_light_world if not self.multiworld.world_state[player].inverted else region.is_dark_world
    ...
    def has_misery_mire_medallion(self, player: int) -> bool:
        return self.has(self.multiworld.misery_mire_medallion[player].current_option_name, player)

    def has_turtle_rock_medallion(self, player: int) -> bool:
        return self.has(self.multiworld.turtle_rock_medallion[player].current_option_name, player)

    def can_boots_clip_lw(self, player: int) -> bool:
        if self.multiworld.world_state[player].inverted:
            return self.has('Pegasus Boots', player) and self.has('Moon Pearl', player)
        return self.has('Pegasus Boots', player)

    def can_boots_clip_dw(self, player: int) -> bool:
        if not self.multiworld.world_state[player].inverted:
            return self.has('Pegasus Boots', player) and self.has('Moon Pearl', player)
        return self.has('Pegasus Boots', player)
