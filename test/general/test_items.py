...
class TestBase(unittest.TestCase):
        ...
        for game_name, testable_world in AutoWorldRegister.testable_worlds.items():
            ...
            for options_name, options in testable_world.testable_options_by_name.items():
                multiworld = setup_solo_multiworld(
                    world_type, steps=("generate_early", "create_regions", "create_items"), options=options
                )
                proxy_world = multiworld.worlds[1]
                for item_name in world_type.item_name_to_id:
                    test_state = CollectionState(multiworld)
                    with self.subTest("Create Item", item_name=item_name, game_name=game_name, options=options_name):
                        item = proxy_world.create_item(item_name)

                    with self.subTest("Item Name", item_name=item_name, game_name=game_name, options=options_name):
                        self.assertEqual(item.name, item_name)

                    if item.advancement:
                        with self.subTest(
                            "Item State Collect", item_name=item_name, game_name=game_name, options=options_name
                        ):
                            test_state.collect(item, True)

                        with self.subTest(
                            "Item State Remove", item_name=item_name, game_name=game_name, options=options_name
                        ):
                        ...
                        with self.subTest(
                            "Item State Collect No Change", item_name=item_name, game_name=game_name, options=options
                        ):
    ...
    def test_items_in_datapackage(self):
        ...
        for game_name, testable_world in AutoWorldRegister.testable_worlds.items():
            world_type = testable_world.world_type
            for options_name, options in testable_world.testable_options_by_name.items():
                with self.subTest("Game", game=game_name, options_name=options_name):
                    multiworld = setup_solo_multiworld(world_type, options=options)
    ...
    def test_item_links(self) -> None:
        ...
        """
        def setup_link_multiworld(world: Type[World], link_replace: bool, test_options: dict[str, Any]) -> None:
            ...
            for name, option in world.options_dataclass.type_hints.items():
                option_value = test_options.get(name, option.default)
                setattr(args, name, {1: option.from_any(option_value), 2: option.from_any(option_value)})
        ...
        for game_name, testable_world in AutoWorldRegister.testable_worlds.items():
            world_type = testable_world.world_type
            for options_name, options in testable_world.testable_options_by_name.items():
                with self.subTest("Can generate with link replacement", game=game_name, options=options_name):
                    setup_link_multiworld(world_type, True, options)
                with self.subTest("Can generate without link replacement", game=game_name, options=options_name):
                    setup_link_multiworld(world_type, False, options)
