...
def get_color_palette(colors_needed: int) -> list[RGB]:
    ...
    step = (5 ** 0.5 - 1)/2 # golden ratio to maximize color separation
    for n in range(colors_needed):
        # a bit of noise on value to add some luminosity difference
        colors.append(RGB(*(val * 255 for val in hsv_to_rgb(step * n, 0.8, 0.8 + (step * n % 0.2)))))

    return colors


def truncated_name(game: str) -> str:
    if len(game) > 20:
        return f"{game[:18]}…"
    return game
...
def create_game_played_figure(all_games_data: dict[date, dict[str, int]], game: str, color: RGB) -> figure:
    ...
    plot.vbar(x="days", top="played", color=color, source=ColumnDataSource(data=data), width=timedelta(days=0.5))
    return plot
...
def stats():
    ...
    # give in order of most common so colors next to each other on pie chart can't randomly be similar
    game_to_color: dict[str, RGB] = {game: color for (game, _), color in zip(total_games.most_common(), color_palette)}
    ...
    for game in sorted(total_games):
        ...
        plot.line([datetime.combine(day, datetime.min.time()) for day in days],
                  occurences, legend_label=truncated_name(game), line_width=2, color=game_to_color[game])
    ...
    for i, (game, count) in enumerate(total_games.most_common()):
        ...
        data["games_abbrev"].append(truncated_name(game))
    ...
    pie.wedge(x=0, y=0, radius=0.5,
              ...
              source=ColumnDataSource(data=data), legend_field="games_abbrev")
