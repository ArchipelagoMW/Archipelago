...
#     item: ItemData
#     count: int
#     def __init__(self, item: ItemData, count: int):
#         self.item = item
#         self.count = count
#
# class POYItemPool:
#     item_register: dict[str, ItemData]
#     required_items: dict[int, ItemRequirement]
#     populated: bool
#     def __init__(self, item_register: list[ItemData]):
#         self.item_register = {i.name: i for i in item_register}
#         self.required_items = []
#         self.populated = False
#
#     def add_required_items(self, required_items: list[ItemRequirement]):
#         for item_req in required_items:
#             if item_req.item not in self.item_register:
#                 raise ValueError("Required item is not registered")
#             if item_req.item.id not in self.required_items.keys():
#                 self.required_items[item_req.item.id] = item_req
#             else:
#                 self.required_items[item_req.item.id].count = (
#                     max(self.required_items[item_req.item.id].count, item_req.count))
#         self.populated = True
#
#     def build_full_pool(self) -> list[ItemData]:
#         pool: list[ItemData] = []
#         for item in self.item_register.values():
#             if item.id in self.required_items:
#                 req_item = self.required_items[item.id]
#                 for i in range(req_item.count):
#                     pool.append(ItemData(item.name, item.id, ItemClassification.progression, item.type, item.is_starter_item, item.is_enabled, item.is_early))
#                     pool.append(req_item.item)
#             else:
#                 pool.append(item)
#         return pool
#
#     def get_item_name_to_id_dict(self) -> dict[str, int]:
#         return {i.name: i.id for i in self.item_register.values()}
#
