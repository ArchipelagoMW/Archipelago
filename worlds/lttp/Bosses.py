...
def can_beat_helma(state, player: int):
    return state.has_sword(player, 1) or state.can_shoot_arrows(player) or (state.has_hammer(player) if logic_trick.hamdorm[player] else False)


def can_beat_arrghus(state, player: int):
    if not state.has('Hookshot', player): return False
    if state.has_melee_weapon(player): return True
    return (
