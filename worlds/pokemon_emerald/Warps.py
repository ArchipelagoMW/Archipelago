...
class Warp:
    ...
    def decode(encoded_string: str):
        ...
        warp_dest_index = int(warp_dest_index) if (not warp_dest_index in special_warp_ids) else -1
    ...
    if (warp_map == None):
        ...
        for warp in warps_json:
            for other_warp in warps_json:
                if (warp in warp_map): continue
                if (warps_connect_ltr(warp, other_warp)):
                    warp_map[warp] = other_warp
            
            if (not warp in warp_map):
                warp_map[warp] = None
