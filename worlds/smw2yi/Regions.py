...
def create_regions(world: MultiWorld, player: int, locations: Tuple[LocationData, ...], location_cache: List[Location]):
    ...
    connect(world, player, names, 'World 6', "King Bowser's Castle", lambda state: state.castle_unlock_con(world, player)),
...
def connectStartingRegion(world: MultiWorld, player: int):
    ...
    tutorial = world.get_region('Overworld', player)
    ...
    menu_to_tutorial = Entrance(player, 'Overworld', menu)
    menu_to_tutorial.connect(tutorial)
    menu.exits.append(menu_to_tutorial)

    tutorial_to_start = Entrance(player, 'Overworld', tutorial)
