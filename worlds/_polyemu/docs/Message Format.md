...
|`0x04`|Game ID|Returns some identifier for the game. This remains consistent until the game is swapped for another, and is only used to track that the same game is loaded as was during previous requests. It could be a hash of the ROM, but may also be a timestamp representing when the game was first loaded, or any other arbitrary data.|
...
|`0x84`|Game ID|Returns the game id.|
|`0x90`|Read|Contai.|
...
|Offset|Field Name|Size|Description|
|--|--|--|--|
|`0x00`|Game ID|8|A sequence of bytes identifying the individual ROM being played.|
