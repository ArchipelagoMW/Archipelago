...
def get_locations(multiworld: Optional[MultiWorld], player: Optional[int]) -> Tuple[LocationData, ...]:
    ...
    if not multiworld or is_option_enabled(multiworld, player, "SpeedySpirits"):
        location_table += (
            LocationData('Wardrobe', 'Wardrobe Speedy Spirit', 8675),
            LocationData('Study', 'Study Speedy Spirit', 8676),
            LocationData('Nursery', 'Nursery Speedy Spirit', 8677),
            LocationData('Storage Room', 'Storage Room Speedy Spirit', 8678),
            LocationData('Hidden Room', 'Hidden Room Speedy Spirit', 8679),
            LocationData('Conservatory', 'Conservatory Speedy Spirit',8680),
            LocationData('Dining Room', 'Dining Room Speedy Spirit',8681),
            LocationData('Kitchen', 'Kitchen Speedy Spirit', 8682),
            LocationData('Rec Room', 'Rec Room Speedy Spirit', 8683),
            LocationData('Nana\' Room', 'Nana\'s Room Speedy Spirit', 8684),
            LocationData('Billiards Room', 'Billiards Room Speedy Spirit', 8685),
            LocationData('Twins\' Room', 'Twins\' Room Speedy Spirit', 8686),
            LocationData('Breaker Room', 'Breaker Room Speedy Spirit', 8687),
            LocationData('Cellar', 'Cellar Speedy Spirit', 8688),
            LocationData('Sealed Room', 'Sealed Room Speedy Spirit',8689),
            # Gold Mice Locations
            LocationData('Study', 'Study Cheese Gold Mouse', 8690),
            LocationData('Fortune-Teller\'s Room', 'Fortune-Teller\'s Room Cheese Gold Mouse', 8691),
            LocationData('Dining Room', 'Dining Room Cheese Gold Mouse', 8692),
            LocationData('Tea Room','Tea Room Cheese Gold Mouse',8693),
            LocationData('Safari Room', 'Safari Room Cheese Gold Mouse', 8694),
            LocationData('1F Hallway', '1F Hallway Chance Gold Mouse',8695),
            LocationData('Kitchen', 'Kitchen Chance Gold Mouse',8696),
            LocationData('Tea Room', 'Tea Room Chance Gold Mouse', 8697),
            LocationData('2F Rear Hallway', '2F Rear Hallway Chance Gold Mouse', 8698),
            LocationData('Sealed Room', 'Sealed Room Chance Gold Mouse', 8699)
        )
    ...
    if not multiworld or is_option_enabled(multiworld, player, "Boosanity"):
        location_table += (
            LocationData('Parlor', 'Parlor Boo', 8700, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Anteroom', 'Anteroom Boo', 8701, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Wardrobe', 'Wardrobe Boo', 8702, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Study', 'Study Boo', 8703, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Master Bedroom', 'Master Bedroom Boo', 8704,
                         rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Nursery', 'Nursery Boo', 8705, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Twins\' Room', 'Twins\' Room Boo', 8706,
                         rule=lambda state: state.has_group("Medal", player) and state.has("Boo Radar", player)),
            LocationData('Laundry Room', 'Laundry Room Boo', 8707, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Butler\'s Room', 'Butler\'s Room Boo', 8708,
                         rule=lambda state: state.has("Fire Element Medal", player) and state.has("Boo Release", player)
                                            and state.has("Boo Radar", player)),
            LocationData('Hidden Room', 'Hidden Room Boo', 8709, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Fortune-Teller\'s Room', 'Fortune-Teller\'s Room Boo', 8710,
                         rule=lambda state: state.has_group("Mario Item", player, multiworld.MarioItems[player])
                         and state.has("Boo Radar", player)),
            LocationData('Mirror Room', 'Mirror Room Boo', 8711, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Ballroom', 'Ballroom Boo', 8712, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Storage Room', 'Storage Room Boo', 8713, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Dining Room', 'Dining Room Boo', 8714,
                         rule=lambda state: state.has("Fire Element Medal", player) and state.has("Boo Radar", player)),
            LocationData('Kitchen', 'Kitchen Boo', 8715, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Conservatory', 'Conservatory Boo', 8716, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Rec Room', 'Rec Room Boo', 8717, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Billiards Room', 'Billiards Room Boo', 8718, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Projection Room', 'Projection Room Boo', 8719,
                         rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Tea Room', 'Tea Room Boo', 8720, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Nana\'s Room', 'Nana\'s Room Boo', 8721, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Sitting Room', 'Sitting Room Boo', 8722,
                         rule=lambda state: state.has("Fire Element Medal", player)
                                            and state.has("Water Element Medal", player)
                                            and state.has("Boo Radar", player)
                         ),
            LocationData('Guest Room', 'Guest Room Boo', 8723,
                         rule=lambda state: state.has("Water Element Medal", player) and state.has("Boo Radar", player)),
            LocationData('Safari Room', 'Safari Room Boo', 8724, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Artist\'s Studio', 'Artist\'s Studio Boo', 8725,
                         rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Armory', 'Armory Boo', 8726, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Ceramics Studio', 'Ceramics Studio Boo', 8727,
                         rule=lambda state: state.has("Ice Element Medal", player)
                                            and state.has("Boo Radar", player)),
            LocationData('Telephone Room', 'Telephone Room Boo', 8728, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Clockwork Room', 'Clockwork Room Boo', 8729, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Astral Hall', 'Astral Hall Boo', 8730,
                         rule=lambda state: state.has("Fire Element Medal", player)
                                            and state.has("Boo Radar", player)),
            LocationData('Breaker Room', 'Breaker Room Boo', 8731,
                         rule=lambda state: state.has("Blackout", player) and state.has("Boo Radar", player)),
            LocationData('Cellar', 'Cellar Boo', 8732, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Pipe Room', 'Pipe Room Boo', 8733, rule=lambda state: state.has("Boo Radar", player)),
            LocationData('Cold Storage', 'Cold Storage Boo', 8734,
                         rule=lambda state: state.has("Fire Element Medal", player)
                                            and state.has("Boo Radar", player))
