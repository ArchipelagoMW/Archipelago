...
def create_regions(world: MultiWorld, player: int, locations: LMLocation[Location, ...], location_cache: List[Location]):
    ...
    connect(world, player, names, '1F Hallway', '2F Stairwell', lambda state: state.has("2F Stairwell Key", player))
    ...
    connect(world, player, names, 'Rec Room', 'Courtyard', lambda state: state.has("Rec Room Key", player))
    ...
    connect(world, player, names, 'Astral Hall', 'Observatory', lambda state: state.has("Fire Element Medal", player))
