            ...
            for location in locations_to_hint[:hint_count]:
                if location == 'Swamp Left':
                    if local_random.randint(0, 1):
                        first_item = hint_text(multiworld.get_location('Swamp Palace - West Side Free Chest', player).item)
                        second_item = hint_text(multiworld.get_location('Swamp Palace - West Side Switch Chest', player).item)
                    else:
                        second_item = hint_text(multiworld.get_location('Swamp Palace - West Side Free Chest', player).item)
                        first_item = hint_text(multiworld.get_location('Swamp Palace - West Side Switch Chest', player).item)
                    this_hint = ('The westmost chests in Swamp Palace contain ' + first_item + ' and ' + second_item + '.')
                    tt[hint_locations.pop(0)] = this_hint
                elif location == 'Mire Left':
                    if local_random.randint(0, 1):
                        first_item = hint_text(multiworld.get_location('Misery Mire - Torch Tiles Chest', player).item)
                        second_item = hint_text(multiworld.get_location('Misery Mire - Cutscene Chest', player).item)
                    else:
                        second_item = hint_text(multiworld.get_location('Misery Mire - Torch Tiles Chest', player).item)
                        first_item = hint_text(multiworld.get_location('Misery Mire - Cutscene Chest', player).item)
