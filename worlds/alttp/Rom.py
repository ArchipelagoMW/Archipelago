...
def patch_enemizer(world, player: int, rom: LocalRom, enemizercli, output_directory):
    ...
    options = {
        ...
        'ManualBosses': {
            ...
            'GanonsTower1':
                world.get_dungeon('Ganons Tower' if not world.world_state[player].inverted
...
def patch_rom(world, rom, player, enemized):
            ...
            if exit.target is not None and exit.player == player:
                if isinstance(exit.addresses, tuple):
                    ...
                    if world.shuffle[player] not in {'insanity', 'insanity_legacy', 'madness_legacy'} and \
                            ...
                            (world.glitches_required[player] not in [Logic.alias_hmg, Logic.option_no_logic] or
    ...
    if difficulty.progressive_bow_limit < 2 and (
            world.swordless[player] or world.glitches_required[player] == Logic.option_no_glitches):
    ...
    rom.write_byte(0x18004A, 0x00 if not world.world_state[player].inverted else 0x01)  # Inverted mode
    rom.write_byte(0x18005D, 0x00)  # Hammer always breaks barrier
    rom.write_byte(0x2AF79, 0xD0 if not \
        world.world_state[player].inverted else 0xF0)  # vortexes: Normal  (D0=light to dark, F0=dark to light, 42 = both)
    rom.write_byte(0x3A943, 0xD0 if not \
        world.world_state[player] else 0xF0)  # Mirror: Normal  (D0=Dark to Light, F0=light to dark, 42 = both)
    rom.write_byte(0x3A96D, 0xF0 if not \
        world.world_state[player] else 0xD0)  # Residual Portal: Normal  (F0= Light Side, D0=Dark Side, 42 = both (Darth Vader))
    ...
    rom.write_byte(0x1800A4, 0x01 if world.glitches_required[player] != Logic.option_no_logic else 0x00)  # enable POD EG fix
    rom.write_byte(0x186383, 0x01 if world.glitch_triforce or world.glitches_required[
        player] == Logic.option_no_logic else 0x00)  # disable glitching to Triforce from Ganons Room
    ...
    if world.shuffle[player] in ['restricted', 'full', 'crossed', 'insanity', 'madness'] or (
            world.shuffle[player] == 'simple' and world.world_state[player].inverted):
...
def apply_rom_settings(rom, beep, color, quickswap, menuspeed, music: bool, sprite: str, palettes_options,
    ...
    if triforcehud:
        ...
        triforce_flag = (rom.read_byte(0x180167) & 0x80) | \
                        {'always_show': 0x00, 'hide_goal': 0x01, 'hide_required': 0x02, 'hide_both': 0x03}[triforcehud]
