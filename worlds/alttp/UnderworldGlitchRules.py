...
def dungeon_reentry_rules(world, player, clip: Entrance, dungeon_region: str, dungeon_exit: str): 
    ...
    if not fix_dungeon_exits: # vanilla, simple, restricted, dungeonssimple; should never have fake worlds fix
        ...
        if dungeon_entrance.name == 'Skull Woods Final Section': 
            ...
            add_rule(clip, lambda state: state.lttp_has_swords(player) and state.lttp_has_misery_mire_medallion(player)) # open the dungeon
        elif dungeon_entrance.name == 'Agahnims Tower': 
            add_rule(clip, lambda state: state.has('Cape', player) or state.lttp_has_swords(player, 2) or state.has('Beat Agahnim 1', player)) # kill/bypass barrier
...
def underworld_glitches_rules(world, player): 
    ...
    add_rule(world.get_entrance('Ice Palace Entrance Room', player), lambda state: state.lttp_can_bomb_clip(world.get_region('Ice Palace (Entrance)', player), player), combine='or')
    add_rule(world.get_location('Ice Palace - Freezor Chest', player), lambda state: state.lttp_can_melt_things(player))


    # Kiki Skip
    kikiskip = world.get_entrance('Kiki Skip', player)
    set_rule(kikiskip, lambda state: state.lttp_can_bomb_clip(kikiskip.parent_region, player))
    ...
    mire = world.get_region('Misery Mire (West)', player)
    mire_clip = lambda state: state.can_reach('Misery Mire (West)', 'Region', player) and state.lttp_can_bomb_clip(mire, player) and state.lttp_has_fire(player)
    hera_clip = lambda state: state.can_reach('Tower of Hera (Top)', 'Region', player) and state.lttp_can_bomb_clip(world.get_region('Tower of Hera (Top)', player), player)
