...
def global_rules(world: MultiWorld, player: int):
    world = world.worlds[player]
    ...
    set_rule(world.get_location('Tower of Hera - Basement Cage', player), lambda state: can_activate_crystal_switch(state, player))
    set_rule(world.get_location('Tower of Hera - Map Chest', player), lambda state: can_activate_crystal_switch(state, player))
    set_rule(world.get_entrance('Tower of Hera Small Key Door', player), lambda state: can_activate_crystal_switch(state, player) and (state._lttp_has_key('Small Key (Tower of Hera)', player) or location_item_name(state, 'Tower of Hera - Big Key Chest', player) == ('Small Key (Tower of Hera)', player)))
    set_rule(world.get_entrance('Tower of Hera Big Key Door', player), lambda state: can_activate_crystal_switch(state, player) and state.has('Big Key (Tower of Hera)', player))
    if world.options.enemy_shuffle:
    ...
    set_rule(world.get_location('Thieves\' Town - Blind\'s Cell', player),
             lambda state: state._lttp_has_key('Small Key (Thieves Town)', player))
    if world.options.accessibility != 'full' and not world.worlds[player].options.key_drop_shuffle:
    ...
    set_rule(world.get_location('Misery Mire - Spike Chest', player), lambda state: (world.can_take_damage and has_hearts(state, player, 4)) or state.has('Cane of Byrna', player) or state.has('Cape', player))
    ...
    set_rule(world.get_entrance('Turtle Rock Big Key Door', player), lambda state: state.has('Big Key (Turtle Rock)', player) and can_kill_most_things(state, player, 10) and can_bomb_or_bonk(state, player))
    ...
    for location in randomizer_room_chests:
        set_rule(world.get_location(location, player), lambda state: can_use_bombs(state, player) and (state._lttp_has_key('Small Key (Ganons Tower)', player, 8) or (
...
def bomb_jump_requirements(world, player):
    ...
    for location in DMs_room_chests:
        add_rule(world.get_location(location, player), lambda state: can_use_bombs(state, player), combine="or")
    set_rule(world.get_entrance('Paradox Cave Bomb Jump', player), lambda state: can_use_bombs(state, player))
    set_rule(world.get_entrance('Skull Woods First Section Bomb Jump', player), lambda state: can_use_bombs(state, player))
