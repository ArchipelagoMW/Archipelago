...
class ALttPLogic(LogicMixin):
    ...
    def lttp_heart_count(self, player: int) -> int:
        diff = self.world.worlds[player].difficulty_requirements
        boss = min(self.item_count('Boss Heart Container', player), diff.boss_heart_container_limit)
        pieces = min(self.item_count('Piece of Heart', player), diff.heart_piece_limit) // 4
        count = boss + pieces + self.item_count('Sanctuary Heart Container', player) + 3
        return count
    ...
    def lttp_can_extend_magic(self, player: int, small_magic: int = 16, full_refill: bool = False) -> bool:
        ...
        if self.lttp_can_buy_unlimited('Green Potion', player) or self.lttp_can_buy_unlimited('Blue Potion', player):
            if self.world.worlds[player].item_functionality == 'hard' and not full_refill:
                base_magic = base_magic + int(base_magic * 0.5 * self.lttp_bottle_count(player))
            elif self.world.worlds[player].item_functionality == 'expert' and not full_refill:
    ...
    def lttp_can_get_good_bee(self, player: int) -> bool:
        cave = self.world.worlds[player].get_region('Good Bee Cave', player)
        return (
    ...
    def lttp_can_retrieve_tablet(self, player: int) -> bool:
        return self.has('Book of Mudora', player) and \
               (self.lttp_has_swords(player, 2) or
                (self.world.worlds[player].swordless and self.has('Hammer', player)))
    ...
    def lttp_can_melt_things(self, player: int) -> bool:
        return self.has('Fire Rod', player) or \
               (self.has('Bombos', player) and
                (self.world.worlds[player].swordless or self.lttp_has_swords(player)))
    ...
    def lttp_can_buy(self, item: str, player: int) -> bool:
        return any(shop.has(item) and shop.region.can_reach(self) for shop in self.world.worlds[player].shops)

    def lttp_can_buy_unlimited(self, item: str, player: int) -> bool:
        return any(shop.has_unlimited(item) and shop.region.can_reach(self) for shop in self.world.worlds[player].shops)
