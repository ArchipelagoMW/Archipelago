...
def set_abilities(multiworld: MultiWorld) -> Dict[str, int]:
...
def set_region_rules(multiworld: MultiWorld, player: int, hexagon_quest_abilities: Dict[str, int]) -> None:
    ...
    if ability_shuffle:
        ...
        multiworld.get_entrance("Ruined Atoll -> Library", player).access_rule = lambda state: state.has_any({grapple, laurels}, player)
        multiworld.get_entrance("Overworld -> Spirit Arena", player).access_rule = lambda state: state.has(gold_hexagon, player, 20) if \
            multiworld.hexagon_quest[player].value else state.has_all({red_hexagon, green_hexagon, blue_hexagon}, player)
...
def set_location_rules(multiworld: MultiWorld, player: int, hexagon_quest_abilities: Dict[str, int]) -> None:
    laurels = "Hero's Laurels"
    grapple = "Magic Orb"
    ...
    set_rule(multiworld.get_location("Far Shore - Secret Chest", player),
             lambda state: state.has(laurels, player) and state.has(prayer, player, prayer_amount) if ability_shuffle
             else state.has(laurels, player))
    ...
    set_rule(multiworld.get_location("Old House - Holy Cross Chest", player),
             lambda state: state.has(house_key, player) and state.has(holy_cross, player, holy_cross_amount)
             if ability_shuffle else state.has(house_key, player))
    ...
    set_rule(multiworld.get_location("East Forest - Ice Rod Grapple Chest", player), lambda state: (
            state.has_all({grapple, ice_dagger, fire_wand}, player) and state.has(ice_rod, player, ice_rod_amount))
            if ability_shuffle else state.has_all({grapple, ice_dagger, fire_wand}, player))
    ...
    set_rule(multiworld.get_location("West Garden - [West Lowlands] Tree Holy Cross Chest", player),
             lambda state: state.has(laurels, player) and state.has(holy_cross, player, holy_cross_amount) if ability_shuffle else state.has(laurels, player))
    set_rule(multiworld.get_location("West Garden - [East Lowlands] Page Behind Ice Dagger House", player),
             lambda state: (state.has(laurels, player) and state.has(prayer, player, prayer_amount)) if ability_shuffle else state.has(laurels, player))
    ...
    set_rule(multiworld.get_location("Fortress Arena - Hexagon Red", player), lambda state: (
            state.has(vault_key, player) and state.has(prayer, player, prayer_amount)) if ability_shuffle else state.has(vault_key, player))
