...
Holy cross (for all holy cross checks), fairies (for the two fairy checks), well (for the coin well checks), shop, bosses (for the bosses with checks associated with them), hero's grave (for the 6 hero grave checks), and ladders (for the ladder items when you have shuffle ladders enabled).
