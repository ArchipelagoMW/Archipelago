...
class PokemonDataEditorFrame(MDBoxLayout):
    ...
    def late_init(self):
        ...
        self.gender_ratio_dropdown = MDDropdownMenu(caller=self.gender_ratio_button, position="bottom", border_margin=8,
                                                    width=self.gender_ratio_button.width)
    ...
    def check_value(self, _field_object: ObjectProperty, _label: TooltipMDLabel, _field: str, _tooltip: str):
        ...
        if not has_error:
            temp_pokemon_data = destringify_pokemon_data(app.sprite_preview.sprite_input.text, temp_pokemon_data_string)
        ...
        if os.path.exists(pokemon_saved_data_path):
            ...
            if pokemon_saved_data_has_error:
                ...
                for field in self.pokemon_saved_data:
                    if field == "dex":
                        pokemon_data[field] = self.pokemon_saved_data[field] = (self.pokemon_saved_data[field] << 7) + \
    ...
    def check_all_fields(self):
        ...
        self.check_value(self.gender_ratio_input, self.gender_ratio_label, "gender_ratio", self.gender_ratio_tooltip)
...
async def main():
    ...
    if not is_command_line:
        ...
        if is_kivy:
            ...
            import_and_run_tkinter()
    else:
        AdjusterRootLayout().adjust(args)


def import_and_run_tkinter():
    from . import adjuster_tkinter
    adjuster_tkinter.adjust_gui()
