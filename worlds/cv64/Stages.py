...
class CV64Stage:
    ...
    def __init__(self, start_region_name: str, startzone_map_offset: typing.Optional[int], startzone_spawn_offset:
                 ...
                 end_map_id: typing.Optional[int], end_spawn_id: typing.Optional[int], boss_count: int,
...
#                                               Previous stage      Next stage            Alternate next stage
vanilla_stage_exits = {"Forest of Silence":    [None,               "Castle Wall",        None],
                       "Castle Wall":          [None,               "Villa",              None],
                       "Villa":                [None,               "Tunnel",             "Underground Waterway"],
                       "Tunnel":               [None,               "Castle Center",      None],
                       "Underground Waterway": [None,               "Castle Center",      None],
                       "Castle Center":        [None,               "Duel Tower",         "Tower of Science"],
                       "Duel Tower":           ["Castle Center",    "Tower of Execution", None],
                       "Tower of Execution":   ["Duel Tower",       "Room of Clocks",     None],
                       "Tower of Science":     ["Castle Center",    "Tower of Sorcery",   None],
                       "Tower of Sorcery":     ["Tower of Science", "Room of Clocks",     None],
                       "Room of Clocks":       [None,               "Clock Tower",        None],
                       "Clock Tower":          [None,               "Castle Keep",        None],
                       "Castle Keep":          [None,               None,                 None]}
...
def shuffle_stages(multiworld, player, active_stage_list, active_stage_exits, stage_1_blacklist):
    ...
    for i in range(total_stages):
        ...
        if multiworld.character_stages[player].value == 0 and i == villa_cc_ids[0]:
            new_stage_order.append(RName.villa)
            villa_cc_ids[1] += 2
        elif multiworld.character_stages[player].value == 0 and i == villa_cc_ids[1]:
        ...
        if multiworld.character_stages[player].value == 0:
            ...
            elif new_stage_order[i - 3] == RName.castle_center or new_stage_order[i - 4] == RName.castle_center:
    ...
    for i in range(len(active_stage_list)):
        # Previous stage
        if vanilla_stage_exits[active_stage_list[i]][0]:
            if i - 1 < 0:
                ...
                elif active_stage_list[i - 1] == alt_cc_stages[1]:
        ...
        # Next stage
        if vanilla_stage_exits[active_stage_list[i]][1]:
            if multiworld.character_stages[player].value == 0:
                if active_stage_list[i + 1] == alt_villa_stage[0]:
        ...
        if vanilla_stage_exits[active_stage_list[i]][2]:
            if multiworld.character_stages[player].value == 0:
