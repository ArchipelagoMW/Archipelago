...
scene_data_modifiers = [
    ...
    0x24, 0x0A, 0x00, 0x01,  # ADDIU T2, R0, 0x0001         <- Werewolf plaque
    ...
    0x15, 0x09, 0x00, 0x08,  # BNE   T0, T1, [forward 0x08]
    0x24, 0x0A, 0x00, 0x01,  # ADDIU T2, R0, 0x0001         <- Fountain FL
    0x24, 0x0B, 0x00, 0x01,  # ADDIU T3, R0, 0x0001         <- Fountain RL
    0x24, 0x0C, 0x00, 0x1F,  # ADDIU T4, R0, 0x0001         <- Dog food gate
    ...
    0x15, 0x09, 0x00, 0x0C,  # BNE   T0, T1, [forward 0x0C]
    0x24, 0x0A, 0x00, 0x01,  # ADDIU T2, R0, 0x0001         <- Storeroom R
    0x24, 0x0B, 0x00, 0x10,  # ADDIU T3, R0, 0x0001         <- Hallway knife
    0x24, 0x0C, 0x00, 0x01,  # ADDIU T4, R0, 0x0001         <- Painting
    0x24, 0x0D, 0x00, 0x01,  # ADDIU T5, R0, 0x0001         <- Dining Room vase
    0x24, 0x0E, 0x00, 0x01,  # ADDIU T6, R0, 0x0001         <- Archives table
    ...
    0x24, 0x0A, 0x00, 0x01,  # ADDIU T2, R0, 0x0001         <- Bucket
    0xA0, 0x4A, 0x02, 0x69,  # SB    T2, 0x0269 (V0)
    0x24, 0x09, 0x00, 0x11,  # ADDIU T1, R0, 0x0011
    0x11, 0x39, 0xFF, 0xCD,  # BEQ   T1, T9, [backward 0x33]
    0x00, 0x00, 0x00, 0x00,  # NOP
    0x03, 0xE0, 0x00, 0x08,  # JR    RA
    ...
    0x24, 0x0A, 0x00, 0x1A,  # ADDIU T2, R0, 0x0001         <- Lizard coffin nearside mid-right
    0x24, 0x0B, 0x00, 0x03,  # ADDIU T3, R0, 0x0001         <- Lizard coffin nearside mid-left
    ...
    0x24, 0x09, 0x00, 0x01,  # ADDIU T1, R0, 0x0001
    0x11, 0x39, 0xFF, 0xA8,  # BEQ   T1, T9, [backward 0x57]
    0x00, 0x00, 0x00, 0x00,  # NOP
    0x03, 0xE0, 0x00, 0x08,  # JR    RA
    ...
    0x24, 0x0A, 0x00, 0x01,  # ADDIU T2, R0, 0x0001         <- Right flame
    0xA0, 0x4A, 0x00, 0x59,  # SB    T2, 0x0059 (V0)
    ...
    0x24, 0x09, 0x00, 0x02,  # ADDIU T1, R0, 0x0002
    0x11, 0x39, 0xFF, 0x9C,  # BEQ   T0, T1, [backward 0x64]
    0x00, 0x00, 0x00, 0x00,  # NOP
    0x03, 0xE0, 0x00, 0x08,  # JR    RA
    # Other level ends TODO: Plz document this!!!
    0x24, 0x09, 0x00, 0x02,
    0x15, 0x09, 0x00, 0x04,
    0x24, 0x09, 0x00, 0x10,
    0x11, 0x39, 0xFF, 0x96,
...
werebull_flag_unsetter = [
    ...
    0x15, 0xCF, 0x00, 0x04,  # BNE   T6, T7, [forward 0x06]
    0x00, 0x00, 0x00, 0x00,  # NOP
...
werebull_flag_unsetter_special2_electric_boogaloo = [
    ...
    0x01, 0x4E, 0x50, 0x25,  # OR    T2, T2, T6
    0xAC, 0x4A, 0x61, 0x3C,  # SW    T2, 0x613C (V0)
    0x17, 0x00, 0x00, 0x03,  # BNEZ  T8, 0x803FC57C
...
item_property_list_extension = [
    # "Invisible" entries in the item property table for every pickup that lacks one
    0x00, 0x01, 0x00, 0x01, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # White jewel
    0x00, 0x01, 0x00, 0x04, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Special1
    0x00, 0x01, 0x00, 0x05, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Special2
    0x00, 0x01, 0x00, 0x0B, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # pot-pourri
    0x00, 0x01, 0x00, 0x0C, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # PowerUp
    0x00, 0x01, 0x00, 0x0E, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Holy water
    0x00, 0x01, 0x00, 0x0F, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Cross
    0x00, 0x01, 0x00, 0x10, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Axe
    0x00, 0x01, 0x00, 0x0D, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Knife
    0x00, 0x01, 0x00, 0x11, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Wooden stake
    0x00, 0x01, 0x00, 0x12, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Rose
    0x00, 0x01, 0x00, 0x13, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # The contract
    0x00, 0x01, 0x00, 0x14, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # engagement ring
    0x00, 0x01, 0x00, 0x15, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Magical Nitro
    0x00, 0x01, 0x00, 0x16, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Mandragora
    0x00, 0x01, 0x00, 0x19, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Incandescent gaze
    0x00, 0x01, 0x00, 0x1A, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Archives key
    0x00, 0x01, 0x00, 0x1B, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Left Tower Key
    0x00, 0x01, 0x00, 0x1C, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Storeroom Key
    0x00, 0x01, 0x00, 0x1D, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Garden Key
    0x00, 0x01, 0x00, 0x1E, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Copper Key
    0x00, 0x01, 0x00, 0x1F, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Chamber Key
    0x00, 0x01, 0x00, 0x20, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Execution Key
    0x00, 0x01, 0x00, 0x21, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Science Key1
    0x00, 0x01, 0x00, 0x22, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Science Key2
    0x00, 0x01, 0x00, 0x23, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Science Key3
    0x00, 0x01, 0x00, 0x24, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Clocktower Key1
    0x00, 0x01, 0x00, 0x25, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00,  # Clocktower Key2
    0x00, 0x01, 0x00, 0x26, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00   # Clocktower Key3
]
...
music_modifier = [
    ...
    0x3C, 0x08, 0x80, 0x40,  # LUI   T0, 0x8040
    0x01, 0x06, 0x48, 0x21,  # ADDU  T1, T0, A2
    0x91, 0x26, 0xCD, 0x20,  # LBU   A2, 0xCD20 (T1)
    0xA4, 0x26, 0x23, 0xA4,  # SH    A2, 0x23A4 (AT)
    0x3C, 0x0A, 0x80, 0x09,  # LUI   T2, 0x8009
    0xA5, 0x46, 0x3D, 0x90,  # SH    A2, 0x3D90 (T2)
    0x03, 0xE0, 0x00, 0x08   # JR    RA
]
...
music_comparer_modifier = [
    ...
    0x08, 0x00, 0x52, 0xD0   # J     0x80014B40
]
