...
def setup_event_rules(multiworld: MultiWorld, player: int):
    ...
    total_revivals = multiworld.worlds[player].total_revivals  # pulling this info we calculated in generate_early
    ...
    if total_revivals or multiworld.start_with_revive[player]:
        add_rule(multiworld.get_location("Victory", player),
                 lambda state: state.has("Dio's Best Friend", player,
                                         total_revivals + multiworld.start_with_revive[player].value))
