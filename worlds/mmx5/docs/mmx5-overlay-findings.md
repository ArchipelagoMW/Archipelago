...
# Mega Man X5 (SLUS-01334) â€” Overlay / RAM-dump analysis: item grants, stage ids, AP hook plan
...
All addresses are PS1 KSEG0 (`0x80xxxxxx`); BizHawk MainRAM offset = addr âˆ’ 0x80000000.
...
| 0x800D1C0C | **Active stage id used by the spawn engine** (1..8 mavericks, 0 intro, 9/10 Dynamo?, 0x0A-0x0C Zero Space, 0x0F = transition) | G=06 (Izzy Glow âœ“), R=0x0F |
| 0x800D1C0D | Active area/section index within stage | G=00 |
| 0x800D1C0F | "boss already beaten" flag for current stage (set from 0x1C4C bit, see Â§2) | |
| 0x800D1C26 | **Stage id of the just-completed sortie** (results input) | R=02 (Dark Dizzy âœ“), G=00 |
| 0x800D1C27 | Sortie result type (0 = maverick defeated â†’ weapon grant path; 1 = special path for stages 9/10; 2 = other) | R=00 |
| 0x800D1C41 | **Selected/current stage id (persistent through the sortie)** | G=06, R=02; matches earlier "02â†’09 entering Dynamo sortie" |
| 0x800D1C44 | Current character (0 = X, 1 = Zero) | copied to overlay global 0x800F5F7F at results |

Stage id â†” weapon bit mapping (verified): bit = stageId âˆ’ 1 in 0x800D1C4C, in
...
## 1. Mission 1 â€” Heart-tank pickup handler (and every other pickup)

**All item pickup handling lives in the STATIC EXE** (0x80010000..0x80092000),
not overlay code â€” it is patchable once at boot and stays patched.
...
| 9 | 0x80054264 | **Sub-tank**: u16 0x800D1C7E \|= 0x1000 << (idâˆ’0x27) â†’ bits 12/13; also sets refill-full flag 0x800D1C76 (X, id 0x27) / 0x800D1C77 (Zero) = 0x80 |
...
### 1.2 Heart handler (kind 0) â€” disassembly, 0x800540A0
...
`bit = 0x10000 << (id âˆ’ 0x10)` (same u32 at 0x1C80) and
...
| 8-15 | DNA-reward applier (Â§3.3), reward id 0-7 | "Life Up" DNA rewards (+2 max HP each) |
| 16-23 | pickup kind 1, id 0x10-0x17 | stage EX energy-up items |
| 24-31 | DNA-reward applier, reward id 8-15 | "Energy Up" DNA rewards (+2 max WE). Old note "0x1C83 bit1" = u32 bit 25 âœ“ |

### 1.4 Where the heart's bit index comes from â€” placement records
...
- `minor` selects the object main function from the per-area table pointed to
  ...
  (entry = 0x800543E4).
- Spawned object gets: +0 = 0x41â†’(ctor overwrites 0x21), +1 = minor,
...
### 1.5 Stage â†’ heart-bit table (partial; data-driven, not in code)
...
| 2 | Dark Dizzy | bit 6 | live capture (prior session); harvest 2026-07-31: **SUB-TANK #2 id=28 @800F3D50 confirmed** (area0; the heart record itself must sit in a later area â€” not in area0's list) |
...
| 7 | Axle the Red | bit 5 | harvest: unique UNGATED heart rec@800FA7C8 (+11 phantom heart recs ids 0-2 gated armorgate=5 â€” dummied placements?) + elimination (5 = only bit left) |
...
(area0) lists were resident during the tour â€” **no EX items (Energy Ups) found
...
The mapping is per-record data, NOT stageIdâˆ’1. **Harvest procedure** (5 min/stage
...
entries â€” heart = id < 0x10, EX = 0x10-0x1F, tanks = 0x27-0x2A. Static intro/
Zero-Space lists are already in the EXE (stage0 @0x80073748, stage10 @0x800739C4,
stage11 @0x80073A8C/0x80073B5C, stage12 @0x80074140 â€” no hearts there, only
...
## 2. Mission 2 â€” Weapon/boss-beaten grant writer (0x800D1C4C)

### 2.1 The grant function â€” 0x800EEC0C (results overlay, resident in R dump)

Reached via the results state machine:
`mainmode(0x800D1C00+0=0x0C)` â†’ sub-mode dispatcher 0x800EFF14 (table
0x800F4FF8, indexed by 0x800D1C01) â†’ sub-state dispatcher 0x800EFFBC (table
**0x800F5024**, indexed by 0x800D1C02) â†’ **index 4 = 0x800EEC0C**, called with
`a0 = 0x800D1C00` (the controller object â€” so `sb 0x4C(s0)` below is 0x800D1C4C).
...
| reader | where | effect |
|---|---|---|
| 0x80056FDC (read at **0x80057008**) | static EXE | boss-spawn gate: if stage 0x800D1C0C in 1..8 and bit (stageâˆ’1) set â†’ set 0x800D1C0F = 1 (boss-already-beaten path; refight/skip handling) â€” **this is the "does granting the weapon mark the stage beaten" answer: yes, same bit** |
| 0x8001D0BC | static EXE | save-file serialize/compare (file+0x4A â†” 0x1C4C), see Â§3.4 |
| stage load | **0x8003C324 / 0x8003D660 / 0x8003D814** (static EXE, three character-init paths) | `lbu 0x4C(ctrl); sb 0xC9(player)` + `lbu 0x4B(ctrl); sb 0x14A(player)` (Ultimate mirror). A char==3 branch zeroes live weapons instead (+ default grants via 0x8003D4C0). CAPTURED 2026-07-31 â€” the capability-decoupling patch site: change offset 0x4Câ†’0x4D (AP weapons byte 0x800D1C4D, unused + memcard-persisted) at all three |
| stage-select checkmarks / Zero Space unlock | stage-select overlay â€” live obs 2026-07-31: stage select shows **NO visual beaten indicator** (suppressed Izzy bit produced zero UI change), so no checkmark reader exists to patch. Zero Space unlock logic still unverified (see Â§6) |
| Enigma/Shuttle **parts screen** | hub overlay â€” **derived from 0x1C4C, no separate parts store** (proven behaviorally 2026-07-31: suppressed Izzy kill â†’ no Laser Device despite "got part" banner; restore_flaser() alone made it appear after a stage round-trip). Ownership is SNAPSHOTTED at hub entry â€” mid-hub RAM edits don't refresh the screen until the hub reloads. AP consequence: with vanilla grants suppressed, parts never accrue; the launch check must be patched to read AP part state |

Confidence: grant writer + semantics **high**; stage-select/endgame reads **gap**.

---

## 3. Mission 3 â€” Other grant writers

### 3.1 Armor capsules â€” static EXE, 0x80055D58 region
...
- id 0-7: **`0x800D1CA1 |= maskTable[id]`** (RMW at 0x80055DB8/0x80055DC8),
  ...
  â†’ ids 0-3 = Falcon parts (bits 0-3), ids 4-7 = Gaea parts (bits 4-7).
- id 8 (Zero-space capsule): if char == 0 (X) â†’ `0x800D1C4B = 1` (Ultimate
...
- Armor completion (results overlay 0x800EFC48): if `(0x1CA1 & 0x0F) == 0x0F`
  and ack-copy 0x800D1CCC differs â†’ `0x800D1C4A |= 2` (Falcon complete);
  the following block does the same for the 0xF0 nibble (Gaea).

### 3.2 Max HP / max weapon energy

Only two +2 writers exist per stat: heart/EX pickup handlers (Â§1.2, static EXE,
...
### 3.3 DNA-reward applier â€” results overlay ~0x800EFB40

Grants the pending DNA reward (buffer 0x800D1D38-3A per old notes) by reward id:

- id 0x00-0x0F: `u32 0x800D1C80 |= rewardTable[id].mask` â€” table at
  **0x800F4ED0** (8 bytes/entry, +4 = mask): ids 0-7 â†’ bits 8-15 (+2 max HP),
  ids 8-15 â†’ bits 24-31 (+2 max WE).
- id 0x10-0x1F: `u32 0x800D1C84 |= rewardTable[id].mask` â€” **0x800D1C84 is the
  DNA-parts-owned u32** (old "parts bitfields 0x1C84/86" solved). Masks are a
  scrambled per-part permutation (id 0x10â†’0x20, 0x11â†’0x20000, 0x12â†’0x80, ...
...
### 3.4 Save-file buffer â†” RAM mapping (memory-card format)

Save-file staging buffer pointer: `*(0x800E81E4)` = **0x80110408** (both dumps).
Loader **0x8001C900** (fileâ†’RAM), storer ~0x8001CC5C (RAMâ†’file, mirror),
verifier ~0x8001CF00 (reads both); results overlay also serializes at
**0x800F2980**. Mapping (file offset â†’ RAM):
...
card through the normal save flow â€” no extra work needed.

---

## 4. Mission 4 â€” Free RAM candidates for the AP mailbox
...
|---|---|---|---|
| 1 (primary) | **0x801F8310-0x801FE84F** â€” recommend mailbox at **0x801FA000** | ~25 KB | Zero in both dumps, zero code refs. Sits between top of used heap (0x801F6000-0x801F8300 buffer, referenced) and the deepest stack byte ever observed (0x801FE850; SP base 0x801FFF00). A mailbox â‰¤0x1000 bytes at 0x801FA000 leaves >18 KB stack headroom. Risk: extreme stack spikes â€” none observed. |
| 2 | **0x801C1660-0x801F2FF0** â€” recommend **0x801E0000** | ~200 KB | Zero in both dumps, zero code refs. Risk: could be a streaming/heap arena used by stages bigger than the two captured; verify with canary before committing. |
...
0x800FDC71+ (overlay data region â€” other stages load bigger data, stage-3 list
...
- **0x80010000-0x8006F000: identical** â€” static EXE code, patch once at boot.
- 0x8006F000-0x80071000, 0x8007D000-0x80082000: EXE data (live variables).
- **0x800EC000-0x800F9000: differs** â€” the swapped overlay window. Gameplay
...
### 6.1 Pickup checks (hearts, EX, tanks, 1-ups, refills) â€” static EXE, best hook

**Hook the jump table at 0x80011068** (data, in the EXE â€” rewrite entries to
point at AP stubs placed in the mailbox region). Each stub:

1. reads `s1+0x82` (kind), `s1+2` (id), `s1+0x10` (placement-record ptr) and
   `0x800D1C41` (stage) â†’ appends a check-record to the AP log (MMX4-style:
...
max-stat `sb` at 0x800540F4 / 0x80054158 â€” but the jump-table redirect is
cleaner and gives the check-record for free.

Item grants FROM the AP client: write the bits/stats directly into
0x800D1C80 (u32), 0x800D1C7E (u16), 0x800D1C45, 0x800D1C47/48, 0x800D1C53/54 â€”
all engine-honored (max HP/WE take effect immediately; tanks appear in menu).

### 6.2 Boss-kill / weapon checks â€” two options

- **No-patch detection (recommended for checks):** poll `0x800D1C00 == 0x0C`
  and `0x800D1C27 == 0` and `0x800D1C26 != 0` â†’ boss of stage 0x800D1C26 was
...
- **Suppressing the vanilla weapon grant (item-randomizer mode):** while the
  ...
  address â€” note it sits in the jal delay slot; replacing with NOP is safe,
  ...
  **PROVEN LIVE 2026-07-31** (`Scripts/mmx5_ap_patch_proto.lua`, Izzy kill):
  ...
  ~5 s before the commit window. Also observed: mode byte 0x0B = gameplayâ†’
  results interstitial, 0x03â†’0x04 = stage-select sequence. Mailbox canaries at
  0x801FA000/0x801E0000 survived the full cycle; **savestate loads wipe them**
  (restored RAM predates arming) â€” client must treat mailbox as
  re-initializable at any time.
  Weapon grants from AP: client ORs bits into 0x800D1C4C (persistent) and
  0x8009A169 (live, if in-stage) â€” exactly what the game itself does.
- Because 0x1C4C doubles as boss-beaten (stage select, boss-skip gate
  ...
  possible** â€” same bit. Decoupling requires patching the two readers
  (0x80056FDC EXE; stage-select overlay TBD) or accepting weapon=boss-beaten
  coupling in logic. Flag for design discussion.

### 6.3 Armor capsules â€” static EXE

Hook/replace the RMW at **0x80055DB8-0x80055DC8** (`lbu/or/sb 0xA1(a1)`), plus
the id-8 branch at 0x80055D78-0x80055D9C (0x1C4B / 0x1C4A|0x10). A `jal` stub
there can log capsule id (s1+2) and optionally skip the grant. Static EXE â€”
boot-time patch.

### 6.4 DNA rewards â€” results overlay
...
- **Gameplay gate for the client**: 0x800D1C00 mode byte (0x0A gameplay /
  0x0C results) â€” much better than the disproven 0x800D4F5x gates.
- Enigma/countdown block: written by save-load at 0x8001CAC4-0x8001CADC â€”
  0x800D1CAA (u8), 0x800D1CAB (u8), **u32 0x800D1CAC** (countdown timer;
  results overlay reads it at 0x800F4844). 0x800D1CAE is the u32's high half â€”
...
- 0x800D1CCB: persisted stage-event flag byte; Izzy Glow overlay polls bit0
  constantly (0x800F4330..0x800F4BA4) â€” likely the stage's light-switch state.
- 0x800D1C88-0x800D1C9F: 6 u32s persisted in the save file (file+0x08) and
  ...
  hypothesis was per-stage injured-Reploid rescue bitfields â€” **DISPROVEN
...
- Zero Space access gating: NOT found â€” neither dump has the stage-select
...
1. **Remaining 6 heart-tank ids** â€” harvest per stage with the Â§1.5 procedure
...
2. **Stage-select overlay** (boss-beaten checkmarks, Zero Space unlock,
   ...
   repeat the Â§2.2 reader scan for 0x1C4C / 0x1D0F / 0x1CAC.
3. ~~**Reploid rescue**: rescue one reploid in Izzy Glow, diff 0x800D1C88-9F
   (and 0x800D1CC4..) before/after~~ â€” DONE 2026-07-31, no save-struct write
   (see Â§7). Remaining: find the LIVE spawn-slot state for rescue detection.
4. **Free-RAM canary validation** across a full session (Â§4).
5. **Dynamo/Zero-space results path** (0x800D1C27==1, stages 9/10, the
   `0x800D1CCA -= 0x14` branch) â€” semantics unknown.
6. Decision needed on weapon/boss-beaten coupling (Â§6.2, third bullet).

## 9. CD streaming / overlay loader (static-EXE + disc analysis 2026-07-31)

### 9.0 HEADLINE: code overlays are NOT compressed â€” patch the disc directly
...
- The code overlays live in **ROCK_X5.BIN** (ISO extent: LBA 23693, size
  ...
  **0x800EE970 â€” exactly the dest pointer in the EXE descriptor at
...
- Disc sectors are Mode2 Form1 (subheader 00 00 08 00).
  **EDC/ECC regeneration is REQUIRED for every modified sector** â€” proven
...
scripts. BizHawk quirk: SaveRAM (memcard) is keyed to the game name â€” clone
...
end at 23432 = SLUS start, confirming DAT â‰  overlay source).
...
STATIC EXE for a fixup hook â€” superseded by Â§9.0 direct disc patching, but
...
- **Streaming state machine** spanning ~0x80014574-0x80015Cxx (EXE):
  TOC-indexed request setup at 0x800148E4 (id*8 into TOC; size â†’
  0x800E5A18, LBA â†’ 0x800E5A24, kick flag 0x800E5A30 = 1) and at
...
1. The **decompressor call** and the per-overlay **destination address
   source** (no code forms 0x800EC000 as a static constant â€” dest comes
...
2. The **completion dispatch** â€” the state-machine slot that runs when a
   ...
   (overlay id â†’ list of {addr, orig word, new word}) before returning to
...
3. Cross-check which TOC id = results overlay (kill a boss while logging
   0x800E5BE9/0x800E5A24 with Lua â€” cheap runtime assist).
...
- **Parts-screen derivation readers** (the Â§2.2 behavioral proof, now in
  code): two sites â€” per display slot s, bit = table[s] from
  **0x800F5194 = [0,4,5,2,7,6,1,3]**, test `(0x800D1C4C >> bit) & 1`,
  ...
  byte (or table-swap) â€” small, local, data-driven.
- **Countdown decoded**: u32 0x800D1CAC counts FRAMES; hour = 0x34BC0
  (216,000 = 60 fps Ã— 3600). Hourly-tick handler at 0x800EFF04-2C
  (countdown âˆ’= hour when flag +0x27 clear, byte +0x02 += 1); an
  hour-RESTORE site at 0x800F01D8-EC (countdown += hour, resets related
  state) â€” candidate Dynamo/event hour-give-back. ("Hours left" display =
...
- **Story-chapter gating found (design-critical)**: function 0x800EEF14
  ...
  0x800D1C4C â€” 2 kills â†’ chapter 2 (sub-mode 0xC event; Enigma), 6 kills â†’
  ...
  weapon-grant NOP would freeze the popcount â†’ story never advances â†’
...
- **Launch flow**: prompt runs in the hub module (dumps `launch` =
  ...
  **0x800D1C1D = 4** (launch-event state byte, new). Outcome/chapter
  ...
  rewound via savestate â€” outcome roll site still unlocated; the two
  ...
  debugger session â€” watchpoints on 0x800D1C1D/0x800D1D0F writes and
  ...
  frame countdown itself â€” check what the resolution fn reads.)

## 11. LAUNCH RESOLUTION FUNCTION â€” fully decoded (DuckStation session 2026-07-31)

Caught live via write-watchpoint on 0x800D1C1D â†’ read-watchpoint on
...
1. **Score** `s0 = 2 Ã— (sum of s8 bytes 0x800D1CC2..0x1CC5) + s8 0x800D1CCA`
   â€” the "unidentified results-screen counters" are LAUNCH-POWER
   ACCUMULATORS; 0x1CCA is the modifier Dynamo-sortie results adjust by
   0x14 (Â§2.1 type-1 path).
2. **Countdown bonus**: adds hours to u32 0x1CAC by score band:
   â‰¥0x51â†’4h, â‰¥0x3Dâ†’3h, â‰¥0x29â†’2h, >0â†’1h, â‰¤0â†’0. (0x34BC0 frames/hour.)
   Also sets 0x800D1CCB = 1 ("launch attempted"), zeroes 0x1CCA.
3. **The roll**: `jal 0x8002DF78` â€” **the game's RNG lives in the STATIC
   EXE at 0x8002DF78** â€” then `roll = (rand >> 2) & 0xF`. Success:
   | score | needs | rate |
   |---|---|---|
   | â‰¤0 | â€” | 0% |
...
0x800FA0D4, disc sector 24319) with `li v1,0` â†’ success â‡” score > 0, RNG
...
suppressed or ignored). Single on-disc copy â‡’ Enigma + shuttle likely share
the fn â€” verify the shuttle path's feeder before shipping.

Community-lore reconciliation (2026-07-31): the known "rand%64 <
4/8/24/48 by parts count" description is the SAME math (x/64 = our
x/4 per 16) seen from outside â€” parts drive the score via the feeder, so
...
shuttle failure" = something dragging score â‰¤ 0 into the always-fail
band. Under AP (client owns the score bytes + li v1,0 roll patch) all
vanilla modifiers become irrelevant, and score â‰¤ 0 doubles as the launch
GATE: no required AP parts â†’ score 0 â†’ guaranteed clean failure through
...
masked 0xCA seen at a re-streamed 0x800FA0C4-phase during the cutscene â€”
