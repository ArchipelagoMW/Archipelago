...
- **Archipelago** 0.6.8 or later, and the `mmx5.apworld` file.
- **BizHawk 2.10.** Use this version — Archipelago's BizHawk connector does not
  support newer releases, and a later BizHawk will fail to connect. The PSX
  core is **NymaShock** (BizHawk's default for PS1).
- A **PS1 BIOS**, placed where BizHawk expects firmware.
...
Both work and produce an identical patched game. If yours matches neither, your
dump is a different format (2048-byte sectors, a `.iso`, or multi-track) — re-dump
as raw 2352-byte mode.
...
3. Double-click it, or open it via the Launcher's **Open Patch**. The first
...
- **Boss checks land a few seconds after the kill**, once the results screen
  commits it. Nothing is wrong if they do not appear instantly.
