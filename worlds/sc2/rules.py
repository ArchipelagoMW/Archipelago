...
from .item.item_names import (
    PROGRESSIVE_PROTOSS_AIR_WEAPON,
    PROGRESSIVE_PROTOSS_AIR_ARMOR,
    PROGRESSIVE_PROTOSS_SHIELDS,
)
from .options import (
    RequiredTactics,
...
class SC2Logic:
    def __init__(self, world: Optional["SC2World"]):
        ...
        self.logic_level: int = (
            world.options.required_tactics.value if world else RequiredTactics.default
        )
        ...
        self.kerrigan_unit_available = (
            (
                True
                if world is None
                else (world.options.kerrigan_presence.value in kerrigan_unit_available)
            )
            and SC2Campaign.HOTS in get_enabled_campaigns(world)
            and SC2Race.ZERG in get_enabled_races(world)
        )
        self.kerrigan_levels_per_mission_completed = (
            0
            if world is None
            else world.options.kerrigan_levels_per_mission_completed.value
        )
        self.kerrigan_levels_per_mission_completed_cap = (
            -1
            if world is None
            else world.options.kerrigan_levels_per_mission_completed_cap.value
        )
        self.kerrigan_total_level_cap = (
            -1 if world is None else world.options.kerrigan_total_level_cap.value
        )
        self.morphling_enabled = (
            False
            if world is None
            else (world.options.enable_morphling.value == EnableMorphling.option_true)
        )
        self.story_tech_granted = (
            False
            if world is None
            else (world.options.grant_story_tech.value == GrantStoryTech.option_true)
        )
        self.story_levels_granted = (
            False
            if world is None
            else (
                world.options.grant_story_levels.value
                != GrantStoryLevels.option_disabled
            )
        )
        ...
        self.spear_of_adun_presence = (
            SpearOfAdunPresence.default
            if world is None
            else world.options.spear_of_adun_presence.value
        )
        self.spear_of_adun_passive_presence = (
            SpearOfAdunPassiveAbilityPresence.default
            if world is None
            else world.options.spear_of_adun_passive_ability_presence.value
        )
        self.enabled_campaigns = get_enabled_campaigns(world)
        self.mission_order = (
            MissionOrder.default if world is None else world.options.mission_order.value
        )
        self.generic_upgrade_missions = (
            0 if world is None else world.options.generic_upgrade_missions.value
        )
        self.all_in_map = (
            AllInMap.option_ground if world is None else world.options.all_in_map.value
        )
        self.war_council_upgrades = (
            True if world is None else not world.options.nerf_unit_baselines.value
        )
        ...
        self.unit_count_functions: Dict[
            Tuple[SC2Race, int], Callable[[CollectionState], bool]
        ] = {}
    ...
    def weapon_armor_upgrade_count(
        self, upgrade_item: str, state: CollectionState
    ) -> int:
        assert upgrade_item in upgrade_bundle_inverted_lookup.keys()
        count: int = 0
        if self.generic_upgrade_missions > 0:
            if (
                not self.is_item_placement(state)
            ) or self.logic_level == RequiredTactics.option_no_logic:
        ...
        count += state.count_from_list(
            upgrade_bundle_inverted_lookup[upgrade_item], self.player
        )
        ...
        if upgrade_item in item_groups.protoss_generic_upgrades and state.has(
            item_names.QUATRO, self.player
        ):
            count += 1
        return count
    ...
    def terran_power_rating(self, state: CollectionState) -> int:
        ...
        power_score += sum(
            (
                rating
                for item, rating in terran_passive_ratings.items()
                if state.has(item, self.player)
            )
        )
        # Spear of Adun
        ...
        if (
            ...
            power_score += sum(
                (
                    rating
                    for item, rating in soa_passive_ratings.items()
                    if state.has(item, self.player)
                )
            )
        return power_score
    ...
    def terran_army_weapon_armor_upgrade_min_level(self, state: CollectionState) -> int:
        ...
        if self.has_barracks_unit:
            count = min(
                count,
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_INFANTRY_WEAPON, state
                ),
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_INFANTRY_ARMOR, state
                ),
            )
        if self.has_factory_unit:
            count = min(
                count,
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_VEHICLE_WEAPON, state
                ),
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_VEHICLE_ARMOR, state
                ),
            )
        if self.has_starport_unit:
            count = min(
                count,
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_SHIP_WEAPON, state
                ),
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_SHIP_ARMOR, state
                ),
            )
        return count

    def terran_very_hard_mission_weapon_armor_level(
        self, state: CollectionState
    ) -> bool:
        return (
            self.terran_army_weapon_armor_upgrade_min_level(state)
    ...
    def terran_air(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or state.has_all(
                (item_names.LIBERATOR, item_names.LIBERATOR_RAID_ARTILLERY), self.player
            )
            or (
                self.advanced_tactics
                and (
                    (
                        state.has_any(
                            {item_names.HERCULES, item_names.MEDIVAC}, self.player
                        )
                        and self.terran_common_unit(state)
                    )
                    or (
                        state.has_all(
                            (item_names.RAVEN, item_names.RAVEN_HUNTER_SEEKER_WEAPON),
                            self.player,
                        )
                    )
                )
    ...
    def terran_air_anti_air(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or state.has_all(
                {item_names.WRAITH, item_names.WRAITH_ADVANCED_LASER_TECHNOLOGY},
                self.player,
            )
            or state.has_all(
                {item_names.BATTLECRUISER, item_names.BATTLECRUISER_ATX_LASER_BATTERY},
                self.player,
            )
            or (
                self.advanced_tactics
                and state.has_any(
                    {item_names.WRAITH, item_names.VALKYRIE, item_names.BATTLECRUISER},
                    self.player,
                )
                and self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_SHIP_WEAPON, state
                )
                >= 2
            )
    ...
    def terran_competent_ground_to_air(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or (
                state.has_any(
                    {item_names.MARINE, item_names.DOMINION_TROOPER}, self.player
                )
            ...
            and (
                ...
                or state.has_all(
                    (item_names.THOR, item_names.THOR_PROGRESSIVE_HIGH_IMPACT_PAYLOAD),
                    self.player,
                )
            )
        )
    ...
    def terran_competent_anti_air(self, state: CollectionState) -> bool:
        ...
        Good AA unit
        """
        return self.terran_competent_ground_to_air(state) or self.terran_air_anti_air(
            state
        )
    ...
    def terran_any_anti_air(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has_all(
                (item_names.REAPER, item_names.REAPER_JET_PACK_OVERDRIVE), self.player
            )
            or state.has_all(
                (
            ...
            or (
                ...
                and state.count(
                    item_names.SIEGE_TANK_PROGRESSIVE_TRANSPORT_HOOK, self.player
                )
                >= 2
            )
        )
    ...
    def terran_any_anti_air_or_science_vessels(self, state: CollectionState) -> bool:
        return self.terran_any_anti_air(state) or state.has(
            item_names.SCIENCE_VESSEL, self.player
        )
    ...
    def terran_moderate_anti_air(self, state: CollectionState) -> bool:
        return self.terran_competent_anti_air(state) or (
            ...
            or (
                self.advanced_tactics
                and state.has_any(
                    (item_names.GHOST, item_names.SPECTRE, item_names.LIBERATOR),
                    self.player,
                )
            )
        )
    ...
    def terran_defense_rating(
        self, state: CollectionState, zerg_enemy: bool, air_enemy: bool = True
    ) -> int:
        """
        ...
        defense_score = sum(
            (
                tvx_defense_ratings[item]
                for item in tvx_defense_ratings
                if state.has(item, self.player)
            )
        )
        # Manned Bunker
        if state.has_any(
            ...
            zerg_enemy
            and state.has(item_names.FIREBAT, self.player)
            and state.has(item_names.BUNKER, self.player)
        ):
            defense_score += 2
        # Siege Tank upgrades
        if state.has_all(
            {item_names.SIEGE_TANK, item_names.SIEGE_TANK_MAELSTROM_ROUNDS}, self.player
        ):
            defense_score += 2
        if state.has_all(
            {item_names.SIEGE_TANK, item_names.SIEGE_TANK_GRADUATING_RANGE}, self.player
        ):
            defense_score += 1
        # Widow Mine upgrade
        if state.has_all(
            {item_names.WIDOW_MINE, item_names.WIDOW_MINE_CONCEALMENT}, self.player
        ):
            defense_score += 1
        # Viking with splash
        if state.has_all(
            {item_names.VIKING, item_names.VIKING_SHREDDER_ROUNDS}, self.player
        ):
            defense_score += 2
        ...
        if air_enemy:
            ...
            defense_score += min(
                sum(
                    (
                        tvx_air_defense_ratings[item]
                        for item in tvx_air_defense_ratings
                        if state.has(item, self.player)
                    )
                ),
                2,
    ...
    def terran_competent_comp(self, state: CollectionState) -> bool:
        ...
        infantry_weapons = self.weapon_armor_upgrade_count(
            item_names.PROGRESSIVE_TERRAN_INFANTRY_WEAPON, state
        )
        infantry_armor = self.weapon_armor_upgrade_count(
            item_names.PROGRESSIVE_TERRAN_INFANTRY_ARMOR, state
        )
        ...
        if (
            infantry_weapons >= 2
            and infantry_armor >= 1
            and infantry
            and self.terran_bio_heal(state)
        ):
            return True
        # Mass Air-To-Ground
        ship_weapons = self.weapon_armor_upgrade_count(
            item_names.PROGRESSIVE_TERRAN_SHIP_WEAPON, state
        )
        ship_armor = self.weapon_armor_upgrade_count(
            item_names.PROGRESSIVE_TERRAN_SHIP_ARMOR, state
        )
        if ship_weapons >= 1 and ship_armor >= 1:
            air = (
                state.has_any(
                    {item_names.BANSHEE, item_names.BATTLECRUISER}, self.player
                )
                or state.has_all(
                    {item_names.LIBERATOR, item_names.LIBERATOR_RAID_ARTILLERY},
                    self.player,
                )
                or state.has_all(
                    {item_names.WRAITH, item_names.WRAITH_ADVANCED_LASER_TECHNOLOGY},
            ...
            if air and self.terran_mineral_dump(state):
                return True
        # Strong Mech
        vehicle_weapons = self.weapon_armor_upgrade_count(
            item_names.PROGRESSIVE_TERRAN_VEHICLE_WEAPON, state
        )
        vehicle_armor = self.weapon_armor_upgrade_count(
            item_names.PROGRESSIVE_TERRAN_VEHICLE_ARMOR, state
        )
        if vehicle_weapons >= 1 and vehicle_armor >= 1:
            strong_vehicle = state.has_any(
                {item_names.THOR, item_names.SIEGE_TANK}, self.player
            )
            light_frontline = state.has_any(
                ...
                {item_names.REAPER, item_names.REAPER_RESOURCE_EFFICIENCY}, self.player
            )
            if strong_vehicle and light_frontline:
                return True
            # Mech with Healing
            vehicle = state.has_any(
                {item_names.GOLIATH, item_names.WARHOUND}, self.player
            )
            micro_gas_vehicle = self.advanced_tactics and state.has_any(
                {item_names.DIAMONDBACK, item_names.CYCLONE}, self.player
            )
            if self.terran_sustainable_mech_heal(state) and (
                vehicle or (micro_gas_vehicle and light_frontline)
            ):
                return True
        return False
    ...
    def terran_mineral_dump(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or (
                self.advanced_tactics
                and state.has_any(
                    {item_names.PERDITION_TURRET, item_names.DEVASTATOR_TURRET},
                    self.player,
                )
            )
        )
    ...
    def marine_medic_upgrade(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or (
                state.count(item_names.MARINE_PROGRESSIVE_STIMPACK, self.player) >= 2
    ...
    def terran_bio_heal(self, state: CollectionState) -> bool:
        ...
        """
        return state.has_any(
            {item_names.MEDIC, item_names.MEDIVAC, item_names.FIELD_RESPONSE_THETA},
            self.player,
        ) or (
            self.advanced_tactics
            and state.has_all(
    ...
    def terran_base_trasher(self, state: CollectionState) -> bool:
        ...
        return (
            state.has_all(
                (item_names.SIEGE_TANK, item_names.SIEGE_TANK_JUMP_JETS), self.player
            )
            or state.has_all(
                {item_names.BATTLECRUISER, item_names.BATTLECRUISER_ATX_LASER_BATTERY},
                self.player,
            ...
            or (
                ...
                and (
                    state.has_all(
                        {item_names.VIKING, item_names.VIKING_SHREDDER_ROUNDS},
                        self.player,
                    )
                    or state.has_all(
                        {
    ...
    def can_nuke(self, state: CollectionState) -> bool:
        ...
        return self.advanced_tactics and (
            ...
            or state.has_all(
                {item_names.THOR, item_names.THOR_BUTTON_WITH_A_SKULL_ON_IT},
                self.player,
            )
        )
    ...
    def terran_sustainable_mech_heal(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or (
                state.has_any(
                    {item_names.MEDIC, item_names.FIELD_RESPONSE_THETA}, self.player
                )
                and state.has(item_names.MEDIC_ADAPTIVE_MEDPACKS, self.player)
            )
            or state.count(item_names.PROGRESSIVE_REGENERATIVE_BIO_STEEL, self.player)
            >= 3
            or (
                self.advanced_tactics
                and (
                    ...
                    or state.count(
                        item_names.PROGRESSIVE_REGENERATIVE_BIO_STEEL, self.player
                    )
                    >= 2
                )
            )
    ...
    def terran_cliffjumper(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has_all(
                {item_names.GOLIATH, item_names.GOLIATH_JUMP_JETS}, self.player
            )
            or state.has_all(
                {item_names.SIEGE_TANK, item_names.SIEGE_TANK_JUMP_JETS}, self.player
            )
    ...
    def nova_splash(self, state: CollectionState) -> bool:
        return state.has_any(
            {item_names.NOVA_HELLFIRE_SHOTGUN, item_names.NOVA_PULSE_GRENADES},
            self.player,
        ) or (
            self.advanced_tactics
            and state.has_any(
                {item_names.NOVA_PLASMA_RIFLE, item_names.NOVA_MONOMOLECULAR_BLADE},
                self.player,
            )
        )
    ...
    def nova_dash(self, state: CollectionState) -> bool:
        return state.has_any(
            {item_names.NOVA_MONOMOLECULAR_BLADE, item_names.NOVA_BLINK}, self.player
        )

    def nova_full_stealth(self, state: CollectionState) -> bool:
        return (
            state.count(item_names.NOVA_PROGRESSIVE_STEALTH_SUIT_MODULE, self.player)
            >= 2
        )

    def nova_heal(self, state: CollectionState) -> bool:
        return state.has_any(
            {item_names.NOVA_ARMORED_SUIT_MODULE, item_names.NOVA_STIM_INFUSION},
            self.player,
        )
    ...
    def zerg_power_rating(self, state: CollectionState) -> int:
        ...
        power_score += sum(
            (
                rating
                for item, rating in zerg_passive_ratings.items()
                if state.has(item, self.player)
            )
        )
        # Spear of Adun
    ...
    def zerg_defense_rating(
        ...
        defense_score = sum(
            (
                zvx_defense_ratings[item]
                for item in zvx_defense_ratings
                if state.has(item, self.player)
            )
        )
        # Twin Drones
        ...
        if self.morph_lurker(state):
            ...
            if state.has(item_names.LURKER_SEISMIC_SPINES, self.player):
                defense_score += 2
            if (
                state.has(item_names.LURKER_ADAPTED_SPINES, self.player)
                and not zerg_enemy
            ):
                defense_score += 1
            if zerg_enemy:
        ...
        # Corpser Roach
        if state.has_all(
            {item_names.ROACH, item_names.ROACH_CORPSER_STRAIN}, self.player
        ):
            defense_score += 1
            if zerg_enemy:
                defense_score += 1
        ...
        if state.has_all(
            (item_names.BILE_LAUNCHER, item_names.BILE_LAUNCHER_RAPID_BOMBARMENT),
            self.player,
        ):
            defense_score += 2
        ...
        if air_enemy:
            ...
            defense_score += min(
                sum(
                    (
                        zvx_air_defense_ratings[item]
                        for item in zvx_air_defense_ratings
                        if state.has(item, self.player)
                    )
                ),
                2,
    ...
    def zerg_melee_weapon_armor_upgrade_min_level(self, state: CollectionState) -> int:
        return min(
            self.weapon_armor_upgrade_count(
                item_names.PROGRESSIVE_ZERG_MELEE_ATTACK, state
            ),
            self.weapon_armor_upgrade_count(
                item_names.PROGRESSIVE_ZERG_GROUND_CARAPACE, state
            ),
        )
    ...
    def zerg_ranged_weapon_armor_upgrade_min_level(self, state: CollectionState) -> int:
        return min(
            self.weapon_armor_upgrade_count(
                item_names.PROGRESSIVE_ZERG_MISSILE_ATTACK, state
            ),
            self.weapon_armor_upgrade_count(
                item_names.PROGRESSIVE_ZERG_GROUND_CARAPACE, state
            ),
    ...
    def zerg_flyer_weapon_armor_upgrade_min_level(self, state: CollectionState) -> int:
        return min(
            self.weapon_armor_upgrade_count(
                item_names.PROGRESSIVE_ZERG_FLYER_ATTACK, state
            ),
            self.weapon_armor_upgrade_count(
                item_names.PROGRESSIVE_ZERG_FLYER_CARAPACE, state
            ),
        )
    ...
    def zerg_moderate_anti_air(self, state: CollectionState) -> bool:
        return (
            ...
            or (
                ...
                or state.has_all(
                    {item_names.SWARM_HOST, item_names.SWARM_HOST_PRESSURIZED_GLANDS},
                    self.player,
                )
                or (
                    self.spread_creep(state, True)
                    and state.has(item_names.INFESTED_BUNKER, self.player)
                )
            )
            or (
                self.advanced_tactics
                and state.has(item_names.INFESTED_MARINE, self.player)
            )
        )
    ...
    def zerg_basic_anti_air(self, state: CollectionState) -> bool:
        return (
            self.zerg_basic_kerriganless_anti_air(state) or self.kerrigan_unit_available
        )
    ...
    def zerg_basic_kerriganless_anti_air(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has_any(
                (item_names.HUNTER_KILLERS, item_names.THORNSHELLS), self.player
            )
            or (
                self.advanced_tactics
                and state.has_any(
    ...
    def zerg_basic_air_to_ground(self, state: CollectionState) -> bool:
        return (
            state.has_any(
                {item_names.MUTALISK, item_names.INFESTED_BANSHEE}, self.player
            )
    ...
    def zerg_versatile_air(self, state: CollectionState) -> bool:
        return self.zerg_basic_air_to_air(state) and self.zerg_basic_air_to_ground(
            state
        )
    ...
    def zerg_infested_tank_with_ammo(self, state: CollectionState) -> bool:
        return state.has(item_names.INFESTED_SIEGE_TANK, self.player) and (
            state.has_all(
                {item_names.INFESTOR, item_names.INFESTOR_INFESTED_TERRAN}, self.player
            )
    ...
    def morph_brood_lord(self, state: CollectionState) -> bool:
        return (
            state.has_any({item_names.MUTALISK, item_names.CORRUPTOR}, self.player)
    ...
    def zerg_competent_comp(self, state: CollectionState) -> bool:
        ...
        return vespene_unit and state.has_any(
            {item_names.ZERGLING, item_names.SWARM_QUEEN}, self.player
        )
    ...
    def zerg_mineral_dump(self, state: CollectionState) -> bool:
        return (
            state.has_any(
                {item_names.ZERGLING, item_names.PYGALISK, item_names.INFESTED_BUNKER},
                self.player,
            )
            or state.has_all(
                {item_names.SWARM_QUEEN, item_names.SWARM_QUEEN_RESOURCE_EFFICIENCY},
                self.player,
            )
            or (
                self.advanced_tactics
    ...
    def zerg_base_buster(self, state: CollectionState) -> bool:
        ...
        return (
            (
                self.zerg_melee_weapon_armor_upgrade_min_level(state)
                >= self.get_very_hard_required_upgrade_level()
                and (
                    self.morph_tyrannozor(state)
                    or (
                    ...
                    or (
                        ...
                        and state.has(item_names.BANELING_SPLITTER_STRAIN, self.player)
                    )
                )
                and state.has(
                    item_names.SWARM_QUEEN, self.player
                )  # Healing to sustain the frontline
            )
            or (
                self.zerg_ranged_weapon_armor_upgrade_min_level(state)
                >= self.get_very_hard_required_upgrade_level()
                and (
                    ...
                    or self.morph_igniter(state)
                    and state.has(
                        item_names.PRIMAL_IGNITER_PRIMAL_TENACITY, self.player
                    )
                    or state.has_all(
                        (item_names.INFESTOR, item_names.INFESTOR_INFESTED_TERRAN),
                        self.player,
                    )
            ...
            or (
                self.zerg_flyer_weapon_armor_upgrade_min_level(state)
                >= self.get_very_hard_required_upgrade_level()
                and (
    ...
    def kerrigan_levels(
        self, state: CollectionState, target: int, story_levels_available=True
    ) -> bool:
        if (
            story_levels_available and self.story_levels_granted
        ...
        levels = self.kerrigan_levels_per_mission_completed * state.count_group(
            "Missions", self.player
        )
    ...
    def protoss_power_rating(self, state: CollectionState) -> int:
        ...
        power_score += sum(
            (
                rating
                for item, rating in protoss_passive_ratings.items()
                if state.has(item, self.player)
            )
        )
        # Spear of Adun
        if self.spear_of_adun_presence in (
            SpearOfAdunPresence.option_everywhere,
            SpearOfAdunPresence.option_protoss,
        ):
            power_score += self.soa_power_rating(state)
        if self.spear_of_adun_passive_presence in (
            SpearOfAdunPassiveAbilityPresence.option_everywhere,
            SpearOfAdunPresence.option_protoss,
        ):
            power_score += sum(
    ...
    def protoss_army_weapon_armor_upgrade_min_level(
        self, state: CollectionState
    ) -> int:
        ...
        if self.has_protoss_ground_unit:
            count = min(
                count,
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_PROTOSS_GROUND_WEAPON, state
                ),
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_PROTOSS_GROUND_ARMOR, state
                ),
            )
        if self.has_protoss_air_unit:
            count = min(
                count,
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_PROTOSS_AIR_WEAPON, state
                ),
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_PROTOSS_AIR_ARMOR, state
                ),
            )
        if self.has_protoss_ground_unit or self.has_protoss_air_unit:
            count = min(
                count,
                self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_PROTOSS_SHIELDS, state
                ),
            )
        return count

    def protoss_very_hard_mission_weapon_armor_level(
        self, state: CollectionState
    ) -> bool:
        return (
            self.protoss_army_weapon_armor_upgrade_min_level(state)
            >= self.get_very_hard_required_upgrade_level()
        )
    ...
    def protoss_defense_rating(self, state: CollectionState, zerg_enemy: bool) -> int:
        ...
        defense_score = sum(
            (
                pvx_defense_ratings[item]
                for item in pvx_defense_ratings
                if state.has(item, self.player)
            )
        )
        # Vanguard + rapid fire
        if state.has_all(
            (item_names.VANGUARD, item_names.VANGUARD_RAPIDFIRE_CANNON), self.player
        ):
            defense_score += 1
        # Fire Colossus
        if state.has_all(
            (item_names.COLOSSUS, item_names.COLOSSUS_FIRE_LANCE), self.player
        ):
            defense_score += 2
            if zerg_enemy:
                defense_score += 2
        if state.has(item_names.SHIELD_BATTERY, self.player) and state.has_any(
            (
        ...
        if zerg_enemy:
            defense_score += sum(
                (
                    pvz_defense_ratings[item]
                    for item in pvz_defense_ratings
                    if state.has(item, self.player)
                )
            )
        # Advanced Tactics bumps defense rating requirements down by 2
    ...
    def protoss_any_gap_transport(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or state.has_all(
                (item_names.MISTWING, item_names.MISTWING_PILOT), self.player
            )
            or (
                ...
                and (
                    state.has_any(
                        ...
                        + [item_names.ELDER_PROBES, item_names.PROBE_WARPIN],
                        self.player,
                    )
                    or (
                        state.has(item_names.WARP_HARMONIZATION, self.player)
    ...
    def protoss_any_anti_air_unit_or_soa_any_protoss(
        self, state: CollectionState
    ) -> bool:
    ...
    def protoss_any_anti_air_unit_or_soa(self, state: CollectionState) -> bool:
        return self.protoss_any_anti_air_unit(state) or self.protoss_any_anti_air_soa(
            state
        )
    ...
    def protoss_any_anti_air_soa(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has(
                item_names.SOA_PROGRESSIVE_PROXY_PYLON, self.player, 2
    ...
    def protoss_any_anti_air_unit(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has_all(
                (item_names.SUPPLICANT, item_names.SUPPLICANT_ZENITH_PITCH), self.player
            )
            or state.has_all(
                (item_names.WARP_PRISM, item_names.WARP_PRISM_PHASE_BLASTER),
                self.player,
            )
            or state.has_all(
                (item_names.WRATHWALKER, item_names.WRATHWALKER_AERIAL_TRACKING),
                self.player,
            )
            or state.has_all(
                (item_names.DISRUPTOR, item_names.DISRUPTOR_PERFECTED_POWER),
                self.player,
            )
            or state.has_all(
                (
            ...
            or state.has_all(
                (item_names.SKIRMISHER, item_names.SKIRMISHER_PEER_CONTEMPT),
                self.player,
            )
            or state.has_all(
                (item_names.TRIREME, item_names.TRIREME_SOLAR_BEAM), self.player
            )
            or (
    ...
    def protoss_anti_armor_anti_air(self, state: CollectionState) -> bool:
        return (
            self.protoss_competent_anti_air(state)
            or state.has_any(
                (item_names.SCOUT, item_names.MISTWING, item_names.DRAGOON), self.player
            )
            or (
                state.has_any(
                    {item_names.IMMORTAL, item_names.ANNIHILATOR}, self.player
                )
                and state.has(
                    item_names.IMMORTAL_ANNIHILATOR_ADVANCED_TARGETING_MECHANICS,
                    self.player,
    ...
    def protoss_common_unit_anti_light_air(self, state: CollectionState) -> bool:
        return self.protoss_common_unit(state) and self.protoss_anti_light_anti_air(
            state
        )

    def protoss_common_unit_anti_armor_air(self, state: CollectionState) -> bool:
        return self.protoss_common_unit(state) and self.protoss_anti_armor_anti_air(
            state
        )
    ...
    def protoss_competent_anti_air(self, state: CollectionState) -> bool:
        return (
            ...
            or (
                ...
                and (
                    state.has_any(
                        (item_names.SCOUT, item_names.MISTWING, item_names.DRAGOON),
                        self.player,
                    )
                    or state.has_all(
                        {
    ...
    def protoss_has_blink(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has_all(
                {item_names.SLAYER, item_names.SLAYER_PHASE_BLINK}, self.player
            )
            or (
                state.has(
                    item_names.DARK_TEMPLAR_AVENGER_BLOOD_HUNTER_BLINK, self.player
                )
                and state.has_any(
                    {
    ...
    def protoss_fleet(self, state: CollectionState) -> bool:
        return (
            ...
            and self.weapon_armor_upgrade_count(PROGRESSIVE_PROTOSS_AIR_WEAPON, state)
            >= 2
            and self.weapon_armor_upgrade_count(PROGRESSIVE_PROTOSS_AIR_ARMOR, state)
            >= 2
    ...
    def protoss_hybrid_counter(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or state.has_all(
                (item_names.VANGUARD, item_names.VANGUARD_FUSION_MORTARS), self.player
            )
            or (
            ...
            or (
                self.advanced_tactics
                and state.has_all(
                    (item_names.OPPRESSOR, item_names.OPPRESSOR_VULCAN_BLASTER),
                    self.player,
                )
            )
        )
    ...
    def protoss_basic_splash(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has_all(
                (item_names.ZEALOT, item_names.ZEALOT_WHIRLWIND), self.player
            )
            or (
                state.has_all(
                    (
    ...
    def protoss_static_defense(self, state: CollectionState) -> bool:
        return state.has_any(
            {item_names.PHOTON_CANNON, item_names.KHAYDARIN_MONOLITH}, self.player
        )
    ...
    def protoss_can_merge_archon(self, state: CollectionState) -> bool:
        return (
            ...
            or state.has_all(
                {item_names.ASCENDANT, item_names.ASCENDANT_ARCHON_MERGE}, self.player
            )
            or state.has_all(
                {item_names.DARK_TEMPLAR, item_names.DARK_TEMPLAR_ARCHON_MERGE},
                self.player,
            )
        )
    ...
    def protoss_can_merge_dark_archon(self, state: CollectionState) -> bool:
        return state.has(item_names.DARK_ARCHON, self.player) or state.has_all(
            {item_names.DARK_TEMPLAR, item_names.DARK_TEMPLAR_DARK_ARCHON_MELD},
            self.player,
        )
    ...
    def protoss_competent_comp(self, state: CollectionState) -> bool:
        ...
        support_unit: bool = (
            ...
            or state.has_all(
                (item_names.MIRAGE, item_names.MIRAGE_GRAVITON_BEAM), self.player
            )
            or state.has_all(
                (
                    item_names.DARK_TEMPLAR,
    ...
    def protoss_mineral_dump(self, state: CollectionState) -> bool:
        return (
            state.has_any(
                (item_names.ZEALOT, item_names.SENTINEL, item_names.PHOTON_CANNON),
                self.player,
            )
            or state.has_all(
                (item_names.CENTURION, item_names.CENTURION_RESOURCE_EFFICIENCY),
                self.player,
            )
            or self.advanced_tactics
            and state.has_any(
                (item_names.SUPPLICANT, item_names.SHIELD_BATTERY), self.player
            )
        )
    ...
    def ghost_in_a_chance_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            or (
                ...
                and state.has_any(
                    {item_names.NOVA_DOMINATION, item_names.NOVA_C20A_CANISTER_RIFLE},
                    self.player,
                )
    ...
    def terran_outbreak_requirement(self, state: CollectionState) -> bool:
        ...
        return self.terran_defense_rating(state, True, False) >= 4 and (
            self.terran_common_unit(state) or state.has(item_names.REAPER, self.player)
        )
        ...
        return (
            ...
            and (
                ...
                or (
                    ...
                    and (
                        state.has_any(
                            (item_names.VOID_RAY, item_names.DESTROYER), self.player
                        )
                    )
                )
            )
    ...
    def terran_havens_fall_requirement(self, state: CollectionState) -> bool:
        ...
        return self.terran_common_unit(state) and (
            ...
            or (
                ...
                and (
                    state.has_any(
                        (item_names.VIKING, item_names.BATTLECRUISER), self.player
                    )
                    or state.has_all(
                        (
                            item_names.WRAITH,
    ...
    def terran_respond_to_colony_infestations(self, state: CollectionState) -> bool:
        ...
        return self.terran_havens_fall_requirement(state) and (
            ...
            or (
                state.has_any(
                    {item_names.BATTLECRUISER, item_names.VALKYRIE}, self.player
                )
                and self.weapon_armor_upgrade_count(
                    item_names.PROGRESSIVE_TERRAN_SHIP_WEAPON, state
                )
    ...
    def zerg_havens_fall_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            and (
                ...
                or self.zerg_competent_comp(state)
            )
        )
    ...
    def zerg_respond_to_colony_infestations(self, state: CollectionState) -> bool:
        ...
        return self.zerg_havens_fall_requirement(state) and (
            ...
            and (
                ...
                or state.has_any(
                    {item_names.BROOD_QUEEN, item_names.SCOURGE}, self.player
                )
            )
        )
    ...
    def protoss_havens_fall_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            and (
                ...
                or (
                    state.has_any(
                        (item_names.TEMPEST, item_names.SKYLORD, item_names.DESTROYER),
                        self.player,
                    )
                    or (
                        self.weapon_armor_upgrade_count(
    ...
    def terran_gates_of_hell_requirement(self, state: CollectionState) -> bool:
        ...
        return self.terran_competent_comp(state) and (
            self.terran_defense_rating(state, True) > 6
        )

    def zerg_gates_of_hell_requirement(self, state: CollectionState) -> bool:
        """Gates of Hell mission requirement"""
        return self.zerg_competent_comp_competent_aa(state) and (
            self.zerg_defense_rating(state, True) > 8
        )

    def protoss_gates_of_hell_requirement(self, state: CollectionState) -> bool:
        """Gates of Hell mission requirement"""
        return self.protoss_competent_comp(state) and (
            self.protoss_defense_rating(state, True) > 6
        )
    ...
    def terran_welcome_to_the_jungle_requirement(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            self.advanced_tactics
            and state.has_any(
                {item_names.MARINE, item_names.DOMINION_TROOPER, item_names.VULTURE},
                self.player,
            )
            and self.terran_air_anti_air(state)
        )
        ...
        return (
            ...
            and (
                ...
                or (
                    self.morph_devourer(state)
                    and state.has_any(
                        {item_names.HYDRALISK, item_names.SWARM_QUEEN}, self.player
                    )
                )
                or (
                    self.morph_viper(state)
                    and state.has(item_names.VIPER_PARASITIC_BOMB, self.player)
                )
            )
    ...
    def terran_can_grab_ghosts_in_the_fog_east_rock_formation(
        self, state: CollectionState
    ) -> bool:
        """
        ...
        return (
            ...
            or state.has_all(
                {item_names.RAVEN, item_names.RAVEN_HUNTER_SEEKER_WEAPON}, self.player
            )
            or (
                state.has_any(
                    {item_names.LIBERATOR, item_names.EMPERORS_GUARDIAN}, self.player
                )
    ...
    def terran_great_train_robbery_train_stopper(self, state: CollectionState) -> bool:
        ...
        return state.has_any(
            ...
            and (
                state.has_all(
                    {item_names.REAPER, item_names.REAPER_G4_CLUSTERBOMB}, self.player
                )
                or state.has_all(
                    {item_names.SPECTRE, item_names.SPECTRE_PSIONIC_LASH}, self.player
                )
                or state.has_any(
                    {item_names.VULTURE, item_names.LIBERATOR}, self.player
                )
            )
        )
    ...
    def zerg_great_train_robbery_train_stopper(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or state.has_all(
                {item_names.MUTALISK, item_names.MUTALISK_SUNDERING_GLAIVE}, self.player
            )
            or state.has_all(
                (item_names.HYDRALISK, item_names.HYDRALISK_MUSCULAR_AUGMENTS),
                self.player,
            )
            or (
    ...
    def protoss_great_train_robbery_train_stopper(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            or state.has_all(
                (item_names.REAVER, item_names.REAVER_KHALAI_REPLICATORS), self.player
            )
            or state.has_all(
                {item_names.VANGUARD, item_names.VANGUARD_FUSION_MORTARS}, self.player
            )
            ...
            or state.has_all(
                (item_names.ORACLE, item_names.ORACLE_TEMPORAL_ACCELERATION_BEAM),
                self.player,
            )
            or (
                self.advanced_tactics
                and (
                    ...
                    or state.has_all(
                        (item_names.ADEPT, item_names.ADEPT_RESONATING_GLAIVES),
                        self.player,
                    )
                    or state.has_all(
                        {item_names.VANGUARD, item_names.VANGUARD_RAPIDFIRE_CANNON},
                        self.player,
                    )
                    or state.has_all(
                        (
                    ...
                    or (
                        ...
                        and (
                            state.has_any(
                                (item_names.DARK_TEMPLAR, item_names.AVENGER),
                                self.player,
                            )
                            or state.has_all(
                                (
    ...
    def protoss_supernova_requirement(self, state: CollectionState):
        return (
            (
                ...
                or (
                    self.advanced_tactics
                    and state.has(item_names.PROGRESSIVE_WARP_RELOCATE, self.player)
                )
            )
            and self.protoss_competent_anti_air(state)
            and (
    ...
    def terran_maw_requirement(self, state: CollectionState) -> bool:
        ...
        return (
            ...
            and (
                ...
                or (
                    state.has(item_names.MARAUDER, self.player)
                    and self.terran_bio_heal(state)
                )
            )
            and (
                ...
                state.has_any(
                    (item_names.GOLIATH, item_names.CYCLONE, item_names.VIKING),
                    self.player,
                )
                or (
                    state.has_any(
    ...
    def zerg_maw_requirement(self, state: CollectionState) -> bool:
        ...
        usable_muta = (
            state.has_all(
                (item_names.MUTALISK, item_names.MUTALISK_RAPID_REGENERATION),
                self.player,
            )
            and state.has_any(
                (
        ...
        return (
            ...
            (
                ...
                and (
                    (
                        self.morph_tyrannozor(state)
                        and state.has(
                            item_names.TYRANNOZOR_HEALING_ADAPTATION, self.player
                        )
                    )
                    or (usable_muta)
                )
            ...
            and (
                state.has_any(
                    (item_names.NYDUS_WORM, item_names.OVERLORD_VENTRAL_SACS),
                    self.player,
                )
                or (
                    self.advanced_tactics
                    and state.has(item_names.YGGDRASIL, self.player)
                )
            )
            # Air to ground
            and (
            ...
            and (
                ...
                or state.has_all(
                    {item_names.HYDRALISK, item_names.HYDRALISK_RESOURCE_EFFICIENCY},
                    self.player,
                )
                or state.has_all(
                    {
            ...
            and (
                state.has_any(
                    {item_names.ABERRATION, item_names.ROACH, item_names.ULTRALISK},
                    self.player,
                )
                or self.morph_tyrannozor(state)
        ...
        return (
            (
                ...
                or (
                    self.advanced_tactics
                    and (
                        state.has(item_names.ARBITER, self.player)
                        or state.has_all(
                            (item_names.MISTWING, item_names.MISTWING_PILOT),
                            self.player,
                        )
                    )
                )
    ...
    def terran_engine_of_destruction_requirement(self, state: CollectionState) -> int:
        ...
        if (
            power_rating < 3
            or not self.marine_medic_upgrade(state)
            or not self.terran_common_unit(state)
        ):
            return False
        if power_rating >= 7 and self.terran_competent_comp(state):
            ...
            return (
                state.has_any(
                    (item_names.WRAITH, item_names.BATTLECRUISER), self.player
                )
                or self.terran_air_anti_air(state)
                and state.has_any(
                    (item_names.BANSHEE, item_names.LIBERATOR), self.player
                )
            )
    ...
    def zerg_repair_odin(self, state: CollectionState):
        return state.has_all(
            {item_names.SWARM_QUEEN_BIO_MECHANICAL_TRANSFUSION, item_names.SWARM_QUEEN},
            self.player,
    ...
    def terran_in_utter_darkness_requirement(self, state: CollectionState) -> bool:
        return (
            self.terran_competent_comp(state)
            and self.terran_defense_rating(state, True, True) >= 8
        )
    ...
    def protoss_in_utter_darkness_requirement(self, state: CollectionState) -> bool:
        return (
            self.protoss_competent_comp(state)
            and self.protoss_defense_rating(state, True) >= 4
        )
    ...
    def terran_all_in_requirement(self, state: CollectionState):
        ...
        beats_kerrigan = (
            state.has_any(
                {item_names.MARINE, item_names.DOMINION_TROOPER, item_names.BANSHEE},
                self.player,
            )
            or state.has_all(
                {item_names.REAPER, item_names.REAPER_RESOURCE_EFFICIENCY}, self.player
            ...
            or (
                self.all_in_map == AllInMap.option_air
                and state.has_all(
                    (item_names.VALKYRIE, item_names.VALKYRIE_FLECHETTE_MISSILES),
                    self.player,
                )
            ...
            or (
                self.advanced_tactics
                and state.has_all(
                    (item_names.GHOST, item_names.GHOST_EMP_ROUNDS), self.player
                )
            )
        )
        ...
        if self.all_in_map == AllInMap.option_ground:
            ...
            if state.has_any(
                {item_names.BATTLECRUISER, item_names.BANSHEE}, self.player
            ):
                defense_rating += 2
            ...
            return (
                ...
                and self.terran_competent_anti_air(state)
                and state.has_any(
                    {item_names.VIKING, item_names.BATTLECRUISER, item_names.VALKYRIE},
                    self.player,
                )
                and state.has_any(
                    {
    ...
    def zerg_all_in_requirement(self, state: CollectionState):
        ...
        if self.all_in_map == AllInMap.option_ground:
            ...
            return (
                defense_rating >= 9
                and state.has_any(
                    {item_names.MUTALISK, item_names.CORRUPTOR}, self.player
                )
                and state.has_any(
                    {item_names.SPORE_CRAWLER, item_names.INFESTED_MISSILE_TURRET},
                    self.player,
    ...
    def protoss_all_in_requirement(self, state: CollectionState):
        ...
        beats_kerrigan = (
            ...
            or state.has_all(
                {item_names.SIGNIFIER, item_names.SIGNIFIER_FEEDBACK}, self.player
            )
            or (
                self.protoss_can_merge_archon(state)
                and state.has(item_names.ARCHON_HIGH_ARCHON, self.player)
            )
            or (
                self.protoss_can_merge_dark_archon(state)
                and state.has(item_names.DARK_ARCHON_FEEDBACK, self.player)
            )
        )
        if not beats_kerrigan:
            return False
        ...
        if self.all_in_map == AllInMap.option_ground:
            ...
            if (
                ...
                or state.has_all(
                    (item_names.AVENGER, item_names.AVENGER_KRYHAS_CLOAK), self.player
                )
            ):
                defense_rating += 2
    ...
    def zerg_can_grab_ghosts_in_the_fog_east_rock_formation(
        self, state: CollectionState
    ) -> bool:
        return (
            state.has_any(
    ...
    def zerg_pass_vents(self, state: CollectionState) -> bool:
        return (
            self.story_tech_granted
            or state.has_any(
                {item_names.ZERGLING, item_names.HYDRALISK, item_names.ROACH},
                self.player,
            )
            or (self.advanced_tactics and state.has(item_names.INFESTOR, self.player))
        )
    ...
    def supreme_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            or (
                state.has_all(
                    {item_names.KERRIGAN_LEAPING_STRIKE, item_names.KERRIGAN_MEND},
                    self.player,
                )
    ...
    def terran_hand_of_darkness_requirement(self, state: CollectionState) -> bool:
        return (
            self.terran_competent_comp(state) and self.terran_power_rating(state) >= 6
        )
    ...
    def protoss_hand_of_darkness_requirement(self, state: CollectionState) -> bool:
        return (
            self.protoss_competent_comp(state) and self.protoss_power_rating(state) >= 6
        )
    ...
    def terran_the_reckoning_requirement(self, state: CollectionState) -> bool:
        return self.terran_very_hard_mission_weapon_armor_level(
            state
    ...
    def protoss_can_attack_behind_chasm(self, state: CollectionState) -> bool:
        return (
            ...
            or (
                ...
                and (
                    self.protoss_common_unit(state)
                    or state.has(item_names.WARP_PRISM_PHASE_BLASTER, self.player)
                )
            )
            or (
                self.advanced_tactics
    ...
    def templars_return_phase_3_reach_colossus_requirement(
        self, state: CollectionState
    ) -> bool:
    ...
    def templars_return_phase_3_reach_dts_requirement(
        self, state: CollectionState
    ) -> bool:
        ...
        return (
            ...
            and (
                ...
                or (
                    self.advanced_tactics
                    and state.has_all(
                        {item_names.SPECTRE, item_names.SPECTRE_PSIONIC_LASH},
                        self.player,
                    )
                )
                or (
    ...
    def protoss_amons_reach_requirement(self, state: CollectionState) -> bool:
        return (
            self.protoss_common_unit_anti_light_air(state)
            and self.protoss_basic_splash(state)
            and self.protoss_power_rating(state) >= 7
        )
    ...
    def terran_last_stand_requirement(self, state: CollectionState) -> bool:
        return (
            self.terran_common_unit(state)
            and state.has_any(
                {item_names.SIEGE_TANK, item_names.LIBERATOR}, self.player
            )
            and state.has_any(
                {
    ...
    def zerg_last_stand_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            and (
                self.morph_devourer(state)
                or state.has_all(
                    {item_names.MUTALISK, item_names.MUTALISK_SUNDERING_GLAIVE},
                    self.player,
                )
                or self.advanced_tactics
    ...
    def terran_temple_of_unification_requirement(self, state: CollectionState) -> bool:
        return (
            self.terran_beats_protoss_deathball(state)
            and self.terran_power_rating(state) >= 10
        )
    ...
    def protoss_temple_of_unification_requirement(self, state: CollectionState) -> bool:
        return (
            self.protoss_competent_comp(state)
            and self.protoss_power_rating(state) >= 10
        )
    ...
    def zerg_harbinger_of_oblivion_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            and (
                self.take_over_ai_allies
                or (self.zerg_competent_comp(state) and self.zerg_big_monsters(state))
            )
            and self.zerg_power_rating(state) >= 6
        )
        ...
        return (
            ...
            and (
                ...
                or (
                    ...
                    and (
                        state.has_all(
                            {item_names.SIEGE_TANK, item_names.SIEGE_TANK_SMART_SERVOS},
                            self.player,
                        )
                        or (
                            state.has_all(
        ...
        return (
            ...
            and (
                ...
                or (
                    ...
                    and (
                        self.advanced_tactics
                        or state.has(
                            item_names.SIEGE_TANK_MAELSTROM_ROUNDS, self.player
                        )
                    )
                )
            )
            and (
                ...
                or (
                    ...
                    and (
                        state.has(
                            item_names.BANSHEE_SHOCKWAVE_MISSILE_BATTERY, self.player
                        )
                        or (
                            self.advanced_tactics
                            and state.has(
                                item_names.BANSHEE_ROCKET_BARRAGE, self.player
                            )
                        )
                    )
                )
    ...
    def terran_steps_of_the_rite_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            and (
                ...
                or state.has_all(
                    (item_names.BANSHEE, item_names.BANSHEE_SHOCKWAVE_MISSILE_BATTERY),
                    self.player,
                )
            )
            and (
                ...
                or state.has_all(
                    (item_names.VIKING, item_names.VIKING_RIPWAVE_MISSILES), self.player
                )
            )
            and self.terran_very_hard_mission_weapon_armor_level(state)
        )
    ...
    def zerg_rak_shir_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            and (
                ...
                or (
                    self.advanced_tactics
                    and state.has(item_names.INFESTOR, self.player)
                )
            )
            and self.zerg_power_rating(state) >= 10
        )
            ...
            and (
                ...
                or (
                    ...
                    and (
                        ...
                        or state.has_all(
                            (item_names.MIRAGE, item_names.MIRAGE_GRAVITON_BEAM),
                            self.player,
                        )
                    )
                )
    ...
    def terran_templars_charge_requirement(self, state: CollectionState) -> bool:
        return self.terran_very_hard_mission_weapon_armor_level(state) and (
            (
                ...
                and state.count(
                    item_names.BATTLECRUISER_PROGRESSIVE_DEFENSIVE_MATRIX, self.player
                )
                >= 2
            )
            or (
        ...
        return (
            ...
            and (
                ...
                or (
                    ...
                    and state.has_any(
                        {item_names.VIKING, item_names.VALKYRIE}, self.player
                    )
                )
                or self.take_over_ai_allies
                and self.protoss_fleet(state)
        ...
        if self.take_over_ai_allies:
                    ...
                    or (
                        ...
                        and (
                            ...
                            or (
                                ...
                                == SpearOfAdunPassiveAbilityPresence.option_everywhere
                                and state.has(
                                    item_names.RECONSTRUCTION_BEAM, self.player
                                )
                            )
                        )
                    )
            ...
            return (
                (
                    ...
                    or state.has_all(
                        (item_names.SCOURGE, item_names.SCOURGE_RESOURCE_EFFICIENCY),
                        self.player,
                    )
                    or self.morph_brood_lord(state)
                    or self.morph_guardian(state)
    ...
    def the_escape_stuff_granted(self) -> bool:
        ...
        return self.story_tech_granted or (
            self.mission_order == MissionOrder.option_vanilla
            and self.enabled_campaigns == {SC2Campaign.NCO}
        )

    def the_escape_first_stage_requirement(self, state: CollectionState) -> bool:
        return self.the_escape_stuff_granted() or (
            self.nova_ranged_weapon(state)
            and (self.nova_full_stealth(state) or self.nova_heal(state))
        )

    def the_escape_requirement(self, state: CollectionState) -> bool:
        return self.the_escape_first_stage_requirement(state) and (
            self.the_escape_stuff_granted() or self.nova_splash(state)
        )
    ...
    def enemy_intelligence_cliff_garrison(self, state: CollectionState) -> bool:
        return (
            ...
            or (
                self.advanced_tactics
                and state.has_any(
                    {item_names.HELS_ANGELS, item_names.BRYNHILDS}, self.player
                )
            )
        )

    def enemy_intelligence_first_stage_requirement(
        self, state: CollectionState
    ) -> bool:
        return (
    ...
    def enemy_intelligence_second_stage_requirement(
        self, state: CollectionState
    ) -> bool:
        return (
    ...
    def enemy_intelligence_third_stage_requirement(
        self, state: CollectionState
    ) -> bool:
    ...
    def enemy_intelligence_cliff_garrison_and_nova_mobility(
        self, state: CollectionState
    ) -> bool:
        return self.enemy_intelligence_cliff_garrison(state) and (
            ...
            or (
                state.has(
                    item_names.NOVA_PROGRESSIVE_STEALTH_SUIT_MODULE, self.player, 2
                )
                and state.has_any(
                    (item_names.NOVA_FLASHBANG_GRENADES, item_names.NOVA_BLINK),
                    self.player,
                )
            )
        )
            ...
            and (
                ...
                or (
                    (
                        ...
                        (
                            state.has_any(
                                (item_names.FIREBAT, item_names.REAPER), self.player
                            )
                            or state.has_all(
                                {item_names.HELLION, item_names.HELLION_HELLBAT},
                                self.player,
                            )
                        )
                        and self.terran_bio_heal(state)
                    ...
                    and (
                        ...
                        or (
                            ...
                            or (
                                state.has(item_names.HERC, self.player)
                                and self.terran_bio_heal(state)
                            )
                        )
                    )
    ...
    def enemy_shadow_nova_damage_and_blazefire_unlock(
        self, state: CollectionState
    ) -> bool:
        return self.nova_any_nobuild_damage(state) and (
            state.has(item_names.NOVA_BLINK, self.player)
            or state.has_all(
                (item_names.NOVA_HOLO_DECOY, item_names.NOVA_DOMINATION), self.player
            )
        )
    ...
    def enemy_shadow_second_stage(self, state: CollectionState) -> bool:
        return self.enemy_shadow_first_stage(state) and (
            ...
            and (
                self.advanced_tactics
                or state.has(item_names.NOVA_GHOST_VISOR, self.player)
            )
        )

    def enemy_shadow_door_controls(self, state: CollectionState) -> bool:
        return self.enemy_shadow_second_stage(state) and (
            self.story_tech_granted or self.enemy_shadow_door_unlocks_tool(state)
        )

    def enemy_shadow_victory(self, state: CollectionState) -> bool:
        return self.enemy_shadow_door_controls(state) and (
            self.story_tech_granted or self.nova_heal(state)
        )
    ...
    def end_game_requirement(self, state: CollectionState) -> bool:
        return (
            ...
            and (
                # Xanthos
                state.has_any(
                    (item_names.BATTLECRUISER, item_names.VIKING, item_names.WARHOUND),
                    self.player,
                )
                or state.has_all(
                    (item_names.LIBERATOR, item_names.LIBERATOR_SMART_SERVOS),
                    self.player,
                )
                or state.has_all(
                    (item_names.THOR, item_names.THOR_PROGRESSIVE_HIGH_IMPACT_PAYLOAD),
                    ...
                    and (
                        (
                            ...
                            and (
                                ...
                                or state.count(
                                    item_names.MARINE_PROGRESSIVE_STIMPACK, self.player
                                )
                                >= 2
                            )
                        )
                        or (
                            state.has(item_names.DOMINION_TROOPER, self.player)
                            and self.terran_bio_heal(state)
                        )
                        or state.has_all(
                            (
                        ...
                        or state.has_all(
                            (item_names.CYCLONE, item_names.CYCLONE_TARGETING_OPTICS),
                            self.player,
                        )
                    )
                )
            ...
            and (  # The enemy has 3/3 BCs
                ...
                or state.has_all(
                    (item_names.GHOST, item_names.GHOST_LOCKDOWN), self.player
                )
                or state.has_all(
                    (
                        item_names.BATTLECRUISER,
    ...
    def has_terran_units(self, target: int) -> Callable[["CollectionState"], bool]:
        def _has_terran_units(state: CollectionState) -> bool:
            return (
                ...
                or state.has_all(
                    (item_names.EMPERORS_GUARDIAN, item_names.LIBERATOR_RAID_ARTILLERY),
                    self.player,
                )
                or state.has_all(
                    (item_names.VALKYRIE, item_names.VALKYRIE_FLECHETTE_MISSILES),
                ...
                or state.has_all(
                    (item_names.WIDOW_MINE, item_names.WIDOW_MINE_DEMOLITION_ARMAMENTS),
                    self.player,
                )
            )
    ...
    def has_zerg_units(self, target: int) -> Callable[["CollectionState"], bool]:
        def _has_zerg_units(state: CollectionState) -> bool:
            return (
                state.count_from_list_unique(
                    item_groups.zerg_units + item_groups.zerg_buildings, self.player
                )
                >= target
            ) and (
                # Anything that can hit buildings
    ...
    def has_protoss_units(self, target: int) -> Callable[["CollectionState"], bool]:
        def _has_protoss_units(state: CollectionState) -> bool:
            return (
                ...
                or state.has_all(
                    (item_names.CALADRIUS, item_names.CALADRIUS_CORONA_BEAM),
                    self.player,
                )
                or state.has_all(
                    (item_names.PHOTON_CANNON, item_names.KHALAI_INGENUITY), self.player
                )
            )
