...
def have_light_source(state: CollectionState, player: int) -> bool:
    return (
        ...
        or (
            state.has("Stick", player)
            and state.has("Boots", player)
            and state.can_reach_region("Swamp Gas Cavern", player)
        )
        or (state.has("20/20 Vision", player))
    )
...
def have_39_badges(state: CollectionState, player: int) -> bool:
    return state.has_group("cheats", player, 39)
