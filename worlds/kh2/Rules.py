...
def set_rules(world: MultiWorld, player: int):
    # three proofs
    if world.Visitlocking[player].value == 0:
        firstvisitlocking = 0
        secondvisitlocking = 0
    elif world.Visitlocking[player].value == 1:
        firstvisitlocking = 0
        secondvisitlocking = 1
    elif world.Visitlocking[player].value == 2:
        firstvisitlocking = 1
        secondvisitlocking = 0
    elif world.Visitlocking[player].value == 3:
        firstvisitlocking = 1
    ...
    for location in {LocationName.FatalCrestGoddessofFateCup, LocationName.OrichalcumPlusGoddessofFateCup,
        ...
        add_rule(world.get_location(location, player),
                 lambda state:
                 state.kh_ag_unlocked(player, firstvisitlocking)
                 and state.kh_ht_unlocked(player, firstvisitlocking)
                 and state.kh_pl_unlocked(player, firstvisitlocking)
                 and state.kh_oc_unlocked(player, firstvisitlocking)
                 and state.kh_dc_unlocked(player, firstvisitlocking)
    ...
    for location in {LocationName.ProtectBeltPainandPanicCup, LocationName.SerenityGemPainandPanicCup}:
        add_rule(world.get_location(location, player),
                 lambda state: state.kh_dc_unlocked(player, firstvisitlocking))
