...
class KH2World(World):
    ...
    def generate_basic(self):
        ...
        add_rule(self.world.get_location(LocationName.FinalXemnas,self.player),lambda state:state.kh_twtnw_unlocked(self.player))
        self.world.get_location(LocationName.FinalXemnas, self.player).place_locked_item(self.create_item())
        ...
        exclude = [item for item in self.world.KH2StartingItems[self.player].value]
        #if self.world.Schmovement[self.player].value:
        ...
        print(exclude)
        print(corChecks)
        print(self.world.precollected_items)
        print(exclusion_table["Level50"])    
        for item in item_dictionary_table:
            if item in exclude:
                #self.world.get_location(LocationName.AbuEscort, self.player).place_locked_item(self.create_item(item))
                print(item)
            else:
                itempool += self._create_items(item)
        
        self.world.get_location(LocationName.Roxas,self.player).place_locked_item(self.create_item(ItemName.ProofofPeace))
        
        
        self.world.itempool += itempool
        print(self.world.get_filled_locations)
        print(self.world.itempool)
    ...
    def _get_slot_data(self):
        return {
            ...
            "KH2StartItems":self.world.KH2StartingItems[self.player].value
        }
