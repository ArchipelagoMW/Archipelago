...
cvcotm_char_dict = {"\n": 0x09, " ": 0x26, "!": 0x4A, '"': 0x51, "#": 0x40, "$": 0x41, "%": 0x68, "&": 0x73, "'": 0x51,
                    "(": 0x54, ")": 0x55, "*": 0x51, "+": 0x50, ",": 0x4C, "-": 0x58, ".": 0x35, "/": 0x70, "0": 0x64,
                    "1": 0x6A, "2": 0x63, "3": 0x6C, "4": 0x71, "5": 0x69, "6": 0x3C, "7": 0x47, "8": 0x72, "9": 0x39,
                    ":": 0x35, ";": 0x4C, "<": 0x5C, "=": 0x5B, ">": 0x5B, "?": 0x48, "@": 0x29, "A": 0x3E, "B": 0x4D,
                    "C": 0x44, "D": 0x45, "E": 0x4E, "F": 0x56, "G": 0x4F, "H": 0x40, "I": 0x43, "J": 0x6B, "K": 0x66,
                    "L": 0x5F, "M": 0x42, "N": 0x52, "O": 0x67, "P": 0x4B, "Q": 0x64, "R": 0x46, "S": 0x41, "T": 0x47,
                    "U": 0x60, "V": 0x6E, "W": 0x49, "X": 0x6D, "Y": 0x53, "Z": 0x6F, "[": 0x59, "\\": 0x70, "]": 0x5A,
                    "^": 0x5D, "_": 0x58, "a": 0x29, "b": 0x3C, "c": 0x33, "d": 0x32, "e": 0x28, "f": 0x3A, "g": 0x39,
...
def cvcotm_string_to_bytearray(cvcotm_text: str, textbox_type: Literal["big top", "big bottom", "little middle"],
    ...
    for i, char in enumerate(cvcotm_text):
        if char == "\b":
            # Clear the text and reset the text alignment to the left. Typically, this is after an A prompt.
...
def cvcotm_text_truncate(cvcotm_text: str, textbox_len_limit: int) -> str:
    ...
    for i in range(len(cvcotm_text)):
        line_len += cvcotm_char_dict[cvcotm_text[i]][1]
...
def cvcotm_text_wrap(cvcotm_text: str, textbox_len_limit: int, total_lines: int = 4) -> str:
    ...
    for i in range(len(words)):
        ...
        for j, char in enumerate(words[i]):
            ...
            # If we're over the allowed number of lines to be displayed in the textbox, insert the A advance characters.
