...
def has_propulsion_cannon(state, player):
    return state.has("Propulsion Cannon", player, 2) or \
           state.has("Repulsion Cannon", player, 1) # This works too
...
def get_max_swim_depth(state, player):
    ...
    # Only go up to 350m without any submarines for now.
    # Also TODO, make this even smaller. Playing without seaglide is just not
    #  fun and painfully slow. Tweak rules!
    return 350
...
def can_access_location(state, player, loc):
    location_depth = loc.get("depth", 0)
    need_laser_cutter = loc.get("need_laser_cutter", False)
    can_slip_through = loc.get("can_slip_through", False) #TODO, options to allow glitch?
    need_radiation_suit = loc.get("need_radiation_suit", False)
    ...
    return get_max_depth(state, player) >= location_depth
...
def set_rules(world, player):
    ...
    # Victory location
    set_rule(world.get_location("Neptune Launch Button", player), lambda state: get_max_depth(state, player) >= 1444 and state.has('Neptune Launch Platform', player))
