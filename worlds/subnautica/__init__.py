...
class SubnauticaWorld(World):
    ...
    # { "id": 33110, "depth": 0, "game_id": "(974.8, 10.0, -77.0)",
    #   "need_laser_cutter": true, "can_slip_through": false, "need_radiation_suit": true, "need_propulsion_cannon": true,
    ...
    def generate_basic(self):
        ...
        for item in item_table:
            for i in range(item["count"]):
                subnautica_item = SubnauticaItem(item["name"], True, item["id"], player = self.player)
        ...
        self.world.get_location("Neptune Launch Button", self.player).place_locked_item(SubnauticaItem("Victory", True, None, player = self.player))
