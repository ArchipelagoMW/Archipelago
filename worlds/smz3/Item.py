...
class Item:
    ...
    def Is(self, type: ItemType, world: World):
        return self.Type == type and self.World == world

    def IsNot(self, type: ItemType, world: World):
        return not self.Is(type, world)

    def __init__(self, itemType: ItemType):
        #self.Name = itemType.GetDescription()
        self.Type = itemType

    def __init__(self, itemType: ItemType, world: World):
        self.__init__(itemType)
        self.World = world

    @staticmethod
    def Nothing(world: World):
        return Item(ItemType.Nothing, world)

    @staticmethod
    def AddRange(itemPool, count, item):
        for i in range(count):
            itemPool.append(copy(item))

    @staticmethod
    def CreateProgressionPool(world: World):
        itemPool = [
    ...
    @staticmethod
    def CreateNicePool(world: World):
        itemPool = [
    ...
    @staticmethod
    def CreateJunkPool(world: World):
        itemPool = [
            Item(ItemType.Arrow),
    ...
    @staticmethod
    def CreateDungeonPool(world: World):
        itemPool = [
    ...
    @staticmethod
    def CreateKeycards(world: World):
        return [
    ...
    @staticmethod
    def Get(items, itemType:ItemType, world: World):
...
class Progression:
    ...
    def CanAccessMaridiaPortal(self, world: World):
        if (world.config.SMLogic == SMLogic.Normal):
            return self.MoonPearl and self.Flippers and \
                    self.Gravity and self.Morph and \
                    (world.CanAquire(self, RewardType.Agahnim) or self.Hammer and self.CanLiftLight() or self.CanLiftHeavy())
        else:
            return self.MoonPearl and self.Flippers and \
