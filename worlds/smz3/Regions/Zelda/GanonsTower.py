...
class GanonsTower(Z3Region):
        ...
        self.Locations = [
            ...
            Location(self, 256+194, 0x1EAD3, LocationType.Regular, "Ganon's Tower - Map Chest",
                lambda items: items.Hammer and (items.Hookshot or items.Boots) and items.KeyGT >=
                    (3 if any(self.GetLocation("Ganon's Tower - Map Chest").ItemIs(type, World) for type in [ItemType.BigKeyGT, ItemType.KeyGT]) else 4))
                .AlwaysAllow(lambda item, items: item.Is(ItemType.KeyGT, World) and items.KeyGT >= 3),
            Location(self, 256+195, 0x1EAD0, LocationType.Regular, "Ganon's Tower - Firesnake Room",
                lambda items: items.Hammer and items.Hookshot and items.KeyGT >= (2 if any(l.ItemIs(ItemType.BigKeyGT, World) for l in [
                    ...
                    ]) or self.GetLocation("Ganon's Tower - Firesnake Room").ItemIs(ItemType.KeyGT, World) else 3)),
            ...
            Location(self, 256+208, 0x1EAD6, LocationType.Regular, "Ganon's Tower - Big Chest",
                lambda items: items.BigKeyGT and items.KeyGT >= 3 and (
                    items.Hammer and items.Hookshot or
                    items.Somaria and items.Firerod))
                .Allow(lambda item, items: item.IsNot(ItemType.BigKeyGT, World)),
            ...
            Location(self, 256+215, 0x1EB06, LocationType.Regular, "Ganon's Tower - Moldorm Chest",
                ...
                .Allow(lambda item, items: all(item.IsNot(type, World) for type in [ ItemType.KeyGT, ItemType.BigKeyGT ]))
            ]

    def LeftSide(self, items: Progression, locations: List[Location]):
        return items.Hammer and items.Hookshot and items.KeyGT >= (3 if any(l.ItemIs(ItemType.BigKeyGT, World) for l in locations) else 4)

    def RightSide(self, items: Progression, locations: List[Location]):
        return items.Somaria and items.Firerod and items.KeyGT >= (3 if any(l.ItemIs(ItemType.BigKeyGT, World) for l in locations) else 4)
    ...
    def CanEnter(self, items: Progression):
        return items.MoonPearl and World.CanEnter("Dark World Death Mountain East", items) and \
            World.CanAquireAll(items, [ RewardType.CrystalBlue, RewardType.CrystalRed, RewardType.GoldenFourBoss ])

    def CanFill(self, item: Item, items: Progression):
        if (Config.GameMode == GameMode.Multiworld):
            if (item.World != World or item.Progression):
                return False
            if (Config.KeyShuffle == KeyShuffle.Keysanity and not ((item.Type == ItemType.BigKeyGT or item.Type == ItemType.KeyGT) and item.World == World) and (item.IsKey or item.IsBigKey or item.IsKeycard)):
                return False
