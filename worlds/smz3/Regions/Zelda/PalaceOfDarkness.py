...
class MiseryMire(Z3Region, IReward):
    ...
    def __init__(self, world: World, config: Config):
        ...
        self.Locations = [
            ...
            Location(self, 256+122, 0x1EA37, LocationType.Regular, "Palace of Darkness - Big Key Chest",
                lambda items: items.KeyPD >= (1 if self.GetLocation("Palace of Darkness - Big Key Chest").ItemIs(ItemType.KeyPD, World) else
                    6 if (items.Hammer and items.Bow and items.Lamp) or config.Keysanity else 5))
                .AlwaysAllow(lambda item, items: item.Is(ItemType.KeyPD, World) and items.KeyPD >= 5),
            ...
            Location(self, 256+128, 0x1EA46, LocationType.Regular, "Palace of Darkness - Harmless Hellway",
                lambda items: items.KeyPD >= (4 if (items.Hammer and items.Bow and items.Lamp) or config.Keysanity else 3 if 
                        self.GetLocation("Palace of Darkness - Harmless Hellway").ItemIs(ItemType.KeyPD, World) else
                        6 if (items.Hammer and items.Bow and items.Lamp) or config.Keysanity else 5))
                .AlwaysAllow(lambda item, items: item.Is(ItemType.KeyPD, World) and items.KeyPD >= 5),
    ...
    def CanEnter(self, items: Progression):
        return items.MoonPearl and World.CanEnter("Dark World North East", items)
