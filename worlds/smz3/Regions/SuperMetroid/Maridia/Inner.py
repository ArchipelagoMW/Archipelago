...
class West(SMRegion, IReward):
        ...
        self.Locations = [
            ...
            Location(self, 151, 0x8FC74D, LocationType.Hidden, "Missile (Draygon)",
                lambda items: 
                    ...
                    items.CanAccessMaridiaPortal(World) if self.Logic == SMLogic.Normal else \
                lambda items: (
                        items.CardMaridiaL1 and items.CardMaridiaL2 and self.CanDefeatBotwoon(items) or
            ...
            Location(self, 152, 0x8FC755, LocationType.Visible, "Energy Tank, Botwoon",
                lambda items: 
                    items.CardMaridiaL1 and items.CardMaridiaL2 and self.CanDefeatBotwoon(items) or
                    items.CanAccessMaridiaPortal(World) and items.CardMaridiaL2),
    ...
    def CanReachAqueduct(self, items: Progression):
        if self.Logic == SMLogic.Normal:
            return items.CardMaridiaL1 and (items.CanFly() or items.SpeedBooster or items.Grapple) \
                    or items.CardMaridiaL2 and items.CanAccessMaridiaPortal(World)
        else:
    ...
    def CanDefeatBotwoon(self, items: Progression):
        if self.Logic == SMLogic.Normal:
            return items.SpeedBooster or items.CanAccessMaridiaPortal(World)
        else:
            return items.Ice or items.SpeedBooster and items.Gravity or items.CanAccessMaridiaPortal(World)


    def CanEnter(self, items: Progression):
        if self.Logic == SMLogic.Normal:
            return items.Gravity and (
                World.CanEnter("Norfair Upper West", items) and items.Super and items.CanUsePowerBombs() and
                    (items.CanFly() or items.SpeedBooster or items.Grapple) or
                items.CanAccessMaridiaPortal(World))
        else:
            return items.Super and World.CanEnter("Norfair Upper West", items) and items.CanUsePowerBombs() and (
