    ...
    def CanEnter(self, items:Progression):
        if self.Logic == SMLogic.Normal:
            return items.Gravity and (
                    World.CanEnter("Norfair Upper West", items) and items.CanUsePowerBombs() or
                    items.CanAccessMaridiaPortal(World) and items.CardMaridiaL1 and items.CardMaridiaL2 and (items.CanPassBombPassages() or items.ScrewAttack))
        else:
            return World.CanEnter("Norfair Upper West", items) and items.CanUsePowerBombs() and (
                    items.Gravity or items.HiJump and (items.CanSpringBallJump() or items.Ice)) or (
                items.CanAccessMaridiaPortal(World)) and items.CardMaridiaL1 and items.CardMaridiaL2 and (
