        ...
        self.Locations = [
            Location(self, 1, 0x8F81E8, LocationType.Visible, "Missile (outside Wrecked Ship bottom)",
                lambda items: items.Morph and (
                    ...
                    World.CanEnter("Wrecked Ship", items)) if self.Logic == SMLogic.Normal else \
                lambda items: items.Morph),
            Location(self, 2, 0x8F81EE, LocationType.Hidden, "Missile (outside Wrecked Ship top)",
                lambda items: World.CanEnter("Wrecked Ship", items) and (not Config.Keysanity or items.CardWreckedShipBoss) and items.CanPassBombPassages()),
    ...
    def CanEnter(self, items:Progression):
        if self.Logic == SMLogic.Normal:
            ...
            return (items.CardCrateriaL2 if Config.Keysanity else items.CanUsePowerBombs()) and items.Super or (
                ...
                items.CanAccessMaridiaPortal(World)) and items.Gravity and items.Super and (
                    # /* Oasis -> Forgotten Highway */
                    items.CardMaridiaL2 and items.CanDestroyBombWalls() or (
                    # /* Draygon -> Cactus Alley -> Forgotten Highway */
                    World.GetLocation("Space Jump").Available(items))) or (
            ...
            return (items.CardCrateriaL2 if Config.Keysanity else  items.CanUsePowerBombs()) and items.Super or (
                ...
                items.CanAccessMaridiaPortal(World)) and (
                    ...
                    # /* Draygon -> Cactus Alley -> Forgotten Highway */
                    items.Gravity and World.GetLocation("Space Jump").Available(items)) or (
                # /*Through Maridia from Pipe*/
