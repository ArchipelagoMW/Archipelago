...
LOCATIONS = [constants.VILLAGE_OF_ALL_BEGINNINGS_MAILBOX]

LOCATION_NAME_TO_ID = {
    name: id for id, name in enumerate(LOCATIONS, constants.BASE_ID)
}
...
def get_location_names_with_ids(
...
def create_regular_locations(world: TombaWorld) -> None:
    village_of_all_beginnings = world.get_region(
        constants.VILLAGE_OF_ALL_BEGINNINGS
    )
    ...
    village_of_all_beginnings.add_locations(
        village_of_all_beginnings_locations, TombaLocation
    )
...
def create_events(world: TombaWorld) -> None:
    ...
    village_of_all_beginnings.add_event(
        constants.FOG_DISSIPATED,
        constants.VICTORY,
        location_type=TombaLocation,
        item_type=items.TombaItem,
    )
