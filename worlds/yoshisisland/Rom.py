...
def Handle_Items(rom):
    ...
    rom.write_bytes(0x007800, bytearray([0xA0, 0x05, 0x98, 0x9D, 0xA2, 0x74, 0x60, 0xFA, 0xA0, 0xFF, 0x98, 0x9D, 0xA2, 0x74, 0x60, 0xFA])),
...
def patch_rom(world, rom, player: int):
    ...
    rom.write_bytes(0x06FC85, ([world.castle_open_condition[player].value]))
    rom.write_bytes(0x06FC86, ([world.castle_clear_condition[player].value]))
    rom.write_bytes(0x06FC87, ([world.disable_autoscroll[player].value]))
    rom.write_bytes(0x06FC8B, ([world.minigame_checks[player].value]))
    ...
    elif get_option_value(world, player, "yoshi_colors") == 2 or get_option_value(world, player, "yoshi_colors") == 3:
        rom.write_bytes(0x010000, bytearray(world.level_colors))

    if get_option_value(world, player, "minigame_checks") == 2 or get_option_value(world, player, "minigame_checks") == 3:
        bonus_checks(rom)

    if get_option_value(world, player, "minigame_checks") == 1 or get_option_value(world, player, "minigame_checks") == 3:
        bandit_checks(rom)

    rom.write_bytes(0x00BF2C, bytearray(world.world_bonus))

    if get_option_value(world, player, "softlock_prevention") == 1:
        rom.write_bytes(0x00C18F, bytearray([0x5C, 0x58, 0xFB, 0x0B])), #R + X Code
    
    from Main import __version__
    rom.name = bytearray(f'YOSHIAP{__version__.replace(".", "")[0:3]}_{player}_{world.seed:11}\0', 'utf8')[:15]
