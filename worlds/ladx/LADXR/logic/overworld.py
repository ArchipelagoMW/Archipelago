...
from .location import OverworldLocation, IndoorLocation, VirtualLocation
...
class World:
    def __init__(self, options, world_setup, r):
        ...
        mabe_village = OverworldLocation("Mabe Village")
        IndoorLocation("Mabe Village Well").add(HeartPiece(0x2A4)).connect(mabe_village, r.bush)  # well
        OverworldLocation("Fishing Game").add(FishingMinigame()).connect(mabe_village, AND(r.bush, COUNT("RUPEES", 20)))  # fishing game, heart piece is directly done by the minigame.
        VirtualLocation().add(Seashell(0x0A3)).connect(mabe_village, r.bush)  # bushes below the shop
        VirtualLocation().add(Seashell(0x0D2)).connect(mabe_village, PEGASUS_BOOTS)  # smash into tree next to lv1
        VirtualLocation().add(Song(0x092)).connect(mabe_village, OCARINA)  # Marins song
        rooster_cave = IndoorLocation("Rooster Cave")
        VirtualLocation().add(DroppedKey(0x1E4)).connect(rooster_cave, AND(OCARINA, SONG3))

        papahl_house = IndoorLocation("Papahl House")
        papahl_house.connect(VirtualLocation().add(TradeSequenceItem(0x2A6, TRADING_ITEM_RIBBON)), TRADING_ITEM_YOSHI_DOLL)

        trendy_shop = IndoorLocation("Trendy Shop").add(TradeSequenceItem(0x2A0, TRADING_ITEM_YOSHI_DOLL))
        ...
        start_house = IndoorLocation("Tarin's House").add(StartItem())
        self._addEntrance("start_house", mabe_village, start_house, None)

        shop = IndoorLocation("Shop")
        VirtualLocation().add(ShopItem(0)).connect(shop, OR(COUNT("RUPEES", 500), SWORD))
        VirtualLocation().add(ShopItem(1)).connect(shop, OR(COUNT("RUPEES", 1480), SWORD))
        self._addEntrance("shop", mabe_village, shop, None)

        dream_hut = IndoorLocation("Dream Hut")
        dream_hut_right = VirtualLocation().add(Chest(0x2BF)).connect(dream_hut, SWORD)
        if options.logic != "casual":
            dream_hut_right.connect(dream_hut, OR(BOOMERANG, HOOKSHOT, FEATHER))
        dream_hut_left = VirtualLocation().add(Chest(0x2BE)).connect(dream_hut_right, PEGASUS_BOOTS)
        self._addEntrance("dream_hut", mabe_village, dream_hut, POWER_BRACELET)

        kennel = IndoorLocation("Kennel").connect(VirtualLocation().add(Seashell(0x2B2)), SHOVEL)  # in the kennel
        kennel.connect(VirtualLocation().add(TradeSequenceItem(0x2B2, TRADING_ITEM_DOG_FOOD)), TRADING_ITEM_RIBBON)
        self._addEntrance("kennel", mabe_village, kennel, None)

        sword_beach = OverworldLocation("Sword Beach").add(BeachSword()).connect(mabe_village, OR(r.bush, SHIELD, r.attack_hookshot))
        banana_seller = IndoorLocation("Banana Seller")
        banana_seller.connect(VirtualLocation().add(TradeSequenceItem(0x2FE, TRADING_ITEM_BANANAS)), TRADING_ITEM_DOG_FOOD)
        self._addEntrance("banana_seller", sword_beach, banana_seller, r.bush)
        boomerang_cave = IndoorLocation("Boomerang Cave")
        if options.boomerang == 'trade':
            VirtualLocation().add(BoomerangGuy()).connect(boomerang_cave, OR(BOOMERANG, HOOKSHOT, MAGIC_ROD, PEGASUS_BOOTS, FEATHER, SHOVEL))
        elif options.boomerang == 'gift':
            VirtualLocation().add(BoomerangGuy()).connect(boomerang_cave, None)
        ...
        sword_beach_to_ghost_hut = OverworldLocation("Sword Beach to Ghost House").add(Chest(0x0E5)).connect(sword_beach, POWER_BRACELET)
        ghost_hut_outside = OverworldLocation("Outside Ghost House").connect(sword_beach_to_ghost_hut, POWER_BRACELET)
        ghost_hut_inside = IndoorLocation("Ghost House").connect(VirtualLocation().add(Seashell(0x1E3)), POWER_BRACELET)
        self._addEntrance("ghost_house", ghost_hut_outside, ghost_hut_inside, None)

        ## Forest area
        forest = OverworldLocation("Forest").connect(mabe_village, r.bush) # forest stretches all the way from the start town to the witch hut
        VirtualLocation().add(Chest(0x071)).connect(forest, POWER_BRACELET)  # chest at start forest with 2 zols
        forest_heartpiece = OverworldLocation("Forest Heart Piece").add(HeartPiece(0x044))  # next to the forest, surrounded by pits
        forest.connect(forest_heartpiece, OR(BOOMERANG, FEATHER, HOOKSHOT, ROOSTER), one_way=True)

        witch_hut = IndoorLocation("Witch's Hut").connect(VirtualLocation().add(Witch()), TOADSTOOL)
        self._addEntrance("witch", forest, witch_hut, None)
        crazy_tracy_hut = OverworldLocation("Outside Crazy Tracy's House").connect(forest, POWER_BRACELET)
        crazy_tracy_hut_inside = IndoorLocation("Crazy Tracy's House")
        VirtualLocation().add(KeyLocation("MEDICINE2")).connect(crazy_tracy_hut_inside, FOUND("RUPEES", 50))
        ...
        forest_madbatter = IndoorLocation("Forest Mad Batter")
        VirtualLocation().add(MadBatter(0x1E1)).connect(forest_madbatter, MAGIC_POWDER)
        ...
        forest_cave = IndoorLocation("Forest Cave")
        VirtualLocation().add(Chest(0x2BD)).connect(forest_cave, SWORD)  # chest in forest cave on route to mushroom
        log_cave_heartpiece = VirtualLocation().add(HeartPiece(0x2AB)).connect(forest_cave, POWER_BRACELET)  # piece of heart in the forest cave on route to the mushroom
        forest_toadstool = OverworldLocation("Mysterious Woods Toadstool").add(Toadstool())
        ...
        hookshot_cave = IndoorLocation("Hookshot Cave")
        hookshot_cave_chest = VirtualLocation().add(Chest(0x2B3)).connect(hookshot_cave, OR(HOOKSHOT, ROOSTER))
        self._addEntrance("hookshot_cave", forest, hookshot_cave, POWER_BRACELET)

        swamp = OverworldLocation("Swamp").connect(forest, AND(OR(MAGIC_POWDER, FEATHER, ROOSTER), r.bush))
        swamp.connect(forest, r.bush, one_way=True) # can go backwards past Tarin
        swamp.connect(forest_toadstool, OR(FEATHER, ROOSTER))
        swamp_chest = OverworldLocation("Swamp Chest").add(Chest(0x034)).connect(swamp, OR(BOWWOW, HOOKSHOT, MAGIC_ROD, BOOMERANG))
        self._addEntrance("d2", swamp, None, OR(BOWWOW, HOOKSHOT, MAGIC_ROD, BOOMERANG))
        forest_rear_chest = OverworldLocation("Forest Rear").add(Chest(0x041)).connect(swamp, r.bush)  # tail key
        self._addEntrance("writes_phone", swamp, None, None)

        writes_hut_outside = OverworldLocation("Outside Write's House").connect(swamp, OR(FEATHER, ROOSTER))  # includes the cave behind the hut
        writes_house = IndoorLocation("Write's House")
        writes_house.connect(VirtualLocation().add(TradeSequenceItem(0x2a8, TRADING_ITEM_BROOM)), TRADING_ITEM_LETTER)
        ...
        writes_cave = IndoorLocation("Write's Cave")
        writes_cave_left_chest = VirtualLocation().add(Chest(0x2AE)).connect(writes_cave, OR(FEATHER, ROOSTER, HOOKSHOT)) # 1st chest in the cave behind the hut
        VirtualLocation().add(Chest(0x2AF)).connect(writes_cave, POWER_BRACELET)  # 2nd chest in the cave behind the hut.
        ...
        graveyard = OverworldLocation("Graveyard").connect(forest, OR(FEATHER, ROOSTER, POWER_BRACELET))  # whole area from the graveyard up to the moblin cave
        if options.owlstatues == "both" or options.owlstatues == "overworld":
        ...
        ghost_grave = OverworldLocation("Ghost Grave").connect(forest, POWER_BRACELET)
        VirtualLocation().add(Seashell(0x074)).connect(ghost_grave, AND(r.bush, SHOVEL))  # next to grave cave, digging spot
        ...
        graveyard_cave_left = IndoorLocation("Graveyard Cave Left")
        graveyard_cave_right = IndoorLocation("Graveyard Cave Right").connect(graveyard_cave_left, OR(FEATHER, ROOSTER))
        graveyard_heartpiece = IndoorLocation("Graveyard Cave Heartpiece").add(HeartPiece(0x2DF)).connect(graveyard_cave_right, OR(AND(BOMB, OR(HOOKSHOT, PEGASUS_BOOTS), FEATHER), ROOSTER))  # grave cave
        ...
        moblin_cave = IndoorLocation("Moblin Cave").connect(VirtualLocation().add(Chest(0x2E2)), AND(r.attack_hookshot_powder, r.miniboss_requirements[world_setup.miniboss_mapping["moblin_cave"]]))
        self._addEntrance("moblin_cave", graveyard, moblin_cave, None)

        # "Ukuku Prairie"
        ukuku_prairie = OverworldLocation("Ukuku Prairie").connect(mabe_village, POWER_BRACELET).connect(graveyard, POWER_BRACELET)
        ukuku_prairie.connect(VirtualLocation().add(TradeSequenceItem(0x07B, TRADING_ITEM_STICK)), TRADING_ITEM_BANANAS)
        ukuku_prairie.connect(VirtualLocation().add(TradeSequenceItem(0x087, TRADING_ITEM_HONEYCOMB)), TRADING_ITEM_STICK)
        ...
        self._addEntrance("prairie_left_cave1", ukuku_prairie, IndoorLocation("Cave East of Mabe").add(Chest(0x2CD)), None) # cave next to town
        ...
        prairie_left_cave2 = IndoorLocation("Boots 'n' Bomb Cave Chest")  # Bomb cave
        VirtualLocation().add(Chest(0x2F4)).connect(prairie_left_cave2, PEGASUS_BOOTS)
        VirtualLocation().add(HeartPiece(0x2E5)).connect(prairie_left_cave2, AND(BOMB, PEGASUS_BOOTS))
        ...
        mamu = IndoorLocation("Mamu").connect(VirtualLocation().add(Song(0x2FB)), AND(OCARINA, COUNT("RUPEES", 1480)))
        self._addEntrance("mamu", ukuku_prairie, mamu, AND(OR(AND(FEATHER, PEGASUS_BOOTS), ROOSTER), OR(HOOKSHOT, ROOSTER), POWER_BRACELET))

        dungeon3_entrance = OverworldLocation("Key Cavern Entrance").connect(ukuku_prairie, OR(FEATHER, ROOSTER, FLIPPERS))
        ...
        OverworldLocation("Above Key Cavern").add(Seashell(0x0A5)).connect(dungeon3_entrance, SHOVEL)  # above lv3
        dungeon3_entrance.connect(ukuku_prairie, None, one_way=True) # jump down ledge back to ukuku_prairie

        prairie_island_seashell = VirtualLocation().add(Seashell(0x0A6)).connect(ukuku_prairie, AND(FLIPPERS, r.bush))  # next to lv3
        VirtualLocation().add(Seashell(0x08B)).connect(ukuku_prairie, r.bush)  # next to seashell house
        VirtualLocation().add(Seashell(0x0A4)).connect(ukuku_prairie, PEGASUS_BOOTS)  # smash into tree next to phonehouse
        self._addEntrance("castle_jump_cave", ukuku_prairie, VirtualLocation().add(Chest(0x1FD)), OR(AND(FEATHER, PEGASUS_BOOTS), ROOSTER)) # left of the castle, 5 holes turned into 3
        VirtualLocation().add(Seashell(0x0B9)).connect(ukuku_prairie, POWER_BRACELET)  # under the rock

        left_bay_area = OverworldLocation("Western Martha's Bay")
        ...
        OverworldLocation("Lone Bush (Martha's Bay)").add(Seashell(0x0E9)).connect(left_bay_area, r.bush)  # same screen as mermaid statue
        tiny_island = OverworldLocation("Tiny Island").add(Seashell(0x0F8)).connect(left_bay_area, AND(OR(FLIPPERS, ROOSTER), r.bush))  # tiny island

        prairie_plateau = OverworldLocation("Donut Plains Plateau")  # prairie plateau at the owl statue
        if options.owlstatues == "both" or options.owlstatues == "overworld":
            prairie_plateau.add(OwlStatue(0x0A8))
        VirtualLocation().add(Seashell(0x0A8)).connect(prairie_plateau, SHOVEL)  # at the owl statue

        prairie_cave = IndoorLocation("Prairie Cave")
        prairie_cave_secret_exit = VirtualLocation().connect(prairie_cave, AND(BOMB, OR(FEATHER, ROOSTER)))
        ...
        bay_madbatter_connector_entrance = IndoorLocation("Martha's Bay Mad Batter Connector Entrance")
        bay_madbatter_connector_exit = IndoorLocation("Martha's Bay Mad Batter Connector Exit").connect(bay_madbatter_connector_entrance, FLIPPERS)
        bay_madbatter_connector_outside = OverworldLocation("Outside Martha's Bay Mad Batter Connector")
        bay_madbatter = IndoorLocation("Martha's Bay Batter").connect(VirtualLocation().add(MadBatter(0x1E0)), MAGIC_POWDER)
        ...
        seashell_mansion = IndoorLocation("Seashell Mansion")
        if options.goal != "seashells":
            VirtualLocation().add(SeashellMansion(0x2E9)).connect(seashell_mansion, COUNT(SEASHELL, 20))
        else:
            seashell_mansion.add(DroppedKey(0x2E9))
        self._addEntrance("seashell_mansion", ukuku_prairie, seashell_mansion, None)

        bay_water = OverworldLocation("Martha's Bay")
        bay_water.connect(ukuku_prairie, FLIPPERS)
        bay_water.connect(left_bay_area, FLIPPERS)
        # Technically kinda "indoors" but has no flagged connector
        fisher_under_bridge = OverworldLocation("Fisherman Under the Bridge").add(TradeSequenceItem(0x2F5, TRADING_ITEM_NECKLACE))
        fisher_under_bridge.connect(bay_water, AND(TRADING_ITEM_FISHING_HOOK, FEATHER, FLIPPERS))
        bay_water.connect(VirtualLocation().add(TradeSequenceItem(0x0C9, TRADING_ITEM_SCALE)), AND(TRADING_ITEM_NECKLACE, FLIPPERS))
        d5_entrance = OverworldLocation("Catfish's Maw Entrance").connect(bay_water, FLIPPERS)
        self._addEntrance("d5", d5_entrance, None, None)

        # Richard
        richard_house = IndoorLocation("Richard's House")
        richard_cave = IndoorLocation("Richard's Cave").connect(richard_house, COUNT(GOLD_LEAF, 5))
        richard_cave.connect(richard_house, None, one_way=True) # can exit richard's cave even without leaves
        richard_cave_chest = VirtualLocation().add(Chest(0x2C8)).connect(richard_cave, OR(FEATHER, HOOKSHOT, ROOSTER))
        richard_maze = OverworldLocation("Richard's Maze")
        ...
        if options.owlstatues == "both" or options.owlstatues == "overworld":
            VirtualLocation().add(OwlStatue(0x0C6)).connect(richard_maze, r.bush)
        VirtualLocation().add(SlimeKey()).connect(richard_maze, AND(r.bush, SHOVEL))

        next_to_castle = OverworldLocation("Next to Kanalet Castle")
        ...
        castle_secret_entrance_left = IndoorLocation("Castle Secret Entrance Left")
        castle_secret_entrance_right = IndoorLocation("Castle Secret Entrance Right").connect(castle_secret_entrance_left, FEATHER)
        castle_courtyard = OverworldLocation("Kanalet Castle Courtyard")
        castle_frontdoor = OverworldLocation("Kanalet Castle Front Door").connect(castle_courtyard, r.bush)
        ...
        VirtualLocation().add(HeartPiece(0x078)).connect(bay_water, FLIPPERS)  # in the moat of the castle
        castle_inside = IndoorLocation("Kanalet Castle")
        OverworldLocation("In the Castle Moat").add(KeyLocation("CASTLE_BUTTON")).connect(castle_inside, None)
        castle_top_outside = OverworldLocation("Atop Kanalet Castle")
        castle_top_inside = IndoorLocation("Kanalet Castle Spire")
        ...
        VirtualLocation().add(GoldLeaf(0x05A)).connect(castle_courtyard, OR(SWORD, BOW, MAGIC_ROD))  # mad bomber, enemy hiding in the 6 holes
        crow_gold_leaf = VirtualLocation().add(GoldLeaf(0x058)).connect(castle_courtyard, AND(POWER_BRACELET, r.attack_hookshot_no_bomb))  # bird on tree, can't kill with bomb cause it flies off. immune to magic_powder
        VirtualLocation().add(GoldLeaf(0x2D2)).connect(castle_inside, r.attack_hookshot_powder)  # in the castle, kill enemies
        VirtualLocation().add(GoldLeaf(0x2C5)).connect(castle_inside, AND(BOMB, r.attack_hookshot_powder))  # in the castle, bomb wall to show enemy
        kanalet_chain_trooper = VirtualLocation().add(GoldLeaf(0x2C6))  # in the castle, spinning spikeball enemy
        castle_top_inside.connect(kanalet_chain_trooper, AND(POWER_BRACELET, r.attack_hookshot), one_way=True)

        animal_village = OverworldLocation("Animal Village")
        animal_village.connect(VirtualLocation().add(TradeSequenceItem(0x0CD, TRADING_ITEM_FISHING_HOOK)), TRADING_ITEM_BROOM)
        cookhouse = IndoorLocation("Cook's House")
        cookhouse.connect(VirtualLocation().add(TradeSequenceItem(0x2D7, TRADING_ITEM_PINEAPPLE)), TRADING_ITEM_HONEYCOMB)
        goathouse = IndoorLocation("Goat's House")
        goathouse.connect(VirtualLocation().add(TradeSequenceItem(0x2D9, TRADING_ITEM_LETTER)), TRADING_ITEM_HIBISCUS)
        mermaid_statue = VirtualLocation()
        ...
        animal_village_connector_left = IndoorLocation("Animal Village Connector Left")
        animal_village_connector_right = IndoorLocation("Animal Village Connector Right").connect(animal_village_connector_left, PEGASUS_BOOTS)
        ...
        VirtualLocation().add(Seashell(0x0DA)).connect(animal_village, SHOVEL)  # owl statue at the water
        desert = OverworldLocation("Yarna Desert").connect(animal_village, r.bush)  # Note: We moved the walrus blocking the desert.
        if options.owlstatues == "both" or options.owlstatues == "overworld":
            desert.add(OwlStatue(0x0CF))
        desert_lanmola = VirtualLocation().add(AnglerKey()).connect(desert, OR(BOW, SWORD, HOOKSHOT, MAGIC_ROD, BOOMERANG))

        animal_village_bombcave = IndoorLocation("Animal Village Bomb Cave")
        ...
        animal_village_bombcave_heartpiece = VirtualLocation().add(HeartPiece(0x2E6)).connect(animal_village_bombcave, OR(AND(BOMB, FEATHER, HOOKSHOT), ROOSTER))  # cave in the upper right of animal town

        desert_cave = IndoorLocation("Lanmolas' Lair")
        self._addEntrance("desert_cave", desert, desert_cave, None)
        desert.connect(desert_cave, None, one_way=True) # Drop down the sinkhole

        VirtualLocation().add(HeartPiece(0x1E8)).connect(desert_cave, BOMB)  # above the quicksand cave
        VirtualLocation().add(Seashell(0x0FF)).connect(desert, POWER_BRACELET) # bottom right corner of the map

        armos_maze = OverworldLocation("Armos Maze").connect(animal_village, POWER_BRACELET)
        armos_temple = IndoorLocation("Armos Temple")
        VirtualLocation().add(FaceKey()).connect(armos_temple, r.miniboss_requirements[world_setup.miniboss_mapping["armos_temple"]])
        if options.owlstatues == "both" or options.owlstatues == "overworld":
            armos_maze.add(OwlStatue(0x08F))
        self._addEntrance("armos_maze_cave", armos_maze, IndoorLocation("Under Armos Cave").add(Chest(0x2FC)), None)
        self._addEntrance("armos_temple", armos_maze, armos_temple, None)

        armos_fairy_entrance = OverworldLocation("Armos Fairy Entrance").connect(bay_water, FLIPPERS).connect(animal_village, POWER_BRACELET)
        ...
        d6_connector_left = IndoorLocation("Face Shrine Connector Left")
        d6_connector_right = IndoorLocation("Face Shrine Connector Right").connect(d6_connector_left, OR(AND(HOOKSHOT, OR(FLIPPERS, AND(FEATHER, PEGASUS_BOOTS))), ROOSTER))
        d6_entrance = OverworldLocation("Face Shrine Entrance")
        d6_entrance.connect(bay_water, FLIPPERS, one_way=True)
        d6_armos_island = OverworldLocation("Armos Island").connect(bay_water, FLIPPERS)
        ...
        windfish_egg = OverworldLocation("Windfish's Egg").connect(swamp, POWER_BRACELET).connect(graveyard, POWER_BRACELET)
        windfish_egg.connect(graveyard, None, one_way=True) # Ledge jump

        obstacle_cave_entrance = OverworldLocation("Obstacle Cave Entrance")
        obstacle_cave_inside = IndoorLocation("Obstacle Cave").connect(obstacle_cave_entrance, SWORD)
        ...
        obstacle_cave_inside_chest = VirtualLocation().add(Chest(0x2BB)).connect(obstacle_cave_inside, OR(HOOKSHOT, ROOSTER))  # chest at obstacles
        obstacle_cave_exit = IndoorLocation("Obstacle Cave Exit").connect(obstacle_cave_inside, OR(PEGASUS_BOOTS, ROOSTER))

        lower_right_taltal = OverworldLocation("Lower Right Tal Tal")
        self._addEntrance("obstacle_cave_entrance", windfish_egg, obstacle_cave_entrance, POWER_BRACELET)
        self._addEntrance("obstacle_cave_outside_chest", VirtualLocation().add(Chest(0x018)), obstacle_cave_inside, None)
        self._addEntrance("obstacle_cave_exit", lower_right_taltal, obstacle_cave_exit, None)

        papahl_cave = IndoorLocation("Papahl Cave").add(Chest(0x28A))
        papahl = OverworldLocation("Papahl").connect(lower_right_taltal, None, one_way=True)
        hibiscus_item = VirtualLocation().add(TradeSequenceItem(0x019, TRADING_ITEM_HIBISCUS))
        ...
        below_right_taltal = OverworldLocation("Tal Tal Heights").connect(windfish_egg, POWER_BRACELET)
        ...
        heartpiece_swim_cave = IndoorLocation("Damp Cave").connect(VirtualLocation().add(HeartPiece(0x1F2)), FLIPPERS)
        self._addEntrance("heartpiece_swim_cave", below_right_taltal, heartpiece_swim_cave, FLIPPERS)  # cave next to level 4
        d4_entrance = OverworldLocation("Angler's Tunnel Entrance").connect(below_right_taltal, FLIPPERS)
        ...
        mambo = IndoorLocation("Mambo's Cave").connect(VirtualLocation().add(Song(0x2FD)), AND(OCARINA, FLIPPERS))  # Manbo's Mambo
        self._addEntrance("mambo", d4_entrance, mambo, FLIPPERS) 

        # Raft game.
        raft_house = IndoorLocation("Raft House")
        VirtualLocation().add(KeyLocation("RAFT")).connect(raft_house, COUNT("RUPEES", 100))
        raft_return_upper = IndoorLocation("Raft Return Upper")
        raft_return_lower = IndoorLocation("Raft Return Lower").connect(raft_return_upper, None, one_way=True)
        outside_raft_house = OverworldLocation("Outside Raft House").connect(below_right_taltal, HOOKSHOT).connect(below_right_taltal, FLIPPERS, one_way=True)
        raft_game = OverworldLocation("Raft Game")
        raft_game.connect(outside_raft_house, "RAFT")
        raft_game.add(Chest(0x05C), Chest(0x05D)) # Chests in the rafting game
        raft_exit = OverworldLocation("Raft Game Exit")
        ...
        outside_rooster_house = OverworldLocation("Outside Rooster House").connect(lower_right_taltal, OR(FLIPPERS, ROOSTER))
        self._addEntrance("rooster_house", outside_rooster_house, None, None)
        bird_cave = IndoorLocation("Bird Cave")
        bird_key = VirtualLocation().add(BirdKey())
        ...
        bridge_seashell = VirtualLocation().add(Seashell(0x00C)).connect(outside_rooster_house, AND(OR(FEATHER, ROOSTER), POWER_BRACELET))  # seashell right of rooster house, there is a hole in the bridge
        ...
        right_taltal_connector1 = IndoorLocation("Eastern Tal Tal Connector 1")
        right_taltal_connector_outside1 = OverworldLocation("Eastern Tal Tal Connector Outside 1")
        right_taltal_connector2 = IndoorLocation("Eastern Tal Tal Connector 2")
        right_taltal_connector3 = IndoorLocation("Eastern Tal Tal Connector 3")
        ...
        mountain_bridge_staircase = OverworldLocation("Tal Tal Mountain Bridge Staircase").connect(outside_rooster_house, OR(HOOKSHOT, ROOSTER)) # cross bridges to staircase
        if options.logic != "casual":  # ledge drop
            mountain_bridge_staircase.connect(windfish_egg, None, one_way=True)

        left_right_connector_cave_entrance = IndoorLocation("Tal Tal Left Right Connector Cave Entrance")
        left_right_connector_cave_exit = IndoorLocation("Tal Tal Left Right Connector Cave Entrance")
        ...
        taltal_boulder_zone = OverworldLocation("Tal Tal Cabbage Zone")
        ...
        mountain_heartpiece = VirtualLocation().add(HeartPiece(0x2BA)) # heartpiece in connecting cave
        ...
        taltal_madbatter = IndoorLocation("Tal Tal Mad Batter").connect(VirtualLocation().add(MadBatter(0x1E2)), MAGIC_POWDER)
        ...
        outside_fire_cave = OverworldLocation("Outside Fire Cave")
        if options.logic != "casual":
        ...
        fire_cave_bottom = IndoorLocation("Fire Cave Bottom")
        fire_cave_top = IndoorLocation("Fire Cave Top").connect(fire_cave_bottom, COUNT(SHIELD, 2))
        ...
        d8_entrance = OverworldLocation("Turtle Rock Entrance")
        if options.logic != "casual":
        ...
        if options.logic == 'glitched' or options.logic == 'hell':
            ...
            ukuku_prairie.connect(richard_maze, OR(SWORD, AND(MAGIC_POWDER, MAX_POWDER_UPGRADE), MAGIC_ROD, BOOMERANG, BOMB)) # break bushes on north side of the maze, and 1 pit buffer into the maze. Do the same in one of the two northern screens of the maze to escape            fisher_under_bridge.connect(bay_water, AND(BOMB, FLIPPERS)) # can bomb trigger the item without having the hook 
