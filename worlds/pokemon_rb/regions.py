...
def create_region(world, name: str, locations_per_region=None, exits=None):
    ret = PokemonRBRegion(name, world.player, world.multiworld)
    for location in locations_per_region.get(name, []):
        location.parent_region = ret
        ret.locations.append(location)
        if world.options.randomize_hidden_items == "exclude" and "Hidden" in location.name:
            location.progress_type = LocationProgressType.EXCLUDED
    if exits:
        for exit in exits:
            ret.exits.append(Entrance(world.player, exit, ret))
...
def create_regions(world):
    ...
    for location in location_data:
        ...
        if location.inclusion(world) and (isinstance(location.original_item, list) or
            ...
            if location.type in ("Item", "Trainer Parties"):
                ...
                if world.options.key_items_only and (not location.event) and (not item.advancement) and location.original_item != "Exp. All":
                    continue
    ...
    regions = [create_region(world, region, locations_per_region) for region in warp_data]
    ...
    connect(multiworld, player, "Route 23-Grass", "Route 23-S", lambda state: logic.can_surf(state, world))
    ...
    connect(multiworld, player, "Route 2-SW", "Route 2-SE", lambda state: logic.can_cut(state, world))
    connect(multiworld, player, "Route 2-NW", "Route 2-NE", lambda state: logic.can_cut(state, world))
    connect(multiworld, player, "Route 2-E", "Route 2-NE", lambda state: logic.can_cut(state, world))
    ...
    connect(multiworld, player, "Pewter City", "Pewter City-M", lambda state: logic.can_cut(state, world), one_way=True)
    connect(multiworld, player, "Pewter City-E", "Route 3", lambda state: logic.route_3(state, world), one_way=True)
    connect(multiworld, player, "Route 3", "Pewter City-E", one_way=True)
    connect(multiworld, player, "Route 4-W", "Route 3")
    connect(multiworld, player, "Route 24", "Cerulean City-Water", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Cerulean City-Water", "Route 4-Lass", lambda state: logic.can_surf(state, world), one_way=True)
    ...
    connect(multiworld, player, "Cerulean City", "Cerulean City-Outskirts", lambda state: logic.can_cut(state, world), one_way=True)
    connect(multiworld, player, "Cerulean City-Outskirts", "Route 9", lambda state: logic.can_cut(state, world))
    connect(multiworld, player, "Cerulean City-Outskirts", "Route 5")
    connect(multiworld, player, "Cerulean Cave B1F", "Cerulean Cave B1F-E", lambda state: logic.can_surf(state, world), one_way=True)
    connect(multiworld, player, "Route 24", "Route 25")
    connect(multiworld, player, "Route 9", "Route 10-N")
    connect(multiworld, player, "Route 10-N", "Route 10-C", lambda state: logic.can_surf(state, world))
    ...
    connect(multiworld, player, "Route 5 Gate-N", "Route 5 Gate-S", lambda state: logic.can_pass_guards(state, world))
    connect(multiworld, player, "Route 6 Gate-N", "Route 6 Gate-S", lambda state: logic.can_pass_guards(state, world))
    connect(multiworld, player, "Route 7 Gate-W", "Route 7 Gate-E", lambda state: logic.can_pass_guards(state, world))
    connect(multiworld, player, "Route 8 Gate-W", "Route 8 Gate-E", lambda state: logic.can_pass_guards(state, world))
    ...
    connect(multiworld, player, "Vermilion City", "Vermilion City-G", lambda state: logic.can_surf(state, world) or logic.can_cut(state, world))
    ...
    connect(multiworld, player, "Route 12-N", "Route 12-S", lambda state: logic.can_surf(state, world))
    ...
    connect(multiworld, player, "Route 12-S", "Route 12-Grass", lambda state: logic.can_cut(state, world), one_way=True)
    ...
    connect(multiworld, player, "Route 8", "Route 8-Grass", lambda state: logic.can_cut(state, world), one_way=True)
    connect(multiworld, player, "Route 7", "Celadon City")
    connect(multiworld, player, "Celadon City", "Celadon City-G", lambda state: logic.can_cut(state, world))
    ...
    connect(multiworld, player, "Route 16-E", "Route 16-NE", lambda state: logic.can_cut(state, world))
    ...
    connect(multiworld, player, "Route 14", "Route 15-N", lambda state: logic.can_cut(state, world), one_way=True)
    connect(multiworld, player, "Route 14", "Route 14-Grass", lambda state: logic.can_cut(state, world), one_way=True)
    connect(multiworld, player, "Route 13", "Route 13-Grass", lambda state: logic.can_cut(state, world), one_way=True)
    connect(multiworld, player, "Route 14", "Route 13")
    connect(multiworld, player, "Route 13", "Route 13-E", lambda state: logic.can_strength(state, world) or logic.can_surf(state, world) or not world.options.extra_strength_boulders.value)
    connect(multiworld, player, "Route 12-S", "Route 13-E")
    connect(multiworld, player, "Fuchsia City", "Route 19-N")
    connect(multiworld, player, "Route 19-N", "Route 19-S", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Route 20-E", "Route 20-IW", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Route 20-E", "Route 19-S")
    connect(multiworld, player, "Route 20-W", "Cinnabar Island", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Route 20-IE", "Route 20-W", lambda state: logic.can_surf(state, world))
    ...
    connect(multiworld, player, "Safari Zone West-NW", "Safari Zone West", lambda state: logic.can_surf(state, world))
    ...
    connect(multiworld, player, "Safari Zone Center-NW", "Safari Zone Center-C", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Safari Zone Center-NE", "Safari Zone Center-C", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Safari Zone Center-S", "Safari Zone Center-C", lambda state: logic.can_surf(state, world))
    ...
    connect(multiworld, player, "Victory Road 3F-S", "Victory Road 3F", lambda state: logic.can_strength(state, world))
    connect(multiworld, player, "Victory Road 3F-SE", "Victory Road 3F-S", lambda state: logic.can_strength(state, world), one_way=True)
    ...
    connect(multiworld, player, "Victory Road 2F-W", "Victory Road 2F-C", lambda state: logic.can_strength(state, world), one_way=True)
    connect(multiworld, player, "Victory Road 2F-NW", "Victory Road 2F-W", lambda state: logic.can_strength(state, world), one_way=True)
    connect(multiworld, player, "Victory Road 2F-C", "Victory Road 2F-SE", lambda state: logic.can_strength(state, world) and state.has("Victory Road Boulder", player), one_way=True)
    connect(multiworld, player, "Victory Road 1F-S", "Victory Road 1F", lambda state: logic.can_strength(state, world))
    ...
    connect(multiworld, player, "Seafoam Islands B4F", "Seafoam Islands B4F-W", lambda state: logic.can_surf(state, world), one_way=True)
    connect(multiworld, player, "Seafoam Islands B4F-W", "Seafoam Islands B4F", one_way=True)
    connect(multiworld, player, "Seafoam Islands B3F", "Seafoam Islands B3F-SE", lambda state: logic.can_surf(state, world) and logic.can_strength(state, world) and state.has("Seafoam Exit Boulder", player, 6))
    connect(multiworld, player, "Viridian City", "Viridian City-N", lambda state: state.has("Oak's Parcel", player) or world.options.old_man.value == 2 or logic.can_cut(state, world))
    connect(multiworld, player, "Route 11", "Route 11-C", lambda state: logic.can_strength(state, world) or not world.options.extra_strength_boulders)
    ...
    connect(multiworld, player, "Route 21", "Cinnabar Island", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Pallet Town", "Route 21", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Celadon Gym", "Celadon Gym-C", lambda state: logic.can_cut(state, world), one_way=True)
    ...
    connect(multiworld, player, "Rock Tunnel 1F-S 1", "Rock Tunnel 1F-S", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel 1F-S 2", "Rock Tunnel 1F-S", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel 1F-NW 1", "Rock Tunnel 1F-NW", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel 1F-NW 2", "Rock Tunnel 1F-NW", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel 1F-NE 1", "Rock Tunnel 1F-NE", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel 1F-NE 2", "Rock Tunnel 1F-NE", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel B1F-W 1", "Rock Tunnel B1F-W", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel B1F-W 2", "Rock Tunnel B1F-W", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel B1F-E 1", "Rock Tunnel B1F-E", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel B1F-E 2", "Rock Tunnel B1F-E", lambda state: logic.rock_tunnel(state, world))
    connect(multiworld, player, "Rock Tunnel 1F-S", "Rock Tunnel 1F-Wild", lambda state: logic.rock_tunnel(state, world), one_way=True)
    connect(multiworld, player, "Rock Tunnel 1F-NW", "Rock Tunnel 1F-Wild", lambda state: logic.rock_tunnel(state, world), one_way=True)
    connect(multiworld, player, "Rock Tunnel 1F-NE", "Rock Tunnel 1F-Wild", lambda state: logic.rock_tunnel(state, world), one_way=True)
    connect(multiworld, player, "Rock Tunnel B1F-W", "Rock Tunnel B1F-Wild", lambda state: logic.rock_tunnel(state, world), one_way=True)
    connect(multiworld, player, "Rock Tunnel B1F-E", "Rock Tunnel B1F-Wild", lambda state: logic.rock_tunnel(state, world), one_way=True)
    ...
    connect(multiworld, player, "Cerulean Cave 1F-SE", "Cerulean Cave 1F-Water", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Cerulean Cave 1F-SW", "Cerulean Cave 1F-Water", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Cerulean Cave 1F-N", "Cerulean Cave 1F-Water", lambda state: logic.can_surf(state, world))
    connect(multiworld, player, "Cerulean Cave 1F-NE", "Cerulean Cave 1F-Water", lambda state: logic.can_surf(state, world))
    ...
    connect(multiworld, player, "Cerulean City-Water", "Cerulean City-Cave", lambda state:
        ...
        logic.has_key_items(state, world.options.cerulean_cave_key_items_condition.total, player) and logic.can_surf(state, world))

    # access to any part of a city will enable flying to the Pokemon Center
    connect(multiworld, player, "Cerulean City-Cave", "Cerulean City", lambda state: logic.can_fly(state, world), one_way=True)
    connect(multiworld, player, "Cerulean City-Badge House Backyard", "Cerulean City", lambda state: logic.can_fly(state, world), one_way=True)
    connect(multiworld, player, "Cerulean City-T", "Cerulean City", lambda state: logic.can_fly(state, world), one_way=True, name="Cerulean City-T to Cerulean City (Fly)")
    connect(multiworld, player, "Fuchsia City-Good Rod House Backyard", "Fuchsia City", lambda state: logic.can_fly(state, world), one_way=True)
    connect(multiworld, player, "Saffron City-G", "Saffron City", lambda state: logic.can_fly(state, world), one_way=True, name="Saffron City-G to Saffron City (Fly)")
    connect(multiworld, player, "Saffron City-Pidgey", "Saffron City", lambda state: logic.can_fly(state, world), one_way=True, name="Saffron City-Pidgey to Saffron City (Fly)")
    connect(multiworld, player, "Saffron City-Silph", "Saffron City", lambda state: logic.can_fly(state, world), one_way=True, name="Saffron City-Silph to Saffron City (Fly)")
    connect(multiworld, player, "Saffron City-Copycat", "Saffron City", lambda state: logic.can_fly(state, world), one_way=True, name="Saffron City-Copycat to Saffron City (Fly)")
    connect(multiworld, player, "Celadon City-G", "Celadon City", lambda state: logic.can_fly(state, world), one_way=True, name="Celadon City-G to Celadon City (Fly)")
    connect(multiworld, player, "Vermilion City-G", "Vermilion City", lambda state: logic.can_fly(state, world), one_way=True, name="Vermilion City-G to Vermilion City (Fly)")
    connect(multiworld, player, "Vermilion City-Dock", "Vermilion City", lambda state: logic.can_fly(state, world), one_way=True, name="Vermilion City-Dock to Vermilion City (Fly)")
    connect(multiworld, player, "Cinnabar Island-G", "Cinnabar Island", lambda state: logic.can_fly(state, world), one_way=True, name="Cinnabar Island-G to Cinnabar Island (Fly)")
    connect(multiworld, player, "Cinnabar Island-M", "Cinnabar Island", lambda state: logic.can_fly(state, world), one_way=True, name="Cinnabar Island-M to Cinnabar Island (Fly)")
    ...
    connect(multiworld, player, "Seafoam Islands B2F-NW", "Seafoam Islands B3F", lambda state: logic.can_strength(state, world) and state.has("Seafoam Exit Boulder", player, 6), one_way=True)
    connect(multiworld, player, "Seafoam Islands B2F-NE", "Seafoam Islands B3F", lambda state: logic.can_strength(state, world) and state.has("Seafoam Exit Boulder", player, 6), one_way=True)
    connect(multiworld, player, "Seafoam Islands B2F-NW", "Seafoam Islands B3F-SE", lambda state: logic.can_strength(state, world) and state.has("Seafoam Exit Boulder", player, 6), one_way=True)
    connect(multiworld, player, "Seafoam Islands B2F-NE", "Seafoam Islands B3F-SE", lambda state: logic.can_strength(state, world) and state.has("Seafoam Exit Boulder", player, 6), one_way=True)
    ...
    connect(multiworld, player, "Seafoam Islands B3F", "Seafoam Islands B4F-W", lambda state: logic.can_surf(state, world), one_way=True)
    ...
    if world.fly_map != "Pallet Town":
        connect(multiworld, player, "Menu", world.fly_map,
                lambda state: logic.can_fly(state, world), one_way=True, name="Free Fly Location")

    if world.town_map_fly_map != "Pallet Town":
        connect(multiworld, player, "Menu", world.town_map_fly_map,
                lambda state: logic.can_fly(state, world) and state.has("Town Map", player), one_way=True,
