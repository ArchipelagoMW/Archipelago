...
def set_rules(world, player):
    ...
    access_rules = {
        ...
        "Indigo Plateau Champion's Room - Become Champion": lambda state: state.pokemon_rb_has_badges(state.multiworld.elite_four_condition[player], player) and state.has_all(item_groups["Key Items"], player),
        ...
        **{f"Pokemon Tower {floor}F - Wild Pokemon - {slot}": lambda state: state.has("Silph Scope", player) for floor in range(3, 8) for slot in range(1, 11)},
        ...
        "Underground Tunnel Route 5 - Spot Trade": lambda state: state.can_reach("Route 24 - Wild Pokemon - 6", "Location", player),
