...
class FaxanaduWorld(World):
    ...
    def prefill_shop_red_potions(self) -> int:
        ...
        if self.options.keep_shop_red_potions.value:
            red_potion_item = next(item for item in Items.items if item.name == "Red Potion")
    ...
    def put_wingboot_in_shop(self, shops, region_name):
        ...
        set_rule(location, lambda state: state.has("Unlock Wingboots", self.player, 1))
    ...
    def prefill_shop_wingboots(self) -> int:
        ...
        for loc in Locations.locations:
            if loc.type == Locations.LocationType.shop:
                if self.options.keep_shop_red_potions.value and loc.original_item == Locations.ItemType.red_potion:
                    continue # Don't override our red potions
                if loc.region not in shops:
                    shops[loc.region] = []
                shops[loc.region].append(loc)
    ...
    def create_items(self):
        ...
        for item in Items.items:
            ...
            if item.name == "Pendant" and not self.options.include_pendant.value:
                continue
            ...
            if item.prefill_location:
                prefill_loc = self.multiworld.get_location(item.prefill_location, self.player)
            ...
            if self.options.require_dragon_slayer.value and item.name == "Progressive Shield":
        ...
        if not self.options.include_poisons.value:
            self.filler_ratios["Poison"] = 0

        # Randomly add fillers to the pool with ratios based on og game occurence counts.
