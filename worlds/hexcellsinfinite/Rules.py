...
def set_rules(world: "HexcellsInfiniteWorld"):
    ...
    if world.options.LevelUnlockType == Options.LevelUnlockType.option_vanilla :
        add_rule(world.get_entrance("Level Group 1 -> Level Group 2", player), lambda state: state.has("Gem", player, 6))
        add_rule(world.get_entrance("Level Group 2 -> Level Group 3", player), lambda state: state.has("Gem", player, 12))
        add_rule(world.get_entrance("Level Group 3 -> Level Group 4", player), lambda state: state.has("Gem", player, 18))
        add_rule(world.get_entrance("Level Group 4 -> Level Group 5", player), lambda state: state.has("Gem", player, 24))
        add_rule(world.get_entrance("Level Group 5 -> Level Group 6", player), lambda state: state.has("Gem", player, 30))
    
        world.completion_condition[player] = lambda state: state.has("Gem", player, 36)
    else:
        ...
        world.completion_condition[player] = lambda state: state.has_all(HEXCELLS_LEVEL_ITEMS,player)
