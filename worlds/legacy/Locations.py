...
additional_location_table = {
    # Fairy Chests
    **{f"Castle Fairy Chest {i + 1}": counter.next() for i in range(0, Constants.FAIRY_CHEST_MAXIMUM_PER_ZONE)},
    **{f"Garden Fairy Chest {i + 1}": counter.next() for i in range(0, Constants.FAIRY_CHEST_MAXIMUM_PER_ZONE)},
    **{f"Tower Fairy Chest {i + 1}": counter.next() for i in range(0, Constants.FAIRY_CHEST_MAXIMUM_PER_ZONE)},
    **{f"Dungeon Fairy Chest {i + 1}": counter.next() for i in range(0, Constants.FAIRY_CHEST_MAXIMUM_PER_ZONE)},

    # Non-Fairy Chests
    **{f"Castle Chest {i + 1}": counter.next() for i in range(0, Constants.CHEST_MAXIMUM_PER_ZONE)},
    **{f"Garden Chest {i + 1}": counter.next() for i in range(0, Constants.CHEST_MAXIMUM_PER_ZONE)},
    **{f"Tower Chest {i + 1}": counter.next() for i in range(0, Constants.CHEST_MAXIMUM_PER_ZONE)},
    **{f"Dungeon Chest {i + 1}": counter.next() for i in range(0, Constants.CHEST_MAXIMUM_PER_ZONE)},
}
