...
class LocalRom:
    def __init__(self, rom_bytes, name=None):
        self.name = name
    ...
    def apply_placements(self, placements, player_name):
        """Replays the seed's item placements onto the (already base-patched) ROM."""
        from .Locations import location_data_table
        from .Items import items_by_id, ItemData, ItemType

        for record in placements:
            location_data = location_data_table[record["location"]]
            if record["external"]:
                item = ItemData(record["item_id"], record["item_name"],
                                ItemClassification.progression, ItemType.External)
                item = item._replace(recipient=record["recipient"])
            else:
                item = items_by_id[record["item_id"]]

            self.replace_item(location_data, item)
            if location_data.inject_name:
                self.insert_hint_text(location_data, record["hint_short"], record["hint_long"])

        self.inject_name(player_name)
        self.write_changed_rom()


class MMBN3PatchExtension(APPatchExtension):
    game = "MegaMan Battle Network 3"

    @staticmethod
    def apply_bn3_base_patch(caller: "MMBN3ProcedurePatch", rom: bytes) -> bytes:
        """Applies the static bn3-ap-patch bsdiff (bundled in the apworld) to the base ROM."""
        patch_bytes = pkgutil.get_data(__name__, "data/bn3-ap-patch.bsdiff")
        return bsdiff4.patch(bytes(rom), patch_bytes)

    @staticmethod
    def apply_bn3_placements(caller: "MMBN3ProcedurePatch", rom: bytes, placements_file: str) -> bytes:
        """Replays the seed's placements onto the base-patched ROM."""
        placements = json.loads(caller.get_file(placements_file).decode("utf-8"))
        local_rom = LocalRom(rom)
        local_rom.apply_placements(placements["locations"], placements["player_name"])
        return bytes(local_rom.rom_data)


class MMBN3ProcedurePatch(APProcedurePatch):
    hash = CHECKSUM_BLUE
    game = "MegaMan Battle Network 3"
    ...
    procedure = [
        ("apply_bn3_base_patch", []),
        ("apply_bn3_placements", ["placements.json"]),
    ]
