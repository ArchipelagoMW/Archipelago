...
Se pueden utilizar otras compilaciones basadas en el PC Port de SM64, pero no se puede utilizar un launcher diferente
con la versión Archipelago de sm64ex.

## Pasos para su instalación e iniciar del juego
...
El launcher necesita `qt6`, `patch` y `git`, y para compilar el juego necesitas `sdl2 glew cmake python make`
(Si instalas `jsoncpp` también, se enlazará dinámicamente).
Luego continúa con `Usando el Launcher`.

*Usando el launcher*
1. Ve a la página vinculada a SM64AP-Launcher y selecciona la última versión.
...
seguido de `Check Requirements`, que te guiará por el resto de pasos necesarios.
    - Windows: Si no has utilizado el directorio de instalación por defecto para MSYS, cierra esta ventana, marca 
    `Show advanced options` y vuelve a abrirla utilizando `Re-check Requirements`. A continuación podrás establecer la
    ruta manualmente.
5. Cuando termine, haz click en el botón `Compile default SM64AP build` para continuar.
    ...
    Dibujo, etc. o aplicar parches del juego como 60FPS, Movimientos Mejorados, etc., usa la opción `Compile custom build`:
      - Ponle un nombre a tu build, por ejemplo «archipiélago» o el que quieras.Recomiendo nombrar la versión del juego
      ...
      - Para más detalles, puedes consultar en:
...
Cuando termine, la lista de compilación debería tener una nueva entrada con el nombre que le hayas dado.

NOTA: Si no se inicia al pulsar `Play selected build`, vuelve a comprobar si has escrito correctamente las opciones de 
ejecución (Descritas en `Unirse a partida MultiWorld`)
...
Copia tu ROM legalmente volcado en la carpeta sm64ex (si no estás seguro de dónde se encuentra la carpeta, haz una
...
Después de que la compilación haya tenido éxito, habrá una carpeta llamada 'bin' en la carpeta `sm64ex/build/REGION_pc/`.

### Unirse a una partida MultiWorld

Para unirse, configura las siguientes opciones de lanzamiento: `--sm64ap_name TuNombre --sm64ap_ip ServerIP:Port`.
...
Opcionalmente, añade `--sm64ap_passwd «TuContraseña»` si es que la sala que estás utilizando requiere una contraseña.
...
te mostrará un texto de estado.

### Jugar sin conexión

Para jugar sin conexión, primero genera una semilla en la página de opciones del juego.
Crea una sala y descarga el archivo `.apsm64ex`, e inicia el juego con el argumento de ejecución `--sm64ap_file "path/to/FileName"`.

### Opcional: Usar archivos por lotes para jugar partidas offline y MultiWorld

Como alternativa al ejecutar el juego con SM64AP-Launcher, también es posible ejecutar la versión completa utilizando 
...
Abre el Bloc de notas (o el editor de texto de tu conveniencia). Pega el siguiente texto:
`start sm64.us.f3dex2e.exe --sm64ap_file %1`

Ve a Archivo > Guardar como...

Ve a la carpeta que seleccionaste para la compilación de SM64 cuando seguiste la guía de compilación de SM64AP-Launcher
Una vez allí, navega hasta `build` y entra en `us_pc`. Esta carpeta debería ser la misma en la que se encuentra
`sm64.us.f3dex2e.exe`.
...
- El número de puerto aparece en la página de la sala. El anfitrión del juego debe compartir esta página con 
  todos los jugadores.
- El campo 'name' es el que hayas escrito en el campo "Name" al crear un archivo de configuración. Todos los
...
Esto ocurre cuando al juego le faltan los datos relevantes del randomizer. Si estás intentando conectarte a un 
