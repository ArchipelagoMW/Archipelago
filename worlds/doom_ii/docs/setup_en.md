...
2. If you have the Steam version of DOOM + DOOM II installed, Archipelago Doom can likely detect it and use the game files from it without needing any further work.
   If this is not the case, however, copy `DOOM2.WAD` from your game's installation directory into the same folder you extracted or installed Archipelago Doom into.

For the Steam version of DOOM + DOOM II, you can find the game files by finding the game in your Steam library, right-clicking it and choosing **Manage -> Browse Local Files**. The WAD file you need is in the `/base/` folder.
...
3. Choose "Select Game..." and then choose "DOOM II: Hell on Earth".
...
2. Run `apdoom-launcher.exe -game doom2 -apserver <server> -applayer <slot name>`, where:
...
- `!hint MAP02`: will hint the level unlock item for that level (`Underhalls (MAP02)`).
- `!hint MAP02 Map`: will hint the Computer area map item that corresponds to that level (`Underhalls (MAP02) - Computer area map`).
- `!hint MAP02 Blue` (or `Yellow`, `Red`): will hint the corresponding key for that level, if it exists (`Underhalls (MAP02) - Blue keycard`).
- `!hint MAP02 Keys`: will hint all keys for that level simultaneously.
