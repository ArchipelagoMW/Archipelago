...
def set_all_entrance_rules(world: NothingWorld) -> None:
    ...
    if world.options.goal > 1200 or world.options.giftcoins:
        if world.options.shop_upgrades:
            coinreq += 4
    ...
    if world.options.shop_upgrades:
        ...
        set_rule(Start_to_shopdigits, lambda state: (
                    (state.has("Nothing Item Progressive Time Cap", world.player,int(math.ceil(600/world.options.timecap_interval))) 
                    and state.has("Nothing Item Timer Digit", world.player,2)) or state.has("Nothing Item Gifted Coin", world.player, coinreq) or ((int(math.ceil(60/world.options.timecap_interval))) 
                    and state.has("Nothing Item Timer Digit", world.player,1)) or ((int(math.ceil(10/world.options.timecap_interval))) 
    ...
    if (world.options.giftcoins or world.options.goal > 1200): 
        if world.options.shop_upgrades:
            ...
            set_rule(Start_to_shopupgrades, lambda state: (
                    (state.has("Nothing Item Progressive Time Cap", world.player,int(math.ceil(1200/world.options.timecap_interval))) 
...
def set_all_location_rules(world: NothingWorld) -> None:
    ...
    for i in range (int(math.ceil(NothingWorld.options.goal/NothingWorld.options.milestone_interval))):
        if world.options.shop_upgrades:
            if world.options.milestone_interval*(i+1) > world.options.timecap_interval or (world.options.milestone_interval*(i+1) > 9 and world.options.shop_upgrades):
                if world.options.milestone_interval*(i+1) < 60:
                    set_rule(world.get_location("Nothing Milestone "+str(i+1)), lambda state: 
                             state.has("Nothing Item Progressive Time Cap",world.player,int(math.ceil(world.options.milestone_interval*(i+1)/world.options.timecap_interval))) 
                             and state.has("Nothing Item Timer Digit",world.player,1))
                elif world.options.milestone_interval*(i+1) < 600:
                    set_rule(world.get_location("Nothing Item Milestone "+str(i+1)), lambda state: 
                             state.has("Nothing Item Progressive Time Cap",world.player,int(math.ceil(world.options.milestone_interval*(i+1)/world.options.timecap_interval))) 
...
def set_completion_condition(world: NothingWorld) -> None:
    if world.options.shop_upgrades and world.options.goal > 9:
        if world.options.goal < 60:
            ...
            state.has("Nothing Item Progressive Time Cap", world.player, int(math.ceil(world.options.goal/world.options.timecap_interval))) and
        ...
        world.multiworld.completion_condition[world.player] = lambda state: (
            state.has("Nothing Item Progressive Time Cap", world.player, int(math.ceil(world.options.goal/world.options.timecap_interval))))
