...
- Fix KirbyAM player template rendering to omit empty `Item & Location Options` sections when all options in that group are hidden, and add regression checks for hidden goal choices plus required world-version output in generated YAML (Issue #442).
