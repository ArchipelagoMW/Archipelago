...
from .Constants import GAME_ABBR, GAME_NAME, GLITCHED_ITEM
...
icon_paths[GAME_NAME] = "ap:worlds.tww/assets/icon.png"
...
class TWWWorld(World):
    ...
    def create_regions(self) -> None:
        ...
        self.charts.setup_progress_sunken_treasure_locations(skip_randomization=self._charts_restored_from_ut)

        # Select required bosses.
        if options.required_bosses:
            self.boss_reqs.randomize_required_bosses(skip_randomization=self._required_bosses_restored_from_ut)
        ...
        self.entrances.randomize_entrances(skip_randomization=self._entrances_restored_from_ut)
            ...
            for i in range(len(rock_spire_shop_ship_locations)):
                ...
                add_item_rule(
                    ...
                    or (
                        item.game != GAME_NAME
                        and all(
                            location.item is None or location.item.game == GAME_NAME for location in locations
                        )
                    ),
                )
    ...
    def _format_datastorage_key(team: int, player: int, stage_name: str) -> str:
        ...
        """
        return f"{GAME_ABBR}_{team}_{player}_{stage_name}"
    ...
    def _parse_datastorage_key(key: str) -> DatastorageParsed | None:
        ...
        parts = key.split("_")
        min_parts = 4
        if len(parts) < min_parts or parts[0] != GAME_ABBR:
            return None
        try:
            team = int(parts[1])
            player = int(parts[2])
