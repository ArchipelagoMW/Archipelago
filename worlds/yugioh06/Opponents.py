...
def get_opponents(multiworld: Optional[MultiWorld], player: Optional[int]) -> Tuple[OpponentData, ...]:
    opponents_table: List[OpponentData] = [
        ...
        OpponentData("Vampire Genesis", ["Quick-Finish"], 3, 2, lambda state: state.yugioh06_difficulty(player, 3)),
        OpponentData("Infernal Flame Emperor", [], 3, 3, lambda state: state.yugioh06_difficulty(player, 3)),
        OpponentData("Ocean Dragon Lord - Neo-Daedalus", [], 3, 4, lambda state: state.yugioh06_difficulty(player, 3)),
        OpponentData("Helios Duo Megiste", ["Quick-Finish"], 3, 5, lambda state: state.yugioh06_difficulty(player, 3)),
        OpponentData("Gilford the Legend", ["Quick-Finish"], 4, 1, lambda state: state.yugioh06_difficulty(player, 5)),
        OpponentData("Dark Eradicator Warlock", ["Quick-Finish"], 4, 2,
                     lambda state: state.yugioh06_difficulty(player, 5)),
        OpponentData("Guardian Exode", [], 4, 3, lambda state: state.yugioh06_difficulty(player, 5)),
        OpponentData("Goldd, Wu-Lord of Dark World", ["Quick-Finish"], 4, 4,
                     lambda state: state.yugioh06_difficulty(player, 5)),
        OpponentData("Elemental Hero Erikshieler", ["Quick-Finish"], 4, 5,
                     lambda state: state.yugioh06_difficulty(player, 5)),
        OpponentData("Raviel, Lord of Phantasms", [], 5, 1, lambda state: state.yugioh06_difficulty(player, 6)),
