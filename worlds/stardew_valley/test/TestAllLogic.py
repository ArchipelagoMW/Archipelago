...
def test_given_item_rule_then_can_be_resolved(item: str):
    ...
    assert rule == _False() or rule(multi_world.state), f"Could not resolve rule for {item} {rule}"
...
def test_given_location_rule_then_can_be_resolved(location: StardewLocation):
    ...
    assert MISSING_ITEM not in repr(rule)
    assert rule == _False() or rule(multi_world.state), f"Could not resolve rule for {location} {rule}"
