...
class TestBaseItemGeneration(SVTestBase):
    ...
    def test_all_progression_items_are_added_to_the_pool(self):
        ...
        items_to_ignore = [event.name for event in item_classes.events]
        items_to_ignore.extend(item.name for item in item_classes.all_items if item.mod_name is not None)
        items_to_ignore.extend(deprecated.name for deprecated in item_classes.items_by_group[Group.DEPRECATED])
        items_to_ignore.extend(season.name for season in item_classes.items_by_group[Group.SEASON])
        items_to_ignore.extend(weapon.name for weapon in item_classes.items_by_group[Group.WEAPON])
        items_to_ignore.extend(baby.name for baby in item_classes.items_by_group[Group.BABY])
        items_to_ignore.extend(resource_pack.name for resource_pack in item_classes.items_by_group[Group.RESOURCE_PACK])
        items_to_ignore.append("The Gateway Gazette")
        progression_items = [item for item in item_classes.all_items if item.classification & ItemClassification.progression and item.name not in items_to_ignore]
    ...
    def test_does_not_create_deprecated_items(self):
        all_created_items = [item.name for item in self.multiworld.itempool]
        for deprecated_item in item_classes.items_by_group[item_classes.Group.DEPRECATED]:
    ...
    def test_does_not_create_more_than_one_maximum_one_items(self):
        all_created_items = [item.name for item in self.multiworld.itempool]
        for maximum_one_item in item_classes.items_by_group[item_classes.Group.MAXIMUM_ONE]:
    ...
    def test_does_not_create_exactly_two_items(self):
        all_created_items = [item.name for item in self.multiworld.itempool]
        for exactly_two_item in item_classes.items_by_group[item_classes.Group.EXACTLY_TWO]:
...
class TestNoGingerIslandItemGeneration(SVTestBase):
    ...
    def test_all_progression_items_except_island_are_added_to_the_pool(self):
        ...
        items_to_ignore.extend(deprecated.name for deprecated in item_classes.items_by_group[Group.DEPRECATED])
        items_to_ignore.extend(season.name for season in item_classes.items_by_group[Group.SEASON])
        items_to_ignore.extend(season.name for season in item_classes.items_by_group[Group.WEAPON])
        items_to_ignore.extend(baby.name for baby in item_classes.items_by_group[Group.BABY])
        items_to_ignore.append("The Gateway Gazette")
        progression_items = [item for item in item_classes.all_items if item.classification & ItemClassification.progression and item.name not in items_to_ignore]
        for progression_item in progression_items:
