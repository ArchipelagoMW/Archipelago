...
class TestBaseItemGeneration(SVTestBase):
    ...
    def test_all_progression_items_are_added_to_the_pool(self):
        ...
        items_to_ignore.extend(item.name for item in item_data.all_items if item.mod_name is not None)
        ...
        progression_items = [item for item in item_data.all_items if item.classification & ItemClassification.progression and item.name not in items_to_ignore]
...
class TestNoGingerIslandItemGeneration(SVTestBase):
    ...
    def test_all_progression_items_except_island_are_added_to_the_pool(self):
        ...
        items_to_ignore.extend(deprecated.name for deprecated in item_data.items_by_group[Group.DEPRECATED])
        items_to_ignore.extend(season.name for season in item_data.items_by_group[Group.SEASON])
        items_to_ignore.extend(season.name for season in item_data.items_by_group[Group.WEAPON])
        items_to_ignore.extend(baby.name for baby in item_data.items_by_group[Group.BABY])
        items_to_ignore.append("The Gateway Gazette")
        progression_items = [item for item in item_data.all_items if item.classification & ItemClassification.progression and item.name not in items_to_ignore]
        for progression_item in progression_items:
