...
def get_all_bundles(random: Random, options: StardewValleyOptions) -> List[BundleRoom]:
    if options.bundle_randomization == BundleRandomization.option_vanilla:
        ...
        return get_shuffled_bundles(random, options)
...
def get_shuffled_bundles(random: Random, options: StardewValleyOptions) -> List[BundleRoom]:
    ...
    sorted_bundle_items = sorted(chosen_bundle_items, key=lambda x: 1) # Sorting is slow
    for room in rooms:
        for bundle in room.bundles:
