...
def set_rules(world):
    ...
    rule_collector = StardewRuleCollector(world.multiworld, world.player, world_options)
    logic = world.logic
    ...
    set_entrance_rules(logic, rule_collector, world_options)
    set_ginger_island_rules(logic, rule_collector, world_options)
    ...
    set_special_order_rules(all_location_names, logic, rule_collector, world_options)
    ...
    set_backpack_rules(logic, rule_collector, world_options)
    ...
    set_isolated_locations_rules(logic, rule_collector)
    set_traveling_merchant_day_rules(logic, rule_collector)
    set_arcade_machine_rules(logic, rule_collector, world_options)

    set_deepwoods_rules(logic, rule_collector, world_options)
    set_magic_spell_rules(logic, rule_collector, world_options)
    set_sve_rules(logic, rule_collector, world_options)


def set_isolated_locations_rules(logic: StardewLogic, rule_collector: StardewRuleCollector):
...
def set_tool_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, content: StardewContent):
    ...
    for (previous, material), tool in itertools.product(zip(materials[:4], materials[1:]), tool):
        if previous is None:
            continue
        rule_collector.set_location_rule(f"{material} {tool} Upgrade", logic.tool.has_tool(tool, previous))
...
def set_entrance_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    ...
    set_skill_entrance_rules(logic, rule_collector, world_options)
    set_traveling_merchant_day_rules(logic, rule_collector)
    set_dangerous_mine_rules(logic, rule_collector, world_options)

    rule_collector.set_entrance_rule(Entrance.enter_tide_pools, logic.received("Beach Bridge") | (logic.mod.magic.can_blink()))
    rule_collector.set_entrance_rule(Entrance.enter_quarry, logic.received("Bridge Repair") | (logic.mod.magic.can_blink()))
    rule_collector.set_entrance_rule(Entrance.enter_secret_woods, logic.tool.has_tool(Tool.axe, "Iron") | (logic.mod.magic.can_blink()))
    rule_collector.set_entrance_rule(Entrance.forest_to_wizard_tower, logic.region.can_reach(Region.community_center))
    ...
    rule_collector.set_entrance_rule(Entrance.enter_movie_theater, movie_theater_rule)
    rule_collector.set_entrance_rule(Entrance.purchase_movie_ticket, movie_theater_rule)
    rule_collector.set_entrance_rule(Entrance.take_bus_to_desert, logic.received("Bus Repair") & logic.money.can_spend(500))
    ...
    set_bedroom_entrance_rules(logic, rule_collector, world_options)
...
def set_dangerous_mine_rules(logic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
...
def set_bedroom_entrance_rules(logic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
...
def set_skill_entrance_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
...
def set_festival_entrance_rules(logic, rule_collector: StardewRuleCollector):
    ...
    rule_collector.set_entrance_rule(LogicEntrance.attend_desert_festival, logic.season.has(Season.spring) & logic.received("Bus Repair"))
...
def set_ginger_island_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    set_island_entrances_rules(logic, rule_collector, world_options)
    if world_options.exclude_ginger_island == ExcludeGingerIsland.option_true:
        return
...
def set_island_entrances_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
...
def set_walnut_puzzle_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options):
    ...
    rule_collector.set_location_rule("Walnutsanity: Starfish Tide Pool", logic.tool.has_fishing_rod(1))
...
def set_walnut_repeatable_rules(logic, rule_collector: StardewRuleCollector, world_options):
    if WalnutsanityOptionName.repeatables not in world_options.walnutsanity:
        return
    for i in range(1, 6):
        rule_collector.set_location_rule(f"Walnutsanity: Fishing Walnut {i}", logic.tool.has_fishing_rod(1))
...
def set_story_quests_rules(all_location_names: Set[str], logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    ...
    for quest in locations.locations_by_tag[LocationTags.STORY_QUEST]:
        if quest.name in all_location_names and (quest.mod_name is None or quest.mod_name in world_options.mods):
            rule_collector.set_location_rule(quest.name, logic.registry.quest_rules[quest.name])
...
def set_help_wanted_quests_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    ...
    for i in range(0, help_wanted_number):
        ...
        if quest_number_in_set < 4:
            ...
            set_help_wanted_delivery_rule(rule_collector, month_rule, quest_number)
        elif quest_number_in_set == 4:
            set_help_wanted_fishing_rule(rule_collector, month_rule, quest_number)
        elif quest_number_in_set == 5:
            set_help_wanted_slay_monsters_rule(rule_collector, month_rule, quest_number)
        elif quest_number_in_set == 6:
            set_help_wanted_gathering_rule(rule_collector, month_rule, quest_number)


def set_help_wanted_delivery_rule(rule_collector: StardewRuleCollector, month_rule, quest_number):
    location_name = f"{help_wanted_prefix} {item_delivery} {quest_number}"
    rule_collector.set_location_rule(location_name, month_rule)


def set_help_wanted_gathering_rule(rule_collector: StardewRuleCollector, month_rule, quest_number):
    location_name = f"{help_wanted_prefix} {gathering} {quest_number}"
    rule_collector.set_location_rule(location_name, month_rule)


def set_help_wanted_fishing_rule(rule_collector: StardewRuleCollector, month_rule, quest_number):
    location_name = f"{help_wanted_prefix} {fishing} {quest_number}"
    rule_collector.set_location_rule(location_name, month_rule)


def set_help_wanted_slay_monsters_rule(rule_collector: StardewRuleCollector, month_rule, quest_number):
...
def set_backpack_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    if world_options.backpack_progression != BackpackProgression.option_vanilla:
        rule_collector.set_location_rule("Large Pack", logic.money.can_spend(2000))
        rule_collector.set_location_rule("Deluxe Pack", logic.money.can_spend(10000) & logic.received("Progressive Backpack"))
        if ModNames.big_backpack in world_options.mods:
            rule_collector.set_location_rule("Premium Pack", logic.money.can_spend(150000) & logic.received("Progressive Backpack", 2))
...
def set_friendsanity_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, content: StardewContent):
    if not content.features.friendsanity.is_enabled:
        return
    rule_collector.set_location_rule("Spouse Stardrop", logic.relationship.has_hearts_with_any_bachelor(13) & logic.relationship.can_get_married())
...
def set_deepwoods_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    if ModNames.deepwoods in world_options.mods:
        rule_collector.set_location_rule("Breaking Up Deep Woods Gingerbread House", logic.tool.has_tool(Tool.axe, "Gold"))
        rule_collector.set_location_rule("Chop Down a Deep Woods Iridium Tree", logic.tool.has_tool(Tool.axe, "Iridium"))
...
def set_magic_spell_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    if ModNames.magic not in world_options.mods:
        return

    rule_collector.set_location_rule("Analyze: Clear Debris", logic.tool.has_tool("Axe", "Basic") | logic.tool.has_tool("Pickaxe", "Basic"))
    rule_collector.set_location_rule("Analyze: Till", logic.tool.has_tool("Hoe", "Basic"))
    rule_collector.set_location_rule("Analyze: Water", logic.tool.has_tool("Watering Can", "Basic"))
    ...
    rule_collector.set_location_rule("Analyze All Life School Locations", logic.has_all("Coffee", "Life Elixir") & logic.ability.can_mine_perfectly())
    ...
    # rule_collector.set_location_rule("Analyze: Lantern",)
...
def set_sve_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    ...
    set_sve_ginger_island_rules(logic, rule_collector, world_options)
    set_boarding_house_rules(logic, rule_collector, world_options)


def set_sve_ginger_island_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
    if world_options.exclude_ginger_island == ExcludeGingerIsland.option_true:
        return
...
def set_boarding_house_rules(logic: StardewLogic, rule_collector: StardewRuleCollector, world_options: StardewValleyOptions):
