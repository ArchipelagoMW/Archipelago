...
from ..strings.geode_names import SVGeode
...
class Artifact:
    ...
    ancient_doll = create_artifact("Ancient Doll", 103, 13.1, (Region.mountain, Region.forest, Region.bus_stop),
                                   geodes=(SVGeode.artifact_trove, common.fishing_chest))
    ...
    dried_starfish = create_artifact("Dried Starfish", 116, 12.5, Region.beach,
                                     geodes=(SVGeode.artifact_trove, common.fishing_chest))
    anchor = create_artifact("Anchor", 117, 8.5, Region.beach, geodes=(SVGeode.artifact_trove, common.fishing_chest))
    ...
    dwarvish_helm = create_artifact("Dwarvish Helm", 121, 8.7, Region.mines_floor_20,
                                    geodes=(SVGeode.geode, SVGeode.omni, SVGeode.artifact_trove))
    dwarf_gadget = create_artifact("Dwarf Gadget", 122, 9.7, Region.mines_floor_60,
                                   geodes=(SVGeode.magma, SVGeode.omni, SVGeode.artifact_trove))
    ancient_drum = create_artifact("Ancient Drum", 123, 9.5, (Region.bus_stop, Region.forest, Region.town),
                                   geodes=(SVGeode.frozen, SVGeode.omni, SVGeode.artifact_trove))
...
class Mineral:
    ...
    fire_quartz = create_mineral("Fire Quartz", 82, Region.mines_floor_100,
                                 geodes=(SVGeode.magma, SVGeode.omni, common.fishing_chest),
                                 difficulty=1.0 / 12.0)
    frozen_tear = create_mineral("Frozen Tear", 84, Region.mines_floor_60,
                                 geodes=(SVGeode.frozen, SVGeode.omni, common.fishing_chest),
                                 monsters=unlikely,
                                 difficulty=1.0 / 12.0)
    earth_crystal = create_mineral("Earth Crystal", 86, Region.mines_floor_20,
                                   geodes=(SVGeode.geode, SVGeode.omni, common.fishing_chest),
