...
def get_home_points(player: Optional[int], options: Optional[CrystalProjectOptions]) -> List[LocationData]:
    ...
    home_point_table: List[LocationData] = [
        ...
        LocationData(ROLLING_QUINTAR_FIELDS_AP_REGION, "HomePoint - Quintar Sanctum", 917, lambda state: logic.has_rental_quintar(state, ROLLING_QUINTAR_FIELDS_DISPLAY_NAME) or logic.has_vertical_movement(state)),
        ...
        LocationData(SARA_SARA_BEACH_EAST_AP_REGION, "HomePoint - Ibek's Cave", 2005, lambda state: logic.has_vertical_movement(state)),
        LocationData(SARA_SARA_BEACH_WEST_AP_REGION, "HomePoint - Beach Bird's Nest", 2709, lambda state: logic.has_vertical_movement(state)),
