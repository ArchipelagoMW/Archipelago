...
* *Deprecation warning: clients that connect with version 0.3.5 or higher will nolonger recieve Print packets, instead all messages are sent as [PrintJSON](#PrintJSON).*
#### Arguments
| Name | Type | Notes |
| ---- | ---- | ----- |
...
#### PrintJsonType
PrintJsonType indicates the type of a [PrintJSON](#PrintJSON) packet. Different types can be handled differently by the client and can also contain additional arguments. When receiving an unknown type, the `data`'s list\[[JSONMessagePart](#JSONMessagePart)\] should still be printed as normal.
