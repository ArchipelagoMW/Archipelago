...
def connect_regions(world: WL4World):
    def connect_level(level_name):
        if level_name == 'Hotel Horror' and world.options.difficulty == Difficulty.option_s_hard:
            rule = None
        else:
            rule = rules.get_access_rule(world.player, level_name)
        connect_entrance(world, level_name, f'{level_name} (entrance)', level_name, rule)
    ...
    connect_level_exit('Golden Passage', 'Golden Minigame Shop',
            lambda state: world.options.open_doors == OpenDoors.option_open or
                          (state.has('Progressive Grab', world.player) and
                           state.has('Progressive Ground Pound', world.player)))
