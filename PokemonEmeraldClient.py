...
class GBACommandProcessor(ClientCommandProcessor):
    ...
    def _cmd_gba(self):
        """Check Game Boy Advance Connection State"""
        if isinstance(self.ctx, GBAContext):
            logger.info(f"Game Boy Advance Status: {self.ctx.gba_status}")
...
# async def parse_locations(data: List, ctx: GBAContext):
#     locations = []
        ...
        if ctx.gba_streams:
            ...
            try:
                ...
                try:
                    ...
                    #         "and PokemonRBClient are from the same Archipelago installation."
...
async def run_game(rom_file):
    auto_start = Utils.get_options()["pokemon_emerald_options"].get("rom_start", True)
    if auto_start is True:
        import webbrowser
        webbrowser.open(rom_file)
    elif os.path.isfile(auto_start):
        subprocess.Popen([auto_start, rom_file],
...
async def patch_and_run_game(patch_file, ctx):
    ...
    delta_patch = PokemonEmeraldDeltaPatch
...
if __name__ == '__main__':
    Utils.init_logging("PokemonEmeraldClient")
    ...
    async def main():
        ...
        parser.add_argument('patch_file', default="", type=str, nargs="?",
                            help='Path to an apemerald patch file')
        ...
        if gui_enabled:
            ctx.run_gui()
        ctx.run_cli()
        ctx.gb_sync_task = asyncio.create_task(gba_sync_task(ctx), name="GB Sync")
