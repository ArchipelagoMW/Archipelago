...
class KH2CommandProcessor(CommandProcessor):
    def __init__(self, ctx: KH2Context):
        self.ctx = ctx
    ...
    def _cmd_autotrack(self):
        """Track AutoTracking for Kingdom Hearts 2""" 
        kh2=pymem.Pymem(process_name="KINGDOM HEARTS II FINAL MIX")
        kh2.write_bytes(self.ctx.kh2save + 0x3594,(2).to_bytes(1,'big'),2)
...
class KH2Context:
    ...
    command_processor: typing.Type[CommandProcessor] = KH2CommandProcessor
    ui = None
    ...
    def __init__(self, server_address: typing.Optional[str], password: typing.Optional[str]) -> None:
        ...
        self.pcsx2 = False
        self.kh2save = 0x09A7070 - 0x56450E
        self.kh2sys3 = 0x2A09C00 - 0x56450E
...
async def keep_alive(ctx: KH2Context, seconds_between_checks=100):
...
async def server_loop(ctx: KH2Context, address: typing.Optional[str] = None) -> None:
...
async def server_autoreconnect(ctx: KH2Context):
    await asyncio.sleep(ctx.current_reconnect_delay)
    if ctx.server_address and ctx.server_task is None:
        ctx.server_task = asyncio.create_task(server_loop(ctx), name="server loop")


async def process_server_cmd(ctx: KH2Context, args: dict):
    try:
        cmd = args["cmd"]
    except:
...
async def console_loop(ctx: KH2Context):
...
async def kh2_write(ctx: KH2Context):
    print()
